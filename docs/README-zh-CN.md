...
| `kelly-instructor-sourcing` | 沙盒版讲师候选人筛选台：为招聘类平台上找到的讲师候选人建立筛选假设和三维评分标准（人物背书、专业深广度、教学服务能力），逐一打分，并在人工于应用外加上微信后记录建联状态。v1 所有数据均为固定的演示数据——不连接任何真实平台或消息账号。 | 需要用统一标准而非临时判断来筛选讲师候选人，或者想跟踪合格/淘汰与微信建联结果，但暂时不需要一个真连平台的外联工具时使用。 | [查看 ↗](https://mr-kelly.github.io/skills/s/kelly-instructor-sourcing.html?lang=zh) |
...
| `kelly-wechat-crm` | 从明确目标出发，在本机微信通讯录中发现候选人，只把用户选中的人晋升为重点联系人、关系快照、下一步和 Agent 工作日志。 | 在不复制完整通讯录、不发送消息、不修改微信数据的前提下，发现并管理少量重点关系时使用。 | [查看 ↗](https://mr-kelly.github.io/skills/s/kelly-wechat-crm.html?lang=zh) |
