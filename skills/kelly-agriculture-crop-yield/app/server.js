...
const AIRAPP_ID = "kelly-agriculture-crop-yield";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-agriculture-crop-yield" }));
...
app.onError((error, context) => {
  console.error("Precision Agronomy & Crop Yield Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33200", 10);
