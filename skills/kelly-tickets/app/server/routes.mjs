...
async function state() {
  ...
  return {
    app: "kelly-tickets",
    data_provider: process.env.KELLY_TICKETS_DATA_PROVIDER || configResult.config.data_provider || "local",
    onboarding,
    lock,
...
async function handleDecision(req, res) {
  ...
  if (lock) {
    sendJson(res, 423, { error: "Agent lock present; decisions are read-only right now.", lock });
    return;
  }
  let payload;
  try {
