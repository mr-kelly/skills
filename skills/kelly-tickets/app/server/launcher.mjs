...
const explicitPort = process.env.KELLY_TICKETS_UI_PORT || "";
    ...
    const req = http.get(stateUrlFor(port), { timeout: 500 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode >= 200 && res.statusCode < 300 && state.app === "kelly-tickets");
        } catch {
          resolve(false);
        }
      });
...
async function main() {
  ...
  if (await isReady(port)) {
    console.log(`Kelly Tickets UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.mjs"], {
    ...
    env: { ...process.env, KELLY_TICKETS_UI_HOST: host, KELLY_TICKETS_UI_PORT: port }
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly Tickets UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly Tickets UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
