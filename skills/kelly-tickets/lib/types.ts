// Core domain types shared across the kelly-tickets data-provider layer, the
// Hono server, scripts, and the demo builder. These model the ACTUAL shapes
// produced by demo.ts / the local provider and the snapshot documented in
// references/tickets-schema.md — a complaint desk (intake -> classify ->
// dispatch -> resolution board).
...
// ---- Config (as it appears in config.example.json / config.local.json) ----
...
// Result of loadConfig() in lib/data-provider/index.ts; passed to providers so
// they can echo the resolved config source without re-reading it.
...
// Input to a human verdict on a board item (intake / ticket / dispatch proposal).
...
// Result of applyDecision / submitDecision — { ok } on success, or an error
// with an HTTP status the server maps onto the response.
...
// Aggregate payload behind GET /api/state (non-demo). Mirrors what the server
// assembled inline before the refactor.
