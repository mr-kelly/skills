...
// lib/ sits alongside app/ under the skill root. These resolve to the SAME
// on-disk locations the server used before the data-provider refactor
// (app/.data/*.json), so the local provider is byte-compatible with the
// pre-refactor store.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "tickets_snapshot.json");
