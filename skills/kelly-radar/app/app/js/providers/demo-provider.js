...
// demoWatchlist/target/source/demoSignals/signal/diffLine/evidence/
// demoResearch/reportSource/demoTrends/mover/localizeSnapshotZh below are
...
function demoSnapshot(scenario = "overview") {
...
function demoWatchlist() {
  return [
    ...
    ...item,
    signals_7d: demoSignals().filter(
      (signal) =>
        signal.target_id === item.target_id && Date.parse(signal.detected_at) >= Date.parse(now) - 7 * 24 * 3600 * 1000,
    ).length,
  }));
}
...
function target(target_id, name, type, status, notes, last_check_at, sources) {
  return { target_id, name, type, status, notes, last_check_at, sources };
}
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-radar");
    return {
      demo: true,
      demo_scenario: scenario,
      app: "kelly-radar",
