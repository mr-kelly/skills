...
import type { Mover, RadarSnapshot } from "../app/server/types.ts";
...
const payload = await readJson<any>(payloadPath, null);
...
let seoImported: any[] = [];
if (seoSnapshotPath) {
  const seo = await readJson<any>(seoSnapshotPath, null);
  if (!seo) {
...
try {
  const snapshot: RadarSnapshot = (await readJson<RadarSnapshot>(SNAPSHOT_PATH, null)) || emptySnapshot();
  ...
  const keyFor = (mover: { keyword: string; source: string }) => `${mover.keyword.toLowerCase()}::${mover.source}`;
