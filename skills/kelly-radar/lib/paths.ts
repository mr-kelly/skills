...
// lib/ is a sibling of app/ under the skill root. Storage paths are anchored to
// app/.data so the local provider reads and writes the SAME files the original
// store.ts used — a config switch, not a data migration.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "radar_snapshot.json");
