...
const AIRAPP_ID = "kelly-revops-commission";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-revops-commission" }));
...
app.onError((error, context) => {
  console.error("Sales Commission & Territory Plan Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33361", 10);
