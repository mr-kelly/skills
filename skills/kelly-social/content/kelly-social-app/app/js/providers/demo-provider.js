...
// Ported verbatim from the retired app/server/demo.ts (same account/post/
// draft/short/engagement ids and copy, same Chinese localization map) — only
// the gate is now computed through social-model.js's evaluateGate() (byte
// -identical logic to the retired lib/social-qa.ts) and the rollup through
// social-model.js's rollup(), instead of local copies.
import { evaluateGate, rollup } from "../social-model.js?v=0.1.0";
...
function demoDrafts() {
  ...
  return raw.map((draft) => {
    ...
    return { ...draft, status, gate, created_at: "2026-07-01T18:00:00.000Z", updated_at: "2026-07-02T08:15:00.000Z" };
  });
}
...
// Localize the ECHO publishing-side + monitoring demo content into Chinese.
// The gate verdict and checks stay computed from the (original-language)
...
function localizeSnapshotZh(snapshot) {
  const accountNames = { "x-kelly": "Kelly 造物日志", "ig-kelly": "Kelly 工作室", "fb-kelly": "Kelly 工作室主页" };
  const postText = {
  ...
  snapshot.accounts = snapshot.accounts.map((acc) => ({
    ...acc,
    display_name: accountNames[acc.account_id] || acc.display_name,
  }));
  snapshot.posts = snapshot.posts.map((p) => ({
    ...p,
    text: postText[p.post_id] || p.text,
    agent_notes: agentNotes[p.post_id] || p.agent_notes,
  }));
...
function demoSnapshot(scenario, zh) {
  const accounts = demoAccounts(scenario);
  const posts = demoPosts();
  const sync_log = demoSyncLog();
    ...
    return {
      ...
      config_summary: {
        ...
        accounts: snapshot.accounts.map((acc) => ({
          ...
          secret_envs: acc.collection === "api" ? [`KELLY_SOCIAL_${acc.platform.toUpperCase()}_TOKEN_DEMO`] : [],
          secrets_ready: true,
        })),
      },
      demo_visuals: demoVisualsForApp("kelly-social"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-social") },
    };
  },

  async applyOperation() {
    // Demo mode never mutates real state; the app treats a resolved provider
    // response with no snapshot change as "reload the deterministic scene."
