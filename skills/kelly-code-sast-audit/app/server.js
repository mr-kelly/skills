...
const AIRAPP_ID = "kelly-code-sast-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-code-sast-audit" }));
...
app.onError((error, context) => {
  console.error(
    "Static Application Security Testing (SAST) Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33335", 10);
