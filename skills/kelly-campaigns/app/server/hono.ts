...
// contract and reaches storage only through the data-provider layer (lib/), so
// the same app runs under @hono/node-server locally and — once the data layer
// moves to a cloud provider (Busabase) — on other fetch-based runtimes.
//
...
console.log(`Kelly Campaigns data provider: ${provider.kind}`);
...
async function state() {
  return { app: "kelly-campaigns", ...(await provider.getState()) };
}
