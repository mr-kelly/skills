...
const AIRAPP_ID = "kelly-endpoint-edr";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-endpoint-edr" }));
...
app.onError((error, context) => {
  console.error("Endpoint EDR Threat Detection Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33337", 10);
