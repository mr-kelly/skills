...
const AIRAPP_ID = "kelly-cold-chain";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-cold-chain" }));
...
app.onError((error, context) => {
  console.error("Cold Chain IoT Excursion Incident Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33318", 10);
