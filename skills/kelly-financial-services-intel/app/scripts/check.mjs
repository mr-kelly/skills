...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Financial Services Intel must be Cloud-only");
if (
  /KELLY_FINANCIAL_SERVICES_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(
    configText,
  )
) {
...
console.log(`Kelly Financial Services Intel AirApp checks OK (${files.length} files)`);
