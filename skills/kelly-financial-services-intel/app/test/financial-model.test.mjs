...
import {
  ...
  parseBatchMeta,
  statusForAction,
} from "../app/js/financial-model.js";
...
test("baseSignalFields/normalizeSignal round trip preserves every field", () => {
  const signal = {
    ...
    kind: "signal",
    ref: 1,
    title: "A market event needs a clear client explanation",
...
test("baseActionFields/normalizeAction round trip preserves linked_signal_ids array", () => {
  const fields = baseActionFields({
    action_id: "action-1",
    ref: 1,
    title: "Draft a sourced internal market brief",
    summary: "summary",
    status: "needs_review",
    priority: "high",
    owner: "operator",
...
test("baseDraftFields/normalizeDraft: effective_body prefers edited_body over body", () => {
  const fields = baseDraftFields({
    ...
    ref: 1,
    channel: "client memo",
    title: "client memo: today's approved angle",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-financial-services-intel-20260806",
    ...
    buyer: "financial-service founders and family office operators",
    offer: "daily financial-services intelligence that becomes sourced internal briefs",
  };
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    drafts: [{ draft_id: "draft-1", ref: 1, channel: "client memo", status: "blocked" }],
    sources: [{ source_id: "news", label: "News" }],
    meta: { batch_id: "b1", buyer: "financial-service founders" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "client memo", title: "client memo: today's angle", next_step: undefined };
