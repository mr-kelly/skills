...
- preferred channels among parent WhatsApp, WeChat post, course pitch;
...
The app uses local HTTP on `127.0.0.1`, preferring port `3000` through `4000`, or `KELLY_EDUCATION_INTEL_UI_PORT` when set.
...
- `#/drafts` and `#/drafts/<id>`: editable parent WhatsApp, WeChat post, course pitch drafts with approve/request-changes/block decisions.
...
```bash
node skills/kelly-education-intel/scripts/validate_ui_schema.ts skills/kelly-education-intel/app/.data/current_batch.json
```
