...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly CLM must be Cloud-only");
if (/KELLY_CLM_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly CLM AirApp checks OK (${files.length} files)`);
