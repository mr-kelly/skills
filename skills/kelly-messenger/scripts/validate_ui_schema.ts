...
// This validator walks untyped JSON parsed off disk, so the traversed values are
// intentionally `any`; the require* helpers enforce the real shape at runtime.
function isObject(value: any): boolean {
  return value && typeof value === "object" && !Array.isArray(value);
}

function requireString(obj: any, key: string, path: string): void {
  if (typeof obj[key] !== "string" || obj[key].length === 0) fail(`${path}.${key} must be a non-empty string`);
}

function requireNumber(obj: any, key: string, path: string): void {
  if (typeof obj[key] !== "number" || Number.isNaN(obj[key])) fail(`${path}.${key} must be a number`);
}

function requireBoolean(obj: any, key: string, path: string): void {
  if (typeof obj[key] !== "boolean") fail(`${path}.${key} must be a boolean`);
}

async function readJsonOrFail(target: string, label: string): Promise<any> {
  const raw = await fs.readFile(target, "utf8").catch((error) => {
    fail(`cannot read ${label} ${target}: ${(error as Error).message}`);
  });
  try {
    return JSON.parse(raw);
  } catch (error) {
    fail(`invalid JSON in ${label}: ${(error as Error).message}`);
  }
}
