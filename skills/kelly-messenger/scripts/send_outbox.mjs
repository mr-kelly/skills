...
import { EXECUTION_REPORT_PATH, LOCK_PATH, OUTBOX_PATH } from "../app/server/paths.mjs";
import {
  ensureDirs,
  envSearchPaths,
  loadDotenvFiles,
...
async function main() {
  ...
  for (const plan of plans) {
    console.log(`  - Reply #${plan.reply.ref} [${plan.reply.platform}] -> ${plan.target || "(no target)"} via ${plan.operation}`);
  ...
  await writeJson(LOCK_PATH, { owner: "kelly-messenger", message: "Sending approved outbox replies", started_at: nowIso() });
  const results = [];
  try {
    ...
    for (const plan of plans) {
      ...
      if (!reply || reply.status !== "approved") {
        results.push(result(plan.reply, "skipped", plan.operation, plan.target, "No longer approved at send time.", plan.connector));
        continue;
      }
      if (plan.blocker) {
      ...
      if (plan.operation === "handoff_to_agent") {
        reply.execution = execution("handoff", "handoff_to_agent", plan.target, `Agent must deliver this reply via the ${reply.platform} ${plan.connector} flow.`);
...
async function sendVia(plan) {
  ...
  if (plan.connector === "slack") {
    ...
    const body = await postJson("https://slack.com/api/chat.postMessage", { channel, text }, { authorization: `Bearer ${token}` });
  ...
  if (plan.connector === "discord") {
    ...
    await postJson(`https://discord.com/api/v10/channels/${channel}/messages`, { content: text }, { authorization: `Bot ${token}` });
    return;
  }
