...
function conversationDetail(conversation) {
  ...
  return `
    ...
    <div class="composer">
      ...
      <textarea id="composer-text" rows="4" placeholder="${escapeHtml(t("replyPlaceholder"))}">${escapeHtml(prefill)}</textarea>
      <input id="composer-note" type="text" placeholder="${escapeHtml(t("notePlaceholder"))}" value="${escapeHtml(state.notes[conversation.conversation_id] || "")}">
      <div class="composer-actions">
        <button type="button" class="primary" data-action="queue-reply" data-conversation="${escapeHtml(conversation.conversation_id)}">${t("queueReply")}</button>
      </div>
    </div>
  `;
}
      ...
      ${replies
        .map((reply) => {
          ...
          <article class="outbox-card" data-reply-card="${escapeHtml(reply.reply_id)}">
            ...
            ${
              editable
                ? `<textarea class="outbox-text" data-reply-text rows="4">${escapeHtml(value)}</textarea>`
            ...
            ${
              ...
              <div class="outbox-actions">
                <input type="text" data-reply-comment placeholder="${escapeHtml(t("commentPlaceholder"))}">
                <div class="outbox-buttons">
                  <button type="button" class="primary" data-action="decide" data-reply="${escapeHtml(reply.reply_id)}" data-decision="approve">${t("approve")}</button>
                  <button type="button" data-action="decide" data-reply="${escapeHtml(reply.reply_id)}" data-decision="request_changes">${t("requestChanges")}</button>
                  <button type="button" data-action="decide" data-reply="${escapeHtml(reply.reply_id)}" data-decision="revise">${t("saveEdit")}</button>
                  <button type="button" class="danger" data-action="decide" data-reply="${escapeHtml(reply.reply_id)}" data-decision="block">${t("block")}</button>
                </div>
              </div>
            `
                : ""
...
export async function queueReplyAction(conversationId) {
  ...
  try {
    ...
    await provider.queueReply({ conversation_id: conversationId, text, note, suggested_by: "human" });
  } catch (error) {
    flashNotice(error.message || "Queue failed");
    return;
  }
  delete state.drafts[conversationId];
  delete state.notes[conversationId];
...
export async function decideAction(replyId, action, card) {
  ...
  try {
    const provider = await getProvider();
    await provider.decideReply({ reply_id: replyId, action, comment, text });
  } catch (error) {
