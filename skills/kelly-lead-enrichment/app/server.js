...
const AIRAPP_ID = "kelly-lead-enrichment";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-lead-enrichment" }));
...
app.onError((error, context) => {
  console.error("Lead Enrichment & Firmographic Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3203", 10);
