...
const AIRAPP_ID = "kelly-incident-postmortem";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-incident-postmortem" }));
...
app.onError((error, context) => {
  console.error(
    "Blameless Incident Postmortem & Action Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33376", 10);
