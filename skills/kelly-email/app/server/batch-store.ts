...
import { BATCH_DIR, CACHE_DIR, CURRENT_BATCH_PATH, DECISIONS_PATH, LEGACY_REVIEW_ITEMS_PATH } from "./paths.ts";
import type { Batch, DecisionsPayload, ItemDecision, ReviewItem } from "./types.ts";
...
async function convertLegacyCache(): Promise<Batch> {
  ...
  const raw = (await readJson(LEGACY_REVIEW_ITEMS_PATH)) as {
...
export async function writeDecisions(batch: Batch): Promise<DecisionsPayload> {
