...
```text
.agents/skills/kelly-email/config.example.yml
```

Put account settings, aliases, folders, identities, user profile, brands, official URLs, knowledge sources, reply style, risk keywords, and `password_env` names in YAML. Put actual IMAP/SMTP app passwords or tokens in the `.env` file only.
...
Secrets stay in env files. The YAML config should only reference secret env variable names.
