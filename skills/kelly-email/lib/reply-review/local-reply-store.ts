...
import type { OpenReplyDraftInput, ReplyRecord, ReplyReviewInput, ReplyReviewMeta } from "../types.ts";

interface LocalStore {
  reviews: Record<string, ReplyRecord>;
}

function storePath(skillDir: string | undefined) {
  return path.join(skillDir || "", "app", ".data", "reply_reviews.json");
}

async function readStore(file: string): Promise<LocalStore> {
  try {
    return JSON.parse(await fs.readFile(file, "utf8")) as LocalStore;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return { reviews: {} };
    throw error;
  }
}

async function writeStore(file: string, data: LocalStore) {
  await fs.mkdir(path.dirname(file), { recursive: true });
  await fs.writeFile(file, `${JSON.stringify(data, null, 2)}\n`);
}

export function createLocalReplyStore(meta: ReplyReviewMeta = {} as ReplyReviewMeta) {
  const file = storePath(meta.skillDir);

  function nextId(reviews: Record<string, ReplyRecord>) {
  ...
  return {
    ...
    async openReplyDraft({ email_id, to, subject, draft, thread_id }: OpenReplyDraftInput) {
    ...
    async reviewReply(reply_id: string, { verdict, edits, comment }: ReplyReviewInput = {}) {
      const store = await readStore(file);
      const r = store.reviews?.[reply_id];
      if (!r) {
        const error = new Error(`reply not found: ${reply_id}`) as Error & { statusCode?: number };
      ...
      if (verdict === "approve") r.status = "approved";
        ...
        const error = new Error(`unknown verdict: ${verdict}`) as Error & { statusCode?: number };
    ...
    async getApprovedReply(reply_id: string) {
      const store = await readStore(file);
      const r = store.reviews?.[reply_id];
    ...
    async markSent(reply_id: string, result?: unknown) {
      const store = await readStore(file);
      const r = store.reviews?.[reply_id];
