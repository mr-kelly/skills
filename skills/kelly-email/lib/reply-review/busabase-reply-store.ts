...
import type { OpenReplyDraftInput, ReplyReviewInput, ReplyReviewMeta } from "../types.ts";
...
export function createBusabaseReplyStore(meta: ReplyReviewMeta = {} as ReplyReviewMeta) {
  const busa: Record<string, any> = meta.config?.busabase || {};
  ...
  async function getCr(reply_id: string) {
  ...
  return {
    ...
    async markSent(reply_id: string) {
