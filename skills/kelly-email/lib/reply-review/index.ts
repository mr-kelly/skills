...
import type { Config, ReplyReviewMeta } from "../types.ts";
import { createBusabaseReplyStore } from "./busabase-reply-store.ts";
import { createLocalReplyStore } from "./local-reply-store.ts";
...
async function loadConfig(): Promise<ReplyReviewMeta> {
  for (const candidate of configCandidates()) {
    try {
      return { config: JSON.parse(await fs.readFile(candidate, "utf8")) as Config, source: candidate };
    } catch (error) {
