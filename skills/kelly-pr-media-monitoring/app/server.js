...
const AIRAPP_ID = "kelly-pr-media-monitoring";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-pr-media-monitoring" }));
...
app.onError((error, context) => {
  console.error(
    "PR Media Distribution & Share of Voice Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33385", 10);
