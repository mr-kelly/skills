...
const AIRAPP_ID = "kelly-threat-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-threat-intel" }));
...
app.onError((error, context) => {
  console.error("Cyber Threat Intelligence & IOC Radar server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33338", 10);
