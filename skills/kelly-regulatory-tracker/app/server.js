...
const AIRAPP_ID = "kelly-regulatory-tracker";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-regulatory-tracker" }));
...
app.onError((error, context) => {
  console.error(
    "Financial & Environmental Regulatory Tracker server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33353", 10);
