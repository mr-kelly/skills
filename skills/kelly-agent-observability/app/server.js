...
const AIRAPP_ID = "kelly-agent-observability";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-agent-observability" }));
...
app.onError((error, context) => {
  console.error("Kelly Agent Observability server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3168", 10);
