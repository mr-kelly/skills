...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Agent Observability must be Cloud-only");
if (/KELLY_AGENT_OBS|KELLY_AGENT_OBSERVABILITY_CONFIG|local-file-provider|config\.local\.json/.test(configText)) {
...
const forbidden = ["local-file-provider.ts", "launcher.ts", "start.sh", "setup-gate.js", "setup-gate.css", "store.ts"];
...
console.log(`Kelly Agent Observability AirApp checks OK (${files.length} files)`);
