...
The normal acceptance path runs the merged AirApp inside Busabase and returns its clickable URL. When local preview is explicitly requested, the standalone app opens with a Cloud/custom-server connection screen and one-click browser OAuth, plus an explicit read-only Demo path. It never requires CLI login or an API-key input. After OAuth, one accessible Space is selected automatically; multiple Spaces require a native selector before any resource initialization. Open-source `local` is single-Space and skips the selector. OAuth tokens stay in an owner-only per-AirApp registration under `~/.busabase/airapps`; deployed AirApps use the ambient Busabase session and current Space.

Starting, opening, or launching an existing local app always uses the bare non-Demo URL so the Busabase connection and OAuth gate remain visible. `?demo=1` is opt-in for a requested demo, screenshot, recording, or a separate UI acceptance pass; it is never the default URL handed to the operator.
