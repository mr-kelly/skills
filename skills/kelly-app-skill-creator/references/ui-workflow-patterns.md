...
- immediate visual feedback after a recorded Busabase decision.
...
Auto-refresh Busabase-backed state on a timer, but do not redraw while the user is actively editing a textarea or non-search input.

The app may still poll claim/lock state during edits so the user can see when the Agent is processing.
...
Show safe summaries only. Never expose secrets, Vault values, raw credentials, or private document contents.

Help & Settings should let the user confirm:

- Busabase server and Space,
- app Folder/root node,
- Base, Doc, Drive, and File resources,
- schema and AirApp sync versions,
- account summaries,
- identities,
- profile,
- style choices,
- official links,
- knowledge sources,
- Vault reference readiness without values.
...
Default language mode should be `Auto`, following `navigator.languages`/browser language. Also provide an explicit language selector in Help & Settings for supported languages and persist a durable override in the operator's Busabase preferences.
