...
6. Show immediate recorded feedback: toast, badge, changed row state, or updated decision label.
...
- `Approved` / `Ready for agent next`: proof that the approval was recorded.
...
Never record an action that performs a real external side effect. Demo actions must use a dedicated Busabase demo/test Space or a clearly isolated presentational simulation:

- Seed deterministic, non-sensitive records through the same Busabase repository and schema used by the app.
- Demo decision endpoints may write only to the dedicated demo/test Space and must not touch external systems.
...
- Vault values or secret env values,
- production Spaces or resources,
- live account data,
- external services.
