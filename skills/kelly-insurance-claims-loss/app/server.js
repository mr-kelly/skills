...
const AIRAPP_ID = "kelly-insurance-claims-loss";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-insurance-claims-loss" }));
...
app.onError((error, context) => {
  console.error(
    "Property & Casualty Loss Adjustment Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33330", 10);
