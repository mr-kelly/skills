...
export const busabaseProvider = {
  ...
  async getState() {
    ...
    const items = itemRows.map(computeItemFromRow);
    ...
    return {
      ...
      onboarding: { completed: vehicleRows.length > 0, config_version: "1" },
      lock: null,
      config_summary,
  ...
  // the item record. Ported from the retired local-file DataProvider's
  // recordDecision(): the item's decision action/comment/decided_at/
  // override_reconciliation all live on the same row — there is no separate
  // decisions.json bucket.
  async submitReview({ id, action, comment = "", overrideReconciliation = false } = {}) {
    ...
    if (!existing) throw new Error(`Disclosure item not found: ${id}`);
    ...
    const fields = {
      ...
      decided_at: now,
      override_reconciliation: overrideReconciliation ? "true" : "false",
    };
