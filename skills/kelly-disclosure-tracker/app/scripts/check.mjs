...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Disclosure Tracker must be Cloud-only");
if (/KELLY_DISCLOSURE_TRACKER_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Disclosure Tracker AirApp checks OK (${files.length} files)`);
