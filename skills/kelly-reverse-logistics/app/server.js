...
const AIRAPP_ID = "kelly-reverse-logistics";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-reverse-logistics" }));
...
app.onError((error, context) => {
  console.error("Returns, RMA & Refurbishment Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33320", 10);
