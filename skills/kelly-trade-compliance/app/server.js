...
const AIRAPP_ID = "kelly-trade-compliance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-trade-compliance" }));
...
app.onError((error, context) => {
  console.error(
    "Export Control & Sanctions Screening Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33352", 10);
