...
const AIRAPP_ID = "kelly-cloud-finops-advisor";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-cloud-finops-advisor" }));
...
app.onError((error, context) => {
  console.error("Multi-Cloud Reserved Instance Optimizer server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33334", 10);
