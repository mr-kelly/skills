...
const AIRAPP_ID = "kelly-influencer-kol";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-influencer-kol" }));
...
app.onError((error, context) => {
  console.error(
    "Influencer Marketing & ROAS Tracking Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33384", 10);
