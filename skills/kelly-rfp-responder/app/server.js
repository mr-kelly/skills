...
const AIRAPP_ID = "kelly-rfp-responder";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-rfp-responder" }));
...
app.onError((error, context) => {
  console.error("Enterprise RFP Response Knowledge Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33367", 10);
