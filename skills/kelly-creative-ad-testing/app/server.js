...
const AIRAPP_ID = "kelly-creative-ad-testing";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-creative-ad-testing" }));
...
app.onError((error, context) => {
  console.error(
    "Creative Ad Variant Performance & Visual Tester server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33390", 10);
