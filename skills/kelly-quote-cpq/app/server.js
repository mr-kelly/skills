...
const AIRAPP_ID = "kelly-quote-cpq";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-quote-cpq" }));
...
app.onError((error, context) => {
  console.error("CPQ Quote & Discount Approval Console server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3204", 10);
