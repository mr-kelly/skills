...
function requireIsoDate(obj: JsonObject, key: string, path: string, allowEmpty = false): void {
  const value = obj[key];
...
function requireStringArray(obj: JsonObject, key: string, path: string): void {
  if (!Array.isArray(obj[key]) || obj[key].some((item: unknown) => typeof item !== "string")) {
