...
// buildDemoSnapshot() below is ported verbatim (same member/day/blocker/
// reminder ids, same copy, same order of operations) from the retired
// app/server/demo.ts's buildDemoSnapshot(); only the recomputeDerived import
// changed (from "./store.ts" to standup-model.js, the same pure function).
// The retired demo.ts's `scenario` parameter never affected the snapshot
// content (only which hash route app.js jumped to), so it is dropped here —
// the route selection lives in app.js's applyDemoRoute()-equivalent.
import { recomputeDerived } from "../standup-model.js?v=0.1.0";
...
export function buildDemoSnapshot(zh = false) {
...
export const demoProvider = {
  ...
  async getState() {
    const params = new URLSearchParams(window.location.search);
    const scenario = String(params.get("demo") || "today");
    const zh = activeLangIsZh();
    const snapshot = buildDemoSnapshot(zh);
    return {
      demo: true,
      ...
      demo_visuals: demoVisualsForApp("kelly-standup"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-standup") },
    };
  },
