...
import { recomputeDerived } from "./store.mjs";
...
export function buildDemoSnapshot(zh = false, scenario = "today") {
  ...
  const blockers = [
    blocker("bl-alex-stripe", "alex", "2026-07-01", "high", "open",
      L("Waiting on Stripe production API keys from vendor onboarding", "等待 Stripe 生产环境 API 密钥（供应商开通流程中）"),
      L("Maya to escalate with the Stripe account rep today — the keys gate the billing launch.", "建议林晓今天向 Stripe 客户经理催办——密钥卡住计费功能上线。")),
    blocker("bl-sam-e2e", "sam", "2026-07-02", "medium", "open",
      L("Flaky signup e2e blocks the release train", "注册 e2e 用例抖动卡住发布列车"),
      L("Merge the race-condition fix and re-enable the suite; hold the release until CI is green.", "合入竞态修复并重新启用用例集；CI 变绿前先冻结发布。")),
    blocker("bl-marco-backlog", "marco", "2026-06-30", "medium", "open",
      L("App Store review reply backlog is above the 48h SLA", "App Store 评论回复积压超过 48 小时 SLA"),
      L("Approve the reply macro so Marco can batch the top 20 reviews this afternoon.", "尽快批准回复模板，让马晨下午批量处理前 20 条评论。")),
    blocker("bl-jonah-review", "jonah", "2026-06-29", "low", "resolved",
  ...
  const dayDates = ["2026-06-19", "2026-06-22", "2026-06-23", "2026-06-24", "2026-06-25",
    "2026-06-26", "2026-06-29", "2026-06-30", "2026-07-01", "2026-07-02"];

  const digests = [
    L("Sprint 14 kickoff: billing page scoped and the usage metering spec agreed.", "第 14 个迭代启动：计费页范围确定，用量计费方案达成一致。"),
    L("Onboarding funnel up 4%; two churned-team interviews booked for next week.", "注册转化提升 4%；约到两个流失团队下周回访。"),
    L("Checkout flow first cut landed behind the flag; support backlog creeping up.", "结账流程初版已在功能开关后合入；支持工单积压开始上升。"),
    L("Design review prep day; a bad migration blocked staging deploys until noon.", "设计评审准备日；一个坏迁移把 staging 部署堵到中午。"),
    L("Usage metering PR opened; App Store replies slipped past the SLA.", "用量计费 PR 提交评审；App Store 回复超出 SLA。"),
    L("Onboarding design review held — two blockers cleared, one sign-off pending.", "完成注册流程设计评审——清掉两个阻塞，剩一个待签核。"),
    L("Quiet Monday: webhook retries fixed, pricing decision moved to Tuesday.", "平静的周一：webhook 重试修复，定价决策移到周二。"),
    L("Annual pricing decided; Stripe production access requested.", "年付定价敲定；已申请 Stripe 生产环境权限。"),
    L("Stripe keys still pending — flagged as a high blocker; billing page copy finalized.", "Stripe 密钥仍未下发——升级为高优先阻塞；计费页文案定稿。"),
    L("Flaky signup e2e caught blocking the release train; fix identified.", "发现注册 e2e 抖动卡住发布列车；修复方案已定位。")
  ];
  ...
  const days = dayDates.map((date, dayIndex) => {
    ...
    const updates = members
      ...
      .map((entry, memberIndex) => {
        ...
        const resolvedHere = blockers.filter((item) => item.member_id === entry.member_id && item.resolved_date === date);
    ...
    return { date, digest: digests[dayIndex], on_leave: onLeave, updates, participation: { submitted: 0, expected: 0, on_leave: 0 } };
  });

  const openBlocker = (id) => updateBlocker(blockers.find((item) => item.blocker_id === id), "open");
  ...
  const todayUpdates = [
    update("maya", TODAY, "00:42", "slack", {
      ...
      blockers: [],
      mood: "good",
      raw: L("yday: billing copy review w/ Jonah, pricing closed. today: investor update, chase stripe keys, churn interviews.", "昨天：和一凡过计费文案、定价敲定。今天：投资人月报、催 Stripe 密钥、流失回访。")
    }),
    update("alex", TODAY, "00:51", "slack", {
      ...
      mood: "ok",
      raw: L("checkout flow done behind flag. still blocked on stripe prod keys (day 3).", "结账流程已在开关后完成。仍被 Stripe 生产密钥卡住（第 3 天）。")
    }),
    update("sam", TODAY, "01:05", "discord", {
      ...
      mood: "ok",
      raw: L("found the race in email stub. fix up today, suite back on in CI.", "找到邮件桩里的竞态。今天提修复，CI 重新跑起来。")
    }),
    update("priya", TODAY, "01:14", "slack", {
      ...
      blockers: [],
      mood: "good",
      raw: L("metering PR is up; staging unblocked. pairing with alex pm.", "计费 PR 已提交；staging 已解堵。下午和陈嘉结对。")
    }),
    update("jonah", TODAY, "01:26", "doc", {
      ...
      blockers: [updateBlocker(blockers.find((item) => item.blocker_id === "bl-jonah-review"), "resolved")],
      mood: "good",
      raw: L("empty states done, review signed off yesterday. specs to alex today.", "空状态完成，评审昨天签核。今天把标注给陈嘉。")
    }),
    update("marco", TODAY, "01:48", "whatsapp", {
      ...
      mood: "stuck",
      raw: L("14/32 reviews done. backlog still above SLA — need the macro approved.", "回复 14/32。积压仍超 SLA——需要尽快批准模板。")
    })
  ];
  ...
  const reminders = [
    {
      ...
      reason: L("No check-in by 10:30 team time; last submitted yesterday. Everyone else is in.", "团队时间 10:30 仍未提交；上次提交是昨天。其他人都已提交。"),
      draft: L(
    ...
    },
    {
      ...
      reason: L("Alex has been blocked for 3 days on a high-severity item that gates the billing launch.", "陈嘉被这个高优先阻塞卡了 3 天，直接影响计费功能上线。"),
      draft: L(
    ...
    },
    {
      ...
      reason: L("Missed the Jul 1 check-in while heads-down on the review backlog.", "7 月 1 日忙于处理评论积压，漏交了日报。"),
      draft: L(
        ...
        operations: [
          {
            ...
            contact_ready: true,
            message_draft: L("Hey Marco, quick nudge — your Wednesday standup is missing.", "马晨，小提醒——周三的日报还没交。")
          }
        ],
        detail: L("Sent via kelly-messenger after approval; Marco checked in 20 minutes later.", "审批后经 kelly-messenger 发送；马晨 20 分钟后补交了日报。"),
    ...
    sync_log: [
      {
        ...
        source: "slack",
        action: "ingest",
        detail: L("Ingested 7 updates for 2026-07-02 from the #daily-standup export.", "从 #daily-standup 导出中收录 2026-07-02 的 7 条日报。"),
        count: 7
      },
      {
        at: NOW,
        source: "slack",
        action: "ingest",
        detail: L("Ingested 6 updates for 2026-07-03; Tessa missing, Ingrid on leave; 1 reminder drafted.", "收录 2026-07-03 的 6 条日报；苏蔓未提交，白洁休假；已起草 1 条提醒。"),
        count: 6
      }
    ],
    warnings: []
