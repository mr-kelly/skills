...
// Canonical path resolver for kelly-standup. lib/ lives directly under the skill
// root, so skillDir is one level up from here. The .data/*.json handoff files
// keep the exact locations the app has always used, so switching store.ts logic
// into the local-file provider is path-for-path identical (byte-identical
// /api/state, same files on disk).
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "standup_snapshot.json");
