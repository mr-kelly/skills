...
const AIRAPP_ID = "kelly-product-usage-health";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-product-usage-health" }));
...
app.onError((error, context) => {
  console.error(
    "PLG Product Usage Adoption & Health Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33365", 10);
