...
const AIRAPP_ID = "kelly-asset-lifecycle";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-asset-lifecycle" }));
...
app.onError((error, context) => {
  console.error(
    "Enterprise Fixed Asset Lifecycle Console server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3193", 10);
