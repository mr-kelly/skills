...
Support a macOS-like accent color picker for operator apps unless the app is truly read-only or visually branded. Keep this as an accent color, not a full skin: selected rows, active tabs, focus rings, links, primary workflow buttons, badges, and human-attention highlights should read from CSS variables, while the app stays neutral. Put compact swatches in `Help & Settings`, persist the selected accent in `localStorage`, and keep native checkboxes/radios aligned with `accent-color`.
