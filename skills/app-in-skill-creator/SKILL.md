...
- Support an Apple/macOS-like accent color system for operator apps unless the app is truly read-only or visually branded. Treat this as a system accent, not a full decorative skin: neutral surfaces stay neutral, while selected rows, active tabs, focus rings, primary workflow buttons, links, badges, and human-attention highlights read from CSS custom properties. Use a token set that separates bright display color from accessible UI color: `--accent`, `--accent-strong`, `--accent-soft`, `--accent-wash`, `--accent-line`, `--accent-focus`, `--accent-text`, and `--accent-contrast`. Start from Apple-style system accents such as Blue, Purple, Pink, Red, Orange, Yellow, Green, and Graphite; deepen `--accent-strong`/`--accent-text` as needed so white text on primary buttons reaches at least 4.5:1 contrast. Put the picker in `Help & Settings` as compact circular swatches with a subtle selected ring/check, persist the choice in `localStorage`, and set `accent-color: var(--accent)` so native checkboxes/radios match. Verify the picker is balanced at desktop width (ideally one tidy row for eight colors) and phone width (wrap without horizontal overflow). Do not make the palette dominate the app or introduce separate layout/theme variants unless the user explicitly asks for a full skin.
