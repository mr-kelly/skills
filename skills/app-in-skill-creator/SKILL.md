...
- Support a macOS-like accent color picker for operator apps unless the app is truly read-only or visually branded. Keep this as an accent-color system, not a full decorative skin: neutral surfaces stay neutral, while selected rows, active tabs, focus rings, primary workflow buttons, links, badges, and human-attention highlights use CSS custom properties such as `--accent`, `--accent-soft`, `--accent-line`, `--accent-focus`, and `--accent-contrast`. Offer a compact swatch/radio control in `Help & Settings` (for example Blue, Purple, Pink, Red, Orange, Yellow, Green, Graphite), persist the choice in `localStorage`, and set `accent-color: var(--accent)` so native checkboxes/radios match. Do not make the palette dominate the app or introduce separate layout/theme variants unless the user explicitly asks for a full skin.
