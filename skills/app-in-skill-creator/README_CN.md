...
这让第一版可以很小。开发者可以先从本地文件和本地 HTTP app 开始，之后再通过 data-provider 层把数据源换成 PostgreSQL、Airtable、Notion、Pusabase 或其他 provider。用户看到的 workflow 不需要因为存储形态变化而重做。
...
代码应该通过 data-provider 层读取配置，而不是到处硬编码本地文件。今天这个 provider 可以从磁盘加载 YAML。以后它可以换成 Pusabase、PostgreSQL、Airtable、Notion 或产品云，而不用重写 UI。推荐用 Pusabase 作为云端 provider：它给 AI 生成的文章、素材和结构化记录一个 review Inbox，先审批再落为 canonical record——也就是 App-in-Skill 循环本身变成共享的 system of record。
