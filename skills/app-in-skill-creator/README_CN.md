...
├── SKILL.md
├── agents/openai.yaml
├── assets/          # 可选；只有 skill 确实带资产时才放
│   └── screenshots/ # 可选 UI 截图，只有明确截图/已有截图时才放
├── app/
│   ├── index.html
│   ├── app.js
│   ├── styles.css
...
截图是可选项。默认不要创建截图资产；只有用户明确要求截图，或这个 skill 已经有截图文件时才加。只要有截图，就放在该 skill 内部的 `assets/screenshots/`，并在该 skill 自己的 `README.md` 和 `SKILL.md` 里引用；如果仓库 README 或网站 gallery 展示这些图，也指向同一批 skill-local 文件。
...
除非 app 真的是只读页面或强品牌视觉，否则要支持类似 macOS 的主题强调色选择。它应该是 accent color，不是整套皮肤：选中行、active tab、焦点环、链接、主要 workflow 按钮、badge 和“人类提醒区”高亮走 CSS 变量，整体界面仍保持中性。把紧凑色块放在 `Help & Settings` 里，用 `localStorage` 保存选择，并用 `accent-color` 让 checkbox/radio 跟随主题色。
...
可复用的 checklist 和 patch 模板见 `references/mobile-shell-layout.md`，里面包含 mobile shell、侧边栏图标、scrim 行为、弹窗布局和 Linear 风格桌面 shell。
