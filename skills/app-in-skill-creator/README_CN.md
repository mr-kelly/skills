...
视觉资产是可选项。默认不要创建截图或演示录屏；只有用户明确要求，或这个 skill 已经有视觉文件时才加。截图放在 `assets/screenshots/`。walkthrough 视频如果是 skill 包的一部分，可以放在 `assets/demo-recordings/`，但 MP4 必须走 Git LFS；否则 repo 内只保留外部路径、录制方法或简短摘要。
