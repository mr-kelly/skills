...
const AIRAPP_ID = "kelly-api-security-gateway";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-api-security-gateway" }));
...
app.onError((error, context) => {
  console.error("API Security & Schema Violation Monitor server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33339", 10);
