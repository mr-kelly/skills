...
const AIRAPP_ID = "kelly-quote-to-cash";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-quote-to-cash" }));
...
app.onError((error, context) => {
  console.error("Quote-to-Cash (Q2C) Deal Approval Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33363", 10);
