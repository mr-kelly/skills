...
const AIRAPP_ID = "kelly-cspm-checker";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-cspm-checker" }));
...
app.onError((error, context) => {
  console.error("Cloud Security Posture & CSPM Inspector server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3185", 10);
