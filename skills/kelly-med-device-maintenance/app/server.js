...
const AIRAPP_ID = "kelly-med-device-maintenance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-med-device-maintenance" }));
...
app.onError((error, context) => {
  console.error(
    "Hospital Medical Device Maintenance Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33304", 10);
