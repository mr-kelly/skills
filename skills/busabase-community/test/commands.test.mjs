...
describe("the publish gate", () => {
  ...
  test("`new --yes` posts once, to the right URL, with the user's key", async () => {
    ...
    assert.equal(calls[0].url, "https://busabase.com/api/v1/community/posts");
...
describe("accounts", () => {
  ...
  test("an unknown account refuses instead of falling back to the default", async () => {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => {
        assert.match(error.message, /BUSABASE_API_KEY_GHOST is not set/);
        return true;
      },
    );
  ...
  test("$BUSABASE_ACCOUNT moves the default, and --as still overrides it", async () => {
    const env = { ...ENV, BUSABASE_ACCOUNT: "alt" };
...
describe("failure modes", () => {
  ...
  test("a 401 names the credential that was actually rejected", async () => {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => {
        assert.match(error.message, /BUSABASE_API_KEY_ALT/);
        return true;
      },
    );
  });
