...
const AIRAPP_ID = "kelly-event-marketing";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-event-marketing" }));
...
app.onError((error, context) => {
  console.error(
    "Summit Event Marketing & Badge Scan Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33383", 10);
