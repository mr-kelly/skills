...
const AIRAPP_ID = "kelly-saas-finops";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-saas-finops" }));
...
app.onError((error, context) => {
  console.error(
    "SaaS License & Subscription FinOps Console server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3184", 10);
