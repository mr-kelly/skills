...
export function configSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-matter-strategy", "config.json"));
...
export function envSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-matter-strategy", ".env"));
  return paths;
}
