#!/usr/bin/env node
// Trusted hand-off step. Legal Matter Strategy's AirApp only ever proposes a
// review decision on a strategy pack (approve / request changes / revise /
// block); this script is the process authorized to act on that decision. It
// performs NO external side effects — it never files documents, sends
// client advice, contacts counterparties, or exports anything itself. This
// mirrors kelly-legal-casebase-ingest's and kelly-legal-firm-radar's
// execute_decisions.mjs precedent: it only ever records a planned operation
...
// real follow-up (export the strategy pack via
// scripts/export_strategy_pack.mjs) is performed by the agent OUTSIDE the
...
import { itemExecution, normalizeItemRow } from "../app/app/js/matter-strategy-model.js";
...
follow-up operation (export_strategy_pack / request_revision) for each. With
...
(scripts/export_strategy_pack.mjs), then records the real result.`);
}
...
async function main() {
  ...
  if (!decided.length) {
    console.log("No approved or changes-requested strategy packs to execute. Nothing written.");
    return;
  }
  ...
  for (const row of decided) {
    ...
    if (apply) {
      await client.records.changeRequest({
        ...
        author: "kelly-legal-matter-strategy-execute-decisions",
