#!/usr/bin/env node
// Trusted hand-off step. Legal Matter Strategy's AirApp never imports matter/
// issue-tree/evidence/risk data itself — the browser cannot read an
// arbitrary local file path. This script parses a JSON payload file (the
// same shape the retired scripts/create_strategy_batch.ts accepted per
// references/strategy-schema.md's "Payload Import" section: any of
...
//
// Usage: node scripts/create_strategy_batch.mjs payload.json [more.json ...] [--apply]
...
async function upsert(client, declared, existing, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "kelly-legal-matter-strategy-create-strategy-batch",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "kelly-legal-matter-strategy-create-strategy-batch",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (const input of incomingItems) {
      ...
      const fields = {
        ...
        proposed_action: input.proposed_action ?? existing?.proposed_action ?? "approve_strategy_pack",
        draft: input.draft ?? existing?.draft ?? "",
        evidence: JSON.stringify(input.evidence ?? (existing ? JSON.parse(existing.evidence || "[]") : [])),
        matter_stage: input.fields?.matter_stage ?? existing?.matter_stage ?? "",
        evidence_gap_count: input.fields?.evidence_gap_count ?? existing?.evidence_gap_count ?? "",
        evidence_gaps_list: JSON.stringify(
          input.fields?.evidence_gaps_list ?? (existing ? JSON.parse(existing.evidence_gaps_list || "[]") : []),
        ),
        issue_tree: JSON.stringify(
          input.fields?.issue_tree ?? (existing ? JSON.parse(existing.issue_tree || "[]") : []),
        ),
        negotiation_options: JSON.stringify(
          input.fields?.negotiation_options ?? (existing ? JSON.parse(existing.negotiation_options || "[]") : []),
        ),
        posture: input.fields?.posture ?? existing?.posture ?? "",
        pleading_outline: input.fields?.pleading_outline ?? existing?.pleading_outline ?? "",
        deadline: input.fields?.deadline ?? existing?.deadline ?? "",
  ...
  if (!apply) {
    ...
    console.log(
      `Merged ${itemWrites.length} item(s), ${entityWrites.length} entit(y/ies), ${checkWrites.length} check(s) into Busabase.`,
    );
  }
}
