#!/usr/bin/env node
// Trusted operator script. Parses local PDF backups with pdftotext/pdfinfo
// and previews (or, with --apply, writes) the Busabase Asset text slot plus
// generated governance metadata for each Drive file. Ported verbatim from
// the retired scripts/backfill_pdf_metadata.ts — same heuristics, same CLI
// flags — onto scripts/lib/busabase-client.mjs (raw fetch; busabase-sdk has
// no Asset text/metadata endpoints). Reads local PDF bytes with fs, which a
// browser AirApp cannot do, so this stays a skill-root Node script.
...
function inferCarrier(filePath, text) {
...
function inferDocumentType(filePath, text) {
...
function inferPolicyType(filePath, text) {
  const haystack = `${filePath}\n${text}`;
...
function inferProductName(filePath, text) {
...
function metadataFor(file, text, pages) {
  ...
  return {
    ...
    parser: {
      method: "pdftotext + path heuristics",
      confidence: "medium",
      parsed_text_chars: parsedText.length,
    },
    ...
    parsed_text_chars: parsedText.length,
    extraction_summary: parsedText.replace(/\s+/g, " ").slice(0, 500),
    governance: {
      status: "needs_review",
      completeness_pct: 75,
      missing_fields: ["human_reviewed_at", "source_url"],
      review_note: "Generated by PDF text-slot backfill from parsed PDF text and file path.",
    },
  };
}
...
async function main() {
  ...
  const files = (await client.listDriveFiles(driveNodeId))
  ...
  await mapLimit(files, concurrency, async (file) => {
    ...
    try {
      ...
      const metadata = metadataFor(file, extractedText, pages);
      metadata.source_file.drive_node_id = driveNodeId;
      if (apply) {
        await client.updateAssetText(file.assetId, extractedText);
        await client.updateAssetMetadata(file.assetId, metadata, "replace");
      }
      results.push({
        path: file.path,
        assetId: file.assetId,
        applied: apply,
        text_status: apply ? "updated" : "planned",
