#!/usr/bin/env node
// Trusted operator script. Exports a portable restore manifest (folder,
// Drive, Base schemas, Drive file + Base record values, sanitized Asset
// metadata) for the active Kelly Insure Data Busabase workspace. Ported
// verbatim from the retired scripts/export_busabase_snapshot.ts onto
// scripts/lib/busabase-client.mjs — same CLI flags, same manifest shape (see
// references/restore-manifest-schema.md).
...
function text(value) {
  ...
  if (value && typeof value === "object") {
    return String(value["zh-CN"] || value["zh-TW"] || value.zh || value.en || "");
  }
  return "";
}
...
function fallbackBase(slug, name, fields) {
  return {
    id: "",
    node_id: "",
    slug,
...
async function main() {
  ...
  const featuredBase = bases.find((base) => base.slug === featuredSlug);
  const noticesBase = bases.find((base) => base.slug === noticesSlug);
  const qaBase = bases.find((base) => base.slug === qaSlug);
  const feedbackBase = bases.find((base) => base.slug === feedbackSlug);
  ...
  const files = await mapLimit(driveFiles, concurrency, async (file) => {
    ...
    if (file.assetId) {
      try {
        ...
        assetMetadata = sanitizeMetadata(asset.metadata || {});
