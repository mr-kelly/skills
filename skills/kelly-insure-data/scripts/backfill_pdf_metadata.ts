...
function metadataFor(file: AnyRecord, text: string, pages: number) {
  ...
  return {
    ...
    governance: {
      ...
      review_note: "Generated by PDF metadata backfill from parsed PDF text and file path.",
    },
  };
}
...
async function main() {
  ...
  await mapLimit(files, concurrency, async (file) => {
    ...
    try {
      ...
      if (apply) {
        await client.updateAssetMetadata(file.assetId, metadata, "merge");
      }
