...
const AIRAPP_ID = "kelly-identity-governance-iga";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-identity-governance-iga" }));
...
app.onError((error, context) => {
  console.error("IGA Access Certification Review Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33340", 10);
