...
const AIRAPP_ID = "kelly-expense-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-expense-audit" }));
...
app.onError((error, context) => {
  console.error(
    "Corporate Expense & Receipt Audit Console server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3192", 10);
