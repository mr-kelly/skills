...
2. Turns each visitor message into a **ticket** on the same channel (`channel: "webchat"`, `connector: "webchat_widget"`), writing through the desk's own Busabase Bases with the gateway's own trusted credentials (`busabase-sdk`'s `bases.createChangeRequest` for a new ticket, `records.changeRequest` to append messages) — the same write path `scripts/execute_decisions.mjs` and the AirApp itself use, never the AirApp's ambient session.
...
- It needs a public, authenticated gateway with abuse protection — a network surface the desk deliberately does not have (the AirApp itself reads and writes Busabase records only and never exposes a public endpoint of its own).
