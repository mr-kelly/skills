...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Support must be Cloud-only");
if (/KELLY_SUPPORT_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Support AirApp checks OK (${files.length} files)`);
