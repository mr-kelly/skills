...
// Ported verbatim from the retired app/server/demo.ts (same ticket_id/ref
// values, same copy, same persona "Nimbus Notes"). Derived fields
// (last_message_at/last_incoming_at, sla.breached, quality_gate, metrics)
...
function sla(policy, due_by, first_response_at = "") {
...
function demoTickets() {
  return [
    ...
    ticket(
      ...
      ["kb-refunds"],
      sla("first_response", "2026-07-06T13:00:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-export"],
      sla("first_response", "2026-07-06T17:00:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-export"],
      sla("first_response", "2026-07-06T07:00:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-sync-offline"],
      sla("first_response", "2026-07-06T06:30:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-billing-seats"],
      sla("first_response", "2026-07-06T16:20:00.000Z"),
      null,
      "Kelly",
      false,
    ...
    ticket(
      ...
      ["kb-2fa"],
      sla("first_response", "2026-07-06T12:00:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-sync-offline"],
      sla("first_response", "2026-07-06T12:30:00.000Z"),
      null,
      "Kelly",
      false,
      ...
      decision(
        "approve",
        "Send as is — good tone, lead with the re-login fix, offer to dig into session logs.",
    ...
    ticket(
      ...
      [],
      sla("first_response", "2026-07-07T08:15:00.000Z"),
      null,
      "Kelly",
      false,
    ...
    ticket(
      ...
      ["kb-billing-seats"],
      sla("first_response", "2026-07-06T07:30:00.000Z"),
      null,
      "Kelly",
      true,
    ...
    ticket(
      ...
      ["kb-2fa"],
      sla("first_response", "2026-07-05T12:00:00.000Z", "2026-07-05T10:12:00.000Z"),
      ...
      decision("approve", "", "2026-07-05T10:05:00.000Z"),
    ),
    ...
    ticket(
      ...
      [],
      sla("first_response", "2026-07-04T16:00:00.000Z", "2026-07-04T13:20:00.000Z"),
      ...
      decision("approve", "", "2026-07-04T13:15:00.000Z"),
    ),
    ...
    ticket(
      ...
      "",
      [],
      sla("first_response", "2026-07-06T08:05:00.000Z"),
      null,
      "Kelly",
      false,
      ...
      decision("block", "Spam pitch, not a support request. Block, no reply.", "2026-07-06T08:10:00.000Z"),
    ),
    ...
    ticket(
      ...
      ["kb-billing-seats"],
      sla("first_response", "2026-07-03T15:00:00.000Z", "2026-07-03T11:30:00.000Z"),
      ...
      decision("approve", "", "2026-07-03T11:25:00.000Z"),
    ),
    ...
    ticket(
      ...
      ["kb-import"],
      sla("first_response", "2026-07-06T18:00:00.000Z"),
      null,
      "Kelly",
      true,
...
function demoSnapshot() {
  ...
  for (const item of tickets) {
    refreshTicketDerived(item, NOW);
    item.quality_gate = runQualityGate(item, knowledge_base, RISK);
  }
...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.tickets = snapshot.tickets.map((t) =>
    t.decision && comments[t.ticket_id] ? { ...t, decision: { ...t.decision, comment: comments[t.ticket_id] } } : t,
  );
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-support");
    return {
      demo: true,
