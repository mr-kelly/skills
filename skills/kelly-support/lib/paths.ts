// Filesystem layout for the kelly-support data-provider layer.
//
// Single source of truth for where handoff state lives. The local provider
// reads/writes app/.data/*.json, so switching KELLY_SUPPORT_DATA_PROVIDER=
// local|busabase is a config flip, not a data migration. app/server/paths.ts
// re-exports these so the server, launcher, and scripts keep stable imports.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "support_snapshot.json");
...
export const PREFERRED_PORT_MIN = 3240;
export const PREFERRED_PORT_MAX = 3999;
