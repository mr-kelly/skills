...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Portfolio Health must be Cloud-only");
if (/KELLY_PORTFOLIO_HEALTH_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Portfolio Health AirApp checks OK (${files.length} files)`);
