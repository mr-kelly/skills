...
// local preview on loopback, before the RBF Portfolio Health shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the RBF Portfolio Health routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Portfolio Health will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Portfolio Health never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the RBF Portfolio Health shell mounts. Returns true once the app is clear to load
