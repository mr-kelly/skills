...
import { CACHE_DIR, DEFAULT_HOST, DEFAULT_PORT, LOG_PATH, PID_PATH, PREFERRED_PORT_MAX } from "./paths.ts";

const SERVER_DIR = path.dirname(fileURLToPath(import.meta.url));
const host = process.env.KELLY_PORTFOLIO_HEALTH_UI_HOST || DEFAULT_HOST;
const explicitPort = process.env.KELLY_PORTFOLIO_HEALTH_UI_PORT || "";
        ...
        try {
          ...
          resolve(
            res.statusCode !== undefined &&
              res.statusCode >= 200 &&
              res.statusCode < 300 &&
              state.app === "kelly-portfolio-health",
          );
        } catch {
          resolve(false);
        }
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`RBF Portfolio Health UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_PORTFOLIO_HEALTH_UI_HOST: host, KELLY_PORTFOLIO_HEALTH_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`RBF Portfolio Health UI started: http://${host}:${port}`);
  ...
  console.error(`RBF Portfolio Health UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
