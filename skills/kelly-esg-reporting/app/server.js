...
const AIRAPP_ID = "kelly-esg-reporting";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-esg-reporting" }));
...
app.onError((error, context) => {
  console.error("Corporate ESG Disclosure & Carbon Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33359", 10);
