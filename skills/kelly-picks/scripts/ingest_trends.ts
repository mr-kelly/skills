...
function contentHash(item: Partial<TrendItem>): string {
  return crypto
    .createHash("sha256")
...
try {
  ...
  const trendKey = (item: Partial<TrendItem>): string =>
  ...
  const candidateKey = (item: Candidate): string =>
  ...
  for (const sweep of sweeps) {
    ...
    if (source) {
      source.last_sweep_at = (sweep.swept_at as string) || now;
      source.status = "ok";
    }
  }
