...
// order of operations) from the retired app/server/demo.ts; only the
// per-proposal `verdictNote` string is now nested as `review: { comment,
// decided_at }` to match the live Busabase provider's proposal shape (a
...
function demoProposals() {
  return [
    ...
    },
    {
      ...
      review: { comment: "Confirmed drop 2026-06-19.", decided_at: "2026-06-19T09:00:00.000Z" },
    },
    {
    ...
    },
    {
      ...
      review: {
        comment: "Blocked on supplier pricing — nothing actionable until new quotes arrive.",
        decided_at: "2026-06-25T11:00:00.000Z",
      },
    },
  ];
}
...
function localizeSnapshotZh(snapshot) {
  ...
  const proposalText = {
    ...
    "prop-organizer": {
      ...
      reviewComment: "卡在供应商价格——新报价来之前没有可执行动作。",
    },
  };
  snapshot.proposals = snapshot.proposals.map((item) => {
    ...
    return {
      ...
      review: text.reviewComment
        ? { comment: text.reviewComment, decided_at: item.review?.decided_at || now }
        : item.review,
    };
  });
  snapshot.sync_log = [
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-picks");
    return {
      demo: true,
      demo_scenario: scenario,
      app: "kelly-picks",
