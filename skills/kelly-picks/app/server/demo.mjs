...
export function demoStatePayload(query = {}) {
  ...
  return {
    ...
    agent_tasks: {
      updated_at: now,
      tasks: [
        { task_id: "task-demo-1", kind: "revise_proposal", ref_id: "prop-egg-cooker", note: "Re-run margins with the sea-freight quote before we drop it.", created_at: now, status: "queued" }
      ]
    },
...
function demoConfigSummary() {
  return {
    ...
    platforms: [
      ...
      { platform_id: "tiktok_shop_us", name: "TikTok Shop US", currency: "USD", referral_fee_pct: 8, fulfillment_flat: 0 }
    ],
    freight: {
    ...
    sources: [
      { source_id: "amazon-bsr-home", kind: "amazon_bsr", name: "Amazon US BSR movers — Home & Kitchen", method: "browser_agent" },
      { source_id: "tiktok-kitchen", kind: "tiktok", name: "TikTok viral videos — #kitchengadgets", method: "browser_agent" },
      { source_id: "temu-home", kind: "temu", name: "Temu rising items — Home", method: "browser_agent" },
      { source_id: "aliexpress-kitchen", kind: "aliexpress", name: "AliExpress hot products — Kitchen", method: "browser_agent" },
      { source_id: "gtrends-kitchen", kind: "trends", name: "Google Trends — kitchen gadget queries", method: "browser_agent" },
      { source_id: "competitor-launches", kind: "competitor", name: "Competitor new launches — 4 tracked stores", method: "manual" }
    ],
...
export function demoSnapshot(scenario = "overview") {
  ...
  const metrics = {
    ...
    candidates_new_7d: candidates.filter((item) => Date.parse(item.first_seen) >= Date.parse("2026-06-25T00:00:00.000Z")).length,
    ...
    avg_margin_approved_pct: Math.round((approved.reduce((sum, item) => sum + item.margin_card.margin_pct, 0) / (approved.length || 1)) * 10) / 10,
  ...
  return {
    ...
    proposals,
    sync_log: [
      { at: "2026-07-02T08:40:00.000Z", actor: "kelly-picks-agent", action: "ingest_trends", detail: "TikTok + BSR sweep: 4 trend items ingested, 2 new candidates filed, 1 duplicate skipped." },
      { at: "2026-07-02T08:45:00.000Z", actor: "kelly-picks-agent", action: "compute_margins", detail: "12 margin cards recomputed from fee tables; 3 candidates below the 25% margin floor." },
      { at: "2026-07-01T19:20:00.000Z", actor: "kelly-picks-agent", action: "execute_decisions", detail: "Dry-run: 2 approved proposals → 3 planned operations (sourcing brief, listing brief handoff, watch)." }
    ]
  };
}
...
function demoSources() {
  return [
    source("amazon-bsr-home", "amazon_bsr", "Amazon US BSR movers — Home & Kitchen", "browser_agent", "2026-07-02T08:35:00.000Z", 3, "ok"),
    source("tiktok-kitchen", "tiktok", "TikTok viral videos — #kitchengadgets", "browser_agent", "2026-07-02T08:20:00.000Z", 4, "ok"),
    source("temu-home", "temu", "Temu rising items — Home", "browser_agent", "2026-07-01T21:10:00.000Z", 2, "ok"),
    source("aliexpress-kitchen", "aliexpress", "AliExpress hot products — Kitchen", "browser_agent", "2026-07-01T21:15:00.000Z", 1, "ok"),
    source("gtrends-kitchen", "trends", "Google Trends — kitchen gadget queries", "browser_agent", "2026-06-30T07:50:00.000Z", 2, "ok"),
    source("competitor-launches", "competitor", "Competitor new launches — 4 tracked stores", "manual", "2026-06-24T10:00:00.000Z", 0, "stale")
  ];
}

function demoTrendItems() {
  return [
    trendItem("tr-lunchbox-tiktok", "tiktok", "Collapsible silicone lunch box — creator demo at 2.1M views/week",
      ...
      "https://tiktok.example.com/@bentobabe/video/7381", "views/week", 2100000, 96, [12, 18, 26, 41, 55, 70, 86, 100],
      "2026-07-02T08:20:00.000Z", "cand-lunchbox", "tt-7381"),
    trendItem("tr-spice-rack-bsr", "amazon_bsr", "Magnetic spice rack jumped 5,214 → 612 in Home & Kitchen BSR",
      ...
      "https://amazon.example.com/bsr/home-kitchen", "BSR rank", 612, 88, [20, 24, 30, 42, 58, 74, 88, 100],
      "2026-07-02T08:35:00.000Z", "cand-spice-rack", "bsr-612-spice"),
    trendItem("tr-humidifier-tiktok", "tiktok", "'Cloud' night-light humidifier is back on the FYP",
      ...
      "https://tiktok.example.com/tag/cloudhumidifier", "views/week", 680000, 44, [30, 28, 34, 40, 48, 55, 61, 66],
      "2026-07-01T20:10:00.000Z", "cand-cloud-humidifier", "tt-cloud-2"),
    trendItem("tr-roller-tiktok", "tiktok", "Reusable pet hair roller: 1.4M views/week, checkout links everywhere",
      ...
      "https://tiktok.example.com/tag/pethairroller", "views/week", 1400000, 71, [22, 30, 36, 45, 58, 70, 84, 92],
      "2026-07-02T08:20:00.000Z", "cand-pet-hair-roller", "tt-roller-1"),
    trendItem("tr-sink-caddy-temu", "temu", "Expandable sink caddy climbing Temu Home best-sellers",
      ...
      "https://temu.example.com/home-organizers", "rank", 7, 52, [18, 22, 27, 35, 41, 50, 58, 63],
      "2026-07-01T21:10:00.000Z", "cand-sink-caddy", "temu-caddy-7"),
    trendItem("tr-chopper-ali", "aliexpress", "14-in-1 vegetable chopper reorder volume rising on AliExpress",
      "Top three suppliers show 3.2k combined orders in 30 days, up ~45%. US-plug variants ship from CN warehouses at $4.90-5.60.",
      "https://aliexpress.example.com/kitchen-hot", "orders/30d", 3200, 45, [40, 42, 45, 51, 56, 60, 66, 72],
      "2026-07-01T21:15:00.000Z", "cand-veggie-chopper", "ali-chopper-14"),
    trendItem("tr-oil-sprayer-gt", "trends", "'oil sprayer for cooking' query interest +80% in 90 days",
      ...
      "https://trends.example.com/oil-sprayer", "interest index", 78, 80, [30, 34, 38, 45, 52, 60, 70, 78],
      "2026-06-30T07:50:00.000Z", "cand-oil-dispenser", "gt-oil-78"),
    trendItem("tr-frother-gt", "trends", "'rechargeable milk frother' holding a high plateau after spring spike",
      "Interest settled ~35% above last year's baseline. Home-café content keeps the query warm outside gifting season.",
      "https://trends.example.com/milk-frother", "interest index", 64, 12, [70, 66, 60, 58, 60, 62, 63, 64],
      "2026-06-30T07:50:00.000Z", "cand-milk-frother", "gt-frother-64"),
    trendItem("tr-organizer-comp", "competitor", "HausWerk (tracked store) launched a bamboo drawer organizer line",
      ...
      "https://amazon.example.com/stores/hauswerk/new", "new SKUs", 6, 0, [],
      "2026-06-24T10:00:00.000Z", "cand-drawer-organizer", "comp-hauswerk-bamboo"),
    trendItem("tr-egg-cooker-bsr", "amazon_bsr", "Mini egg cooker re-entered the Small Appliance top 100",
      "Rank improved 41% in two weeks, but the price war is visible: median price fell from $18.99 to $15.99 in the same window.",
      "https://amazon.example.com/bsr/small-appliances", "BSR rank", 87, 41, [35, 38, 36, 44, 50, 58, 62, 68],
      "2026-07-02T08:35:00.000Z", "cand-egg-cooker", "bsr-87-egg"),
    trendItem("tr-ice-tray-temu", "temu", "Stackable ice tray with lid trending in Temu Kitchen",
      "Summer seasonality plus a viral 'clear ice' angle. Sold ~9k units platform-wide last week at $3.98-5.20.",
      "https://temu.example.com/kitchen-rising", "units/week", 9000, 38, [25, 28, 30, 36, 40, 47, 52, 55],
      "2026-07-01T21:10:00.000Z", "cand-ice-tray", "temu-ice-9k"),
    trendItem("tr-led-strip-tiktok", "tiktok", "LED strip room-makeover format still farming views",
      "High views but the compilation format is ad-driven; top listings have 40k+ reviews and $9.99 anchor prices.",
      "https://tiktok.example.com/tag/ledlights", "views/week", 3200000, 8, [95, 92, 96, 90, 94, 96, 93, 95],
      "2026-06-28T09:00:00.000Z", "cand-led-strip", "tt-led-old")
  ];
}
...
function demoCandidates() {
  return [
    {
      ...candidate("cand-lunchbox", "Collapsible silicone lunch box", "kitchen", "tiktok", "tr-lunchbox-tiktok", "reviewing", "amazon_us", "B", 96,
        21.99, 4.6, 2.1, 15, 3.6,
      ...
      competition: competition([214, 187, 150, 122, 98, 76, 61, 44, 28, 15], 11,
        ...
        4, "4 new entrants in 90 days, all under 50 reviews — early but not crowded yet."),
      evidence: [
    ...
    {
      ...candidate("cand-spice-rack", "Magnetic fridge spice rack", "home", "amazon_bsr", "tr-spice-rack-bsr", "new", "amazon_us", "B", 88,
        24.99, 7.2, 2.6, 15, 3.7,
      ...
      competition: competition([214, 168, 141, 120, 103, 88, 60, 41, 22, 9], 14,
        "Head seller is out of stock this week; second seller raised price by $3. Short window before both restock.",
        3, "3 new entrants in 90 days; velocity moderate."),
      evidence: [
    ...
    {
      ...candidate("cand-cloud-humidifier", "Cloud night-light humidifier", "home", "tiktok", "tr-humidifier-tiktok", "develop", "tiktok_shop_us", "B", 44,
        27.99, 6.8, 2.9, 15, 2.3,
      ...
      competition: competition([320, 240, 190, 150, 110, 85, 60, 40, 25, 12], 16,
        "Amazon shelf is mid-depth, but TikTok Shop is nearly empty: two listings, both with weak video assets.",
        5, "5 new entrants in 90 days, mostly Amazon-only — the TikTok Shop lane is open."),
      evidence: [
    ...
    {
      ...candidate("cand-pet-hair-roller", "Reusable pet hair roller", "home", "tiktok", "tr-roller-tiktok", "develop", "amazon_us", "A", 71,
        14.99, 2.1, 1.2, 15, 2.6,
      ...
      competition: competition([9500, 480, 260, 190, 140, 90, 70, 45, 30, 18], 62,
        ...
        6, "6 new entrants in 90 days and climbing; speed matters more than polish here."),
      evidence: [
        evidenceLink("#pethairroller tag velocity", "https://tiktok.example.com/tag/pethairroller"),
        evidenceLink("TikTok Shop listings (4, all <300 sales)", "https://shop.tiktok.example.com/search?q=pet+hair+roller")
      ]
    },
    {
      ...candidate("cand-milk-frother", "Rechargeable milk frother", "kitchen", "trends", "tr-frother-gt", "watch", "amazon_us", "C", 12,
        19.99, 4.9, 2.0, 15, 3.4,
      ...
      competition: competition([4100, 2800, 900, 640, 420, 300, 210, 150, 90, 60], 34,
        "Two entrenched sellers above 2.8k reviews; differentiation must come from the charging dock and gift packaging.",
        2, "Only 2 new entrants in 90 days — the shelf is mature, not hot."),
      evidence: [
    ...
    {
      ...candidate("cand-led-strip", "RGB LED strip lights 32ft", "home", "tiktok", "tr-led-strip-tiktok", "dropped", "amazon_us", "D", 8,
        12.99, 3.2, 1.6, 15, 3.9,
      ...
      competition: competition([41000, 28000, 15000, 9800, 7600, 5200, 3900, 2800, 1900, 1200], 36,
        "Fully saturated head: five listings above 7.5k reviews and brand ad walls on every keyword.",
        1, "New entrants have stopped appearing — the market already consolidated."),
      evidence: [
    ...
    {
      ...candidate("cand-sink-caddy", "Expandable sink caddy organizer", "home", "temu", "tr-sink-caddy-temu", "new", "amazon_us", "C", 52,
        16.99, 4.2, 1.9, 15, 2.9,
      ...
      competition: competition([1900, 850, 610, 430, 350, 240, 180, 120, 70, 40], 33,
        "One 1.9k-review leader, then a long tail. A rust-proof steel variant could take the mid shelf.",
        3, "3 new entrants in 90 days, all following the Temu design."),
      evidence: [
    ...
    {
      ...candidate("cand-veggie-chopper", "14-in-1 vegetable chopper", "kitchen", "aliexpress", "tr-chopper-ali", "reviewing", "amazon_us", "C", 45,
        18.99, 5.4, 2.3, 15, 3.3,
      ...
      competition: competition([5200, 3100, 1400, 800, 560, 400, 300, 210, 130, 80], 42,
        "Two big incumbents, but their 1-star reviews cluster on cracked containers — a materials upgrade is the wedge.",
        4, "4 new entrants in 90 days; mid velocity."),
      evidence: [
        evidenceLink("AliExpress hot products — kitchen", "https://aliexpress.example.com/kitchen-hot"),
        evidenceLink("Incumbent 1-star review cluster", "https://amazon.example.com/product-reviews/B0CHOP01?filterByStar=one_star")
      ]
    },
    {
      ...candidate("cand-oil-dispenser", "Glass oil sprayer dispenser", "kitchen", "trends", "tr-oil-sprayer-gt", "new", "amazon_us", "C", 80,
        13.99, 3.1, 1.5, 15, 2.4,
      ...
      competition: competition([2600, 1200, 700, 500, 380, 260, 180, 110, 60, 30], 39,
        "Leader owns the 'plastic' segment; the glass sub-niche head has only 700 reviews.",
        3, "3 new entrants in 90 days; glass variants underrepresented."),
      evidence: [
    ...
    {
      ...candidate("cand-drawer-organizer", "Bamboo drawer organizer set", "home", "competitor", "tr-organizer-comp", "watch", "amazon_de", "C", 0,
        22.99, 8.4, 3.4, 15, 3.2,
      ...
      competition: competition([1100, 640, 520, 380, 290, 200, 150, 90, 50, 20], 32,
        "HausWerk will own the head within two quarters based on their launch history. DE marketplace is one cycle behind US.",
        6, "6 new entrants in 90 days on .com; .de shelf still thin."),
      evidence: [
    ...
    {
      ...candidate("cand-egg-cooker", "Mini rapid egg cooker", "kitchen", "amazon_bsr", "tr-egg-cooker-bsr", "reviewing", "amazon_us", "D", 41,
        15.99, 5.9, 2.4, 15, 2.4,
        ...
        "2026-07-02T08:36:00.000Z"),
      competition: competition([31000, 5400, 2100, 1300, 900, 640, 420, 300, 180, 90], 73,
        "Dash owns 73% of review volume; everyone else fights for scraps under their price umbrella.",
        2, "2 new entrants in 90 days; both already discounting."),
      evidence: [
    ...
    {
      ...candidate("cand-ice-tray", "Stackable ice cube tray with lid", "kitchen", "temu", "tr-ice-tray-temu", "new", "tiktok_shop_us", "C", 38,
        11.99, 2.6, 1.3, 8, 2.2,
        ...
        "2026-07-01T21:20:00.000Z"),
      competition: competition([850, 620, 400, 310, 240, 170, 120, 80, 40, 20], 30,
        "No dominant seller; the shelf resets every summer as listings go stale off-season.",
        5, "5 new entrants in 90 days — seasonal rush already starting."),
      evidence: [
    ...
    {
      ...candidate("cand-mini-blender", "Portable mini blender bottle", "kitchen", "aliexpress", "", "dropped", "amazon_us", "D", 15,
        17.99, 6.2, 2.5, 15, 3.8,
      ...
      competition: competition([18000, 9200, 4400, 2600, 1800, 1200, 800, 500, 280, 140], 41,
        "Saturated head plus a safety-recall shadow over the category since spring.",
        1, "Entrant velocity collapsed after the recall coverage."),
  ...
  return [
    {
      ...
      reason: "TikTok demand is compounding while the Amazon shelf is still shallow (head: 214 reviews, 11% share). Margin card clears the floor at 38.2% with breakeven ACOS 54.5%. Window: 6-8 weeks before the shelf fills.",
    ...
    },
    {
      ...
      reason: "Second demand wave with a desk-setup angle; TikTok Shop lane nearly empty. 42.1% margin leaves room for 15% creator commission.",
    ...
    },
    {
      ...
      reason: "Best margin structure on the board (45.6%) and speed-sensitive: no TikTok Shop listing above 300 sales yet.",
    ...
    },
    {
      ...
      reason: "Saturated head (41k reviews), $1.40+ CPC, price collapse to $9.99. 18% margin is below the 25% floor before returns. Views here do not convert to a wedge for a new entrant.",
      brief: "Drop and archive. Re-open only if a genuinely new form factor (e.g. peel-and-stick corner channels) appears with sub-$0.80 CPC.",
    ...
    },
    {
      ...
      proposed_at: "2026-07-02T08:50:00.000Z",
      reason: "Dash holds 73% review share and the median price fell $3 in two weeks. At 18.1% margin this is below floor — recommend dropping before sample spend.",
      brief: "Drop. Price umbrella belongs to Dash; entering means funding a discount war with no review base.",
      verdictNote: "Kelly (2026-07-02): re-run the margin card with the consolidated sea-freight quote first — if it stays under 22%, I'll approve the drop."
    },
    {
      ...
      reason: "Category proven by HausWerk's launch velocity, but our COGS quote puts margin at 20%. Blocked until a supplier lands under $6.50 or the DE angle is validated.",
...
function localizeSnapshotZh(snapshot) {
  const candidateText = {
    "cand-lunchbox": ["可折叠硅胶饭盒", "TikTok 周播放 210 万且评论区大量『求链接』，而亚马逊货架还很浅：头部卖家仅 214 条评论、11% 份额。带蒸汽阀的折叠款毛利 38%、保本 ACOS 约 55%——TikTok 起量的产品里少见的利润空间。"],
    "cand-spice-rack": ["磁吸冰箱调料架", "BSR 排名 9 天跳升 88%，且前五名里两家断货——这是供给缺口而不只是需求。强磁款配防滑垫可以站稳 $24.99、毛利 31%。"],
    "cand-cloud-humidifier": ["云朵小夜灯加湿器", "第二波需求换了『桌面搭建』角度，说明趋势熬过了冬季峰值。$27.99 定价下毛利 42%，TikTok Shop 费率还留得出达人佣金。"],
    "cand-pet-hair-roller": ["可重复用宠物除毛滚筒", "全场最好的利润结构：毛利 45%、保本 ACOS 63%、COGS 仅 $2.10。每条爆款视频都在带购物车，且 TikTok Shop 还没有超过 300 单的链接。"],
    "cand-milk-frother": ["充电式奶泡器", "搜索热度常年高出基线 35%，但成败在运费：海运报价低于 $2.20/件 才能过毛利线。"],
    "cand-led-strip": ["RGB LED 灯带 32 英尺", "有流量没切口：头部链接 4.1 万条评论，CPC 抬到 $1.40+，中位价跌到 $9.99。预估毛利 18%，未计退货就已低于 25% 红线。"],
    "cand-sink-caddy": ["伸缩水槽置物架", "Temu 冲到第一页通常领先亚马逊需求曲线 6-10 周。$16.99 下毛利 32%，已有两家验证过的供应商。"],
    "cand-veggie-chopper": ["14 合 1 切菜器", "复购单量（而非首单）上升 45%，说明是动销不是压货。毛利 27% 偏薄——避开刀片数量战、主打可进洗碗机才有戏。"],
    "cand-oil-dispenser": ["玻璃喷油壶", "跟着空气炸锅生态稳涨 90 天的词，不是脉冲热点。$13.99 价位毛利 35%，买家评论里点名要玻璃材质加刻度。"],
    "cand-drawer-organizer": ["竹制抽屉收纳套装", "HausWerk 的上新节奏证明品类成立，但我们 $8.40 的 COGS 报价只有 20% 毛利，低于红线。等低于 $6.50 的供应商，或验证德国站差异化再说。"],
    "cand-egg-cooker": ["迷你煮蛋器", "排名在涨但中位价两周跌了 $3——以 18% 毛利入场等于给价格战送钱。提议放弃，除非海运报价改变测算。"],
    "cand-ice-tray": ["带盖可叠冰格", "季节性但每年重复：『透明冰』内容角度每个夏天都会再火一次。TikTok Shop 8% 费率下 $11.99 仍有 34% 毛利。"],
    "cand-mini-blender": ["便携迷你榨汁杯", "电池 + 液体 + 刀片意味着高退货率（品类均值 11%），广告成本始终压不下来。计退货后实际毛利 15%——上月评审已放弃。"]
  };
  const dominanceText = {
    "cand-lunchbox": ["货架分散：没有超过 250 条评论的链接，前三名只有颜色差别。评论护城河很浅，靠更好的场景图就能超越。", "90 天新进 4 家卖家，都在 50 条评论以下——尚早但还不算拥挤。"],
    "cand-spice-rack": ["头部卖家本周断货；第二名涨价 $3。窗口期在两家补货前。", "90 天新进 3 家，速度中等。"],
    "cand-cloud-humidifier": ["亚马逊货架中等深度，但 TikTok Shop 几乎是空的：只有两条链接且视频素材很弱。", "90 天新进 5 家，基本只做亚马逊——TikTok Shop 赛道是开着的。"],
    "cand-pet-hair-roller": ["一个老品牌（ChomChom）以 $24.95 占了 62% 评论量——但所有爆款视频卖的都是 $12-15 的通用款式。", "90 天新进 6 家且还在加速；这里拼速度胜过拼打磨。"],
    "cand-milk-frother": ["两家头部超过 2.8k 评论；差异化只能靠充电底座和礼盒包装。", "90 天仅 2 家新进——货架是成熟，不是火热。"],
    "cand-led-strip": ["头部完全饱和：五条链接超过 7.5k 评论，每个关键词都是品牌广告墙。", "新卖家已经停止进场——市场早已完成整合。"],
    "cand-sink-caddy": ["一个 1.9k 评论的头部之后是长尾。防锈钢材版本可以吃下中部货架。", "90 天新进 3 家，全在模仿 Temu 的设计。"],
    "cand-veggie-chopper": ["两家大卖坐镇，但他们的一星差评集中在容器开裂——材质升级就是切口。", "90 天新进 4 家；速度中等。"],
    "cand-oil-dispenser": ["头部占据『塑料』分段；玻璃细分的头部只有 700 条评论。", "90 天新进 3 家；玻璃款供给不足。"],
    "cand-drawer-organizer": ["按 HausWerk 的上新历史，两个季度内头部就是他们的。德国站比美国站慢一个周期。", "90 天美国站新进 6 家；德国站货架仍然很薄。"],
  ...
  const trendText = {
    "tr-lunchbox-tiktok": ["可折叠硅胶饭盒 — 达人演示周播放 210 万", "本周三位达人发布折叠饭盒演示；头部视频日增约 40%，评论区反复出现『求链接』。"],
    "tr-spice-rack-bsr": ["磁吸调料架 BSR 从 5,214 跳到 612（家居厨房）", "9 天排名跳升 88%，头部仅 214 条评论。前五名中两条链接本周断货。"],
    "tr-humidifier-tiktok": ["『云朵』小夜灯加湿器重回 FYP", "同一造型的第二波：#roomdecor 下周播放 68 万。冬季角度消失，达人改推『桌面搭建』场景。"],
    "tr-roller-tiktok": ["宠物除毛滚筒：周播放 140 万，遍地购物车链接", "清洁技巧账号在重剪同一个演示。7 天内出现 4 条 TikTok Shop 链接，均未超过 300 单。"],
    "tr-sink-caddy-temu": ["伸缩水槽置物架爬升 Temu 家居热销榜", "两周内从收纳类第 4 页升到第 1 页，售价 $6.48。评论图显示两家供应商品质稳定。"],
    "tr-chopper-ali": ["14 合 1 切菜器速卖通复购量上升", "前三供应商 30 天合计 3.2k 单，增长约 45%。美规插头版本 CN 仓发货，$4.90-5.60。"],
    "tr-oil-sprayer-gt": ["『oil sprayer for cooking』搜索热度 90 天 +80%", "稳步上涨而非脉冲：空气炸锅关联持续拉动。相关词『olive oil sprayer glass』同样在涨。"],
    "tr-frother-gt": ["『rechargeable milk frother』春季高峰后维持高位", "热度稳定在去年基线上方约 35%。家庭咖啡内容让这个词在送礼季之外也保持温度。"],
    "tr-organizer-comp": ["HausWerk（追踪店铺）上线竹制抽屉收纳产品线", "一周上了 6 个 SKU，带 A+ 页面和新品券。他们最近三次上新都在一个季度内做到 500+ 评论。"],
    "tr-egg-cooker-bsr": ["迷你煮蛋器重回小家电前 100", "两周排名改善 41%，但价格战清晰可见：同期中位价从 $18.99 跌到 $15.99。"],
    "tr-ice-tray-temu": ["带盖可叠冰格在 Temu 厨房类走强", "夏季季节性叠加『透明冰』爆款角度。上周全平台约售 9k 件，$3.98-5.20。"],
    "tr-led-strip-tiktok": ["LED 灯带房间改造格式仍在刷播放", "播放量高但混剪格式靠投放驱动；头部链接 4 万+ 评论，$9.99 锚定价。"]
  };
  ...
  const proposalText = {
    "prop-lunchbox": {
      title: "开发：可折叠硅胶饭盒",
      reason: "TikTok 需求在复利式增长，而亚马逊货架仍然很浅（头部 214 条评论、11% 份额）。利润卡过线：毛利 38.2%、保本 ACOS 54.5%。窗口期约 6-8 周。",
    ...
    "prop-led-strip": {
      title: "放弃：RGB LED 灯带",
      reason: "头部饱和（4.1 万评论）、CPC $1.40+、价格崩到 $9.99。毛利 18% 未计退货已低于 25% 红线。这里的流量换不来新卖家的切口。",
    ...
    "prop-organizer": {
      title: "观察：竹制抽屉收纳套装",
      reason: "HausWerk 的上新速度证明品类成立，但我们的 COGS 报价只给到 20% 毛利。等供应商低于 $6.50 或验证德国站角度后再动。",
  ...
  snapshot.proposals = snapshot.proposals.map((item) => {
    ...
    if (!text) return item;
    return { ...item, title: text.title, reason: text.reason, brief: text.brief, verdictNote: text.verdictNote || item.verdictNote };
  });
  snapshot.sync_log = [
    { at: "2026-07-02T08:40:00.000Z", actor: "kelly-picks-agent", action: "ingest_trends", detail: "TikTok + BSR 扫描：写入 4 条趋势信号、新建 2 个候选品、跳过 1 条重复。" },
    { at: "2026-07-02T08:45:00.000Z", actor: "kelly-picks-agent", action: "compute_margins", detail: "按费率表重算 12 张利润卡；3 个候选品低于 25% 毛利红线。" },
    { at: "2026-07-01T19:20:00.000Z", actor: "kelly-picks-agent", action: "execute_decisions", detail: "Dry-run：2 个已批准提案 → 3 个计划操作（采购简报、listing 简报交接、观察项）。" }
  ];
  return snapshot;
}
...
function trendItem(trend_id, sourceKind, title, summary, url, metric_label, metric_value, delta_pct, momentum, observed_at, candidate_id, external_id) {
  return {
    trend_id,
    source: sourceKind,
    title,
...
function candidate(candidate_id, name, category, sourceKind, source_ref, stage, platform_id, grade, momentum_pct, price, cogs, freight, feePct, adCost, why_it_matters, first_seen) {
