import type { DecisionBody, DemoQuery, PicksSnapshot } from "./types.ts";
...
export function demoSnapshot(scenario = "overview"): PicksSnapshot {
...
function localizeSnapshotZh(snapshot: PicksSnapshot): PicksSnapshot {
  ...
  snapshot.candidates = snapshot.candidates.map((item) => {
    ...
    if (comp) {
      next.competition = { ...(next.competition as object), dominance_note: comp[0], velocity_note: comp[1] };
    }
    return next;
  });
  const trendText = {
