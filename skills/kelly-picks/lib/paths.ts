...
// lib/ sits at skills/kelly-picks/lib/, so the skill root is one level up. The
// handoff state files still live under app/.data (authoritative, not disposable)
// and regenerable runtime artifacts under app/.cache, so the on-disk layout — and
// therefore a byte-identical /api/state — is unchanged from the pre-lib server.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "picks_snapshot.json");
