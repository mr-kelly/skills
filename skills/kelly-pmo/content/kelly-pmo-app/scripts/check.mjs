...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly PMO must be Cloud-only");
if (/KELLY_PMO_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly PMO AirApp checks OK (${files.length} files)`);
