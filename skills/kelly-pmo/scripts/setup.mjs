...
import { syncNativeViews } from "./sync-native-views.mjs";
import { syncSupportNodes } from "./sync-support-nodes.mjs";

const appRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "content", "kelly-pmo-app");
...
if (dataReady && current.airApp) {
  const viewResult = await syncNativeViews(client, { apply });
  const nodeResult = await syncSupportNodes(client, { apply, baseUrl: process.env.BUSABASE_BASE_URL });
  const remaining = apply ? 0 : viewResult.changes + nodeResult.missing + (nodeResult.repairs ?? 0);
  console.log(
    remaining
      ? `\n数据层和 AirApp 已存在；还有 ${remaining} 个原生视图/配套节点变更，确认后加 --apply。`
      : "\n工作区已经就绪（数据层、原生视图、配套节点和 AirApp 均已存在）。",
  );
  process.exit(0);
}
...
if (!apply) {
  ...
  console.log(
    `将同步 ${appConfig.bases.reduce((total, base) => total + (base.views?.length ?? 0), 0)} 个原生视图（已存在且一致的会跳过）。`,
  );
  console.log(`将同步 ${appConfig.supportNodes.length} 个配套节点（已存在且类型正确的会跳过）。`);
...
await syncNativeViews(client, { apply: true });
await syncSupportNodes(client, { apply: true, baseUrl: process.env.BUSABASE_BASE_URL });
