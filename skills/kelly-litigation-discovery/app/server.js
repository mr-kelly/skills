...
const AIRAPP_ID = "kelly-litigation-discovery";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-litigation-discovery" }));
...
app.onError((error, context) => {
  console.error("E-Discovery Document Review Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33354", 10);
