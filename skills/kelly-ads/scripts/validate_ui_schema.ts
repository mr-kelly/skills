...
for (const key of [
  ...
  "anomalies_open",
  "anomalies_critical",
  "adjustments_needing_review",
  "budget_at_risk_today",
]) {
...
snapshot.platforms.forEach((platform, index) => {
  ...
  if (!PLATFORM_IDS.has(platform.platform_id))
    fail(`${path}.platform_id must be one of: ${[...PLATFORM_IDS].join(", ")}`);
...
snapshot.campaigns.forEach((campaign, index) => {
  ...
  if (!CAMPAIGN_STATUSES.has(campaign.status))
    fail(`${path}.status must be one of: ${[...CAMPAIGN_STATUSES].join(", ")}`);
  ...
  for (const key of ["spend", "clicks", "conversions", "revenue", "roas", "acos_pct"])
    requireNumber(campaign.totals_7d, key, `${path}.totals_7d`);
});
...
snapshot.adjustments.forEach((adjustment, index) => {
  ...
  if (!ADJUSTMENT_STATUSES.has(adjustment.status))
    fail(`${path}.status must be one of: ${[...ADJUSTMENT_STATUSES].join(", ")}`);
  if (!campaignIds.has(adjustment.campaign_id))
    fail(`${path}.campaign_id does not match a campaign: ${adjustment.campaign_id}`);
  ...
  if (adjustment.status === "done" && !isObject(adjustment.execution))
    fail(`${path}.execution must be an object when status is done`);
});
...
snapshot.anomalies.forEach((anomaly, index) => {
  ...
  for (const key of ["anomaly_id", "type", "severity", "state", "campaign_id", "platform", "evidence"])
  ...
  if (!campaignIds.has(anomaly.campaign_id))
    fail(`${path}.campaign_id does not match a campaign: ${anomaly.campaign_id}`);
  if (anomaly.adjustment_id && !adjustmentIds.has(anomaly.adjustment_id))
    fail(`${path}.adjustment_id does not match an adjustment: ${anomaly.adjustment_id}`);
