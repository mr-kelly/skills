...
async function submitDecision(adjustmentId, verdict) {
  ...
  try {
    ...
    await provider.decideAdjustment({ adjustment_id: adjustmentId, verdict, note });
  } catch (error) {
  ...
  els.content.innerHTML = `
    ...
    <div class="settings">
      <section>
        ...
        <dl>
          <dt>${t("provider")}</dt><dd>${escapeHtml(state.settings?.data_provider || "busabase")}</dd>
      ...
      <section>
        ...
        ${
          ...
          <div class="settings-row">
            ...
            <span class="mono muted">${escapeHtml(platform.account_id || "")}</span>
          </div>
        `,
            )
      ...
      <section>
        <h2>${t("targets")}</h2>
        <dl>
          <dt class="mono">default_acos_pct</dt><dd>${escapeHtml(String(targets.default_acos_pct ?? ""))}</dd>
          <dt class="mono">default_roas</dt><dd>${escapeHtml(String(targets.default_roas ?? ""))}</dd>
        </dl>
      </section>
      <section>
        <h2>${t("thresholds")}</h2>
