...
export async function readSnapshot(): Promise<AdsSnapshot> {
  return (await readJson<AdsSnapshot>(SNAPSHOT_PATH, emptySnapshot())) as AdsSnapshot;
}
...
export async function applyDecision({
  ...
  if (verdict === "request_changes") {
    const tasks = (await readJson<{ tasks: Record<string, any>[]; updated_at?: string }>(AGENT_TASKS_PATH, {
      tasks: [],
    })) || { tasks: [] };
