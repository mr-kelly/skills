...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Demo Video Factory must be Cloud-only");
if (!configText.includes("readOnly: true")) throw new Error("Kelly Demo Video Factory AirApp must be read-only");
if (/KELLY_VIDEO_FACTORY_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
const walk = async (directory) => {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if (entry.name === "node_modules" || entry.name === ".data" || entry.name === ".cache") continue;
...
console.log(`Kelly Demo Video Factory AirApp checks OK (${files.length} files)`);
