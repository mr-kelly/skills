...
const AIRAPP_ID = "kelly-aiops-rootcause";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-aiops-rootcause" }));
...
app.onError((error, context) => {
  console.error(
    "AIOps Incident Alert & Root Cause Analyzer server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3189", 10);
