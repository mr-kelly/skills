...
const port = Number.parseInt(process.env.KELLY_AGENT_BUILDER_UI_PORT || process.env.PORT || String(DEFAULT_PORT), 10);

await ensureDirs();

serve({ fetch: app.fetch, hostname: host, port }, (info) => {
  console.log(`Agent Builder & Governance Console UI: http://${host}:${info.port}`);
});
