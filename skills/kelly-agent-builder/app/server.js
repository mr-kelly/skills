...
const AIRAPP_ID = "kelly-agent-builder";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-agent-builder" }));
...
app.onError((error, context) => {
  console.error("Kelly Agent Builder server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3122", 10);
