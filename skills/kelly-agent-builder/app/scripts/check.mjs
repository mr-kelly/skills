...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Agent Builder must be Cloud-only");
if (/KELLY_AGENT_BUILDER_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Agent Builder AirApp checks OK (${files.length} files)`);
