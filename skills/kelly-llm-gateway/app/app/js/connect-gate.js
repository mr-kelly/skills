...
// local preview on loopback, before the LLM Gateway shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the LLM Gateway routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly LLM Gateway will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly LLM Gateway never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the LLM Gateway shell mounts. Returns true once the app is clear to load
