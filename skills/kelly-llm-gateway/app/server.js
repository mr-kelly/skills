...
const AIRAPP_ID = "kelly-llm-gateway";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-llm-gateway" }));
...
app.onError((error, context) => {
  console.error("LLM Gateway server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3164", 10);
