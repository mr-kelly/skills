...
const AIRAPP_ID = "kelly-contract-risk";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-contract-risk" }));
...
app.onError((error, context) => {
  console.error(
    "Contract Clause Risk & Indemnity Inspector server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3198", 10);
