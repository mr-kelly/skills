...
const AIRAPP_ID = "kelly-health-payer-claims";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-health-payer-claims" }));
...
app.onError((error, context) => {
  console.error(
    "Health Insurance Claims Adjudication Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33306", 10);
