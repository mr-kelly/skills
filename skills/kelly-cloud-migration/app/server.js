...
const AIRAPP_ID = "kelly-cloud-migration";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-cloud-migration" }));
...
app.onError((error, context) => {
  console.error("Multi-Cloud Workload Migration Analyzer server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3202", 10);
