...
const AIRAPP_ID = "kelly-customer-onboarding";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-customer-onboarding" }));
...
app.onError((error, context) => {
  console.error("B2B Customer Onboarding & TTV Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33362", 10);
