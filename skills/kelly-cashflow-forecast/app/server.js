...
const AIRAPP_ID = "kelly-cashflow-forecast";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-cashflow-forecast" }));
...
app.onError((error, context) => {
  console.error(
    "Enterprise Cash Flow Forecast & Liquidity Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3205", 10);
