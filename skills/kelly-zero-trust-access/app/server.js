...
const AIRAPP_ID = "kelly-zero-trust-access";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-zero-trust-access" }));
...
app.onError((error, context) => {
  console.error("Zero Trust Access & Device Trust Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33332", 10);
