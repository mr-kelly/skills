...
const AIRAPP_ID = "kelly-property-lease-admin";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-property-lease-admin" }));
...
app.onError((error, context) => {
  console.error("Commercial Property Lease & Rent Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33293", 10);
