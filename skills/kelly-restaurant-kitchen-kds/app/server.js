...
const AIRAPP_ID = "kelly-restaurant-kitchen-kds";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-restaurant-kitchen-kds" }));
...
app.onError((error, context) => {
  console.error(
    "Restaurant POS, KDS & Recipe Costing Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33292", 10);
