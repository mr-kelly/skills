...
const AIRAPP_ID = "kelly-iam-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-iam-audit" }));
...
app.onError((error, context) => {
  console.error("IAM Privilege Audit & Identity Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3182", 10);
