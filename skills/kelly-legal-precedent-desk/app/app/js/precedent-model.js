// Pure domain logic for kelly-legal-precedent-desk, ported verbatim (same
...
export function normalizeItemRow({
  ...
  return {
    ...
    fields: {
      query,
      jurisdiction,
      match_count: match_count === "" ? undefined : Number(match_count),
      high_match_count: high_match_count === "" ? undefined : Number(high_match_count),
      top_similarity: top_similarity === "" ? undefined : Number(top_similarity),
      avg_similarity: avg_similarity === "" ? undefined : Number(avg_similarity),
      court_pattern,
      citation_count: citation_count === "" ? undefined : Number(citation_count),
    },
    decision_action,
...
// (firm_profile/search_policy/export).
/**
  ...
  return {
    ...
    search_policy: {
      ...
      minimum_similarity_score:
        settings.minimum_similarity_score === "" ? undefined : Number(settings.minimum_similarity_score),
      require_source_case_ids: boolField(settings.require_source_case_ids, true),
      quote_limit_words: settings.quote_limit_words === "" ? undefined : Number(settings.quote_limit_words),
    },
    export: {
      format: settings.export_format || "markdown+json",
      out_dir: settings.export_out_dir || "exports/research-packs",
    },
  };
}
...
// checks_failed) — query_count/high_matches/local_patterns have no
// dedicated Busabase column on items (app.js's businessMetricsHtml already
// falls back to summing the item fields / counting all items when absent);
// `extra` lets a caller merge in display-only metrics the same way the
// retired demo/server payload did. ----
  ...
  for (const item of items) {
    if (item.created_at) {
      entries.push({
        ...
        actor: "agent",
        action: "prepare",
        detail: `Prepared precedent research pack "${item.title}" for review.`,
        count: 1,
      });
    }
    if (item.updated_at && item.updated_at !== item.created_at) {
      entries.push({
        ...
        actor: "agent",
        action: "update",
        detail: `Updated precedent research pack "${item.title}".`,
        count: 1,
      });
    }
...
export function itemExecution(item, action, { apply = false } = {}) {
  if (action === "approve") {
    const target = `exports/research-packs/${slugify(item.title || item.id)}.md`;
    return {
      operation: "export_research_pack",
      target,
      status: apply ? "ready_for_agent" : "planned",
      detail: `Run scripts/export_research_pack.mjs to write ${target}. Client advice, filings, and public citation remain a separate explicit approval, never this script.`,
    };
  }
  if (action === "request_changes") {
    return {
      ...
      status: apply ? "ready_for_agent" : "planned",
      detail:
        "Strengthen the precedent research pack per the review note, then re-import with scripts/create_research_batch.mjs.",
    };
  }
  return null;
}
