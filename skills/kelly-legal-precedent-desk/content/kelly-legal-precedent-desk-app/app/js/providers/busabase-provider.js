...
// structured `fields{}` (query/jurisdiction/court_pattern/...) directly —
// only draft/review-note/status change through a decision, so this is a
...
export const busabaseProvider = {
  ...
  // Human verdict on a precedent research pack, written directly onto the
  // item record. Ported from the retired local-file DataProvider's
  ...
  async decideItem({ id, action, comment = "", draft } = {}) {
    ...
    await runtimeClient.records.changeRequest({
      ...
      message: `Decision on precedent research pack ${id}: ${action}`,
