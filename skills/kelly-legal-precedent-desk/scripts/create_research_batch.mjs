#!/usr/bin/env node
// Trusted hand-off step. Legal Precedent Desk's AirApp never imports
// similar-case/citation/court-pattern data itself — the browser cannot read
// an arbitrary local file path. This script parses a JSON payload file (the
// same shape the retired scripts/create_research_batch.ts accepted per
// references/precedent-schema.md's "Payload Import" section: any of
...
//
// Usage: node scripts/create_research_batch.mjs payload.json [more.json ...] [--apply]
...
async function upsert(client, declared, existing, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "kelly-legal-precedent-desk-create-research-batch",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "kelly-legal-precedent-desk-create-research-batch",
    autoMerge: true,
  });
  return "created";
}
  ...
  for (const file of files) {
    ...
    for (const input of incomingItems) {
      ...
      const fields = {
        ...
        proposed_action: input.proposed_action ?? existing?.proposed_action ?? "approve_research_pack",
        draft: input.draft ?? existing?.draft ?? "",
        evidence: JSON.stringify(input.evidence ?? (existing ? JSON.parse(existing.evidence || "[]") : [])),
        query: input.fields?.query ?? existing?.query ?? "",
        jurisdiction: input.fields?.jurisdiction ?? existing?.jurisdiction ?? "",
        match_count: input.fields?.match_count ?? existing?.match_count ?? "",
        high_match_count: input.fields?.high_match_count ?? existing?.high_match_count ?? "",
        top_similarity: input.fields?.top_similarity ?? existing?.top_similarity ?? "",
        avg_similarity: input.fields?.avg_similarity ?? existing?.avg_similarity ?? "",
        court_pattern: input.fields?.court_pattern ?? existing?.court_pattern ?? "",
        citation_count: input.fields?.citation_count ?? existing?.citation_count ?? "",
