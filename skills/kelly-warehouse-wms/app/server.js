...
const AIRAPP_ID = "kelly-warehouse-wms";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-warehouse-wms" }));
...
app.onError((error, context) => {
  console.error(
    "Warehouse Putaway, Picking & Cycle Count Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33312", 10);
