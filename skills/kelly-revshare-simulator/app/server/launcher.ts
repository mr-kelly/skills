...
import { CACHE_DIR, DEFAULT_HOST, DEFAULT_PORT, LOG_PATH, PID_PATH, PREFERRED_PORT_MAX } from "../../lib/paths.ts";
import { SERVER_DIR } from "./paths.ts";

const host = process.env.KELLY_REVSHARE_SIMULATOR_UI_HOST || DEFAULT_HOST;
const explicitPort = process.env.KELLY_REVSHARE_SIMULATOR_UI_PORT || "";
    ...
    const req = http.get(stateUrlFor(port), { timeout: 500 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode >= 200 && res.statusCode < 300 && state.app === "kelly-revshare-simulator");
        } catch {
          resolve(false);
        }
      });
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`Revenue-Share Contract Simulator UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_REVSHARE_SIMULATOR_UI_HOST: host, KELLY_REVSHARE_SIMULATOR_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Revenue-Share Contract Simulator UI started: http://${host}:${port}`);
  ...
  console.error(`Revenue-Share Contract Simulator UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
