...
// local preview on loopback, before the Kelly Revenue-Share Simulator shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the Kelly Revenue-Share Simulator routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Revenue-Share Simulator will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Revenue-Share Simulator never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Kelly Revenue-Share Simulator shell mounts. Returns true once the app is clear to load
