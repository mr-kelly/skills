...
import { type ScalePptxProvider, assertProvider } from "./provider-interface.ts";

function configCandidates(): string[] {
  return [
    process.env.KELLY_SCALE_PPTX_CONFIG,
    path.join(SKILL_DIR, "config.local.json"),
    path.join(os.homedir(), ".config", "kelly-scale-pptx", "config.json"),
...
export function resolveProviderKind(config: Config = {}): string {
  return String(process.env.KELLY_SCALE_PPTX_DATA_PROVIDER || config.data_provider || "local").toLowerCase();
}

export async function createProvider(): Promise<ScalePptxProvider> {
  ...
  throw new Error(`Unknown KELLY_SCALE_PPTX_DATA_PROVIDER: "${kind}" (expected "local")`);
}
