...
const explicitPort = process.env.KELLY_SCALE_PPTX_UI_PORT || "";
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`Kelly Scale PPTX UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_SCALE_PPTX_UI_HOST: host, KELLY_SCALE_PPTX_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly Scale PPTX UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly Scale PPTX UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
