...
const AIRAPP_ID = "kelly-hotel-revenue-mgmt";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-hotel-revenue-mgmt" }));
...
app.onError((error, context) => {
  console.error(
    "Hotel Room Yield & RevPAR Management Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33294", 10);
