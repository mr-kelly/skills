...
const AIRAPP_ID = "kelly-behavior-predict";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-behavior-predict" }));
...
app.onError((error, context) => {
  console.error("Kelly Behavior Predict server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3153", 10);
