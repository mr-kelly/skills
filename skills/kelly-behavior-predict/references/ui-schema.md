...
app code (`content/kelly-behavior-predict-app/app/js/providers/busabase-provider.js`,
`content/kelly-behavior-predict-app/app/js/behavior-model.js`). `predicted_action`, rule `triggers`, the
...
## Sessions (`kelly-behavior-predict-sessions`)
...
`evaluateRules()`/`predictNextAction()` in `content/kelly-behavior-predict-app/app/js/behavior-model.js`,
...
## Segments (`kelly-behavior-predict-segments`)
...
`SEGMENTS` constant in `content/kelly-behavior-predict-app/app/js/behavior-model.js`, not in Busabase). The
...
## Settings (`kelly-behavior-predict-settings`)
...
`evaluateRules(features)` in `content/kelly-behavior-predict-app/app/js/behavior-model.js` evaluates a
...
`generateAllSessions()`, now living in `content/kelly-behavior-predict-app/app/js/behavior-model.js`) into
...
`content/kelly-behavior-predict-app/app/js/behavior-model.js`) is deterministic: no `fs`, no network, no
...
— verified in `content/kelly-behavior-predict-app/test/behavior-model.test.mjs` by calling
