...
const AIRAPP_ID = "kelly-convo-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-convo-intel" }));
...
app.onError((error, context) => {
  console.error(
    "Conversation Intelligence & Sales Analytics server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3187", 10);
