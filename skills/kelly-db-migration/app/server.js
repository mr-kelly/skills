...
const AIRAPP_ID = "kelly-db-migration";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-db-migration" }));
...
app.onError((error, context) => {
  console.error("Zero-Downtime DB Schema Migration Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33375", 10);
