// Filesystem layout for the kelly-money data-provider layer.
//
// The lib lives at skills/kelly-money/lib/; the skill root is one level up and
// the Hono app (with its authoritative .data/ handoff files) sits under app/.
// These paths are byte-for-byte the same locations the previous
...
export const ledgerPath = path.join(dataDir, "ledger_snapshot.json");
export const syncReportPath = path.join(dataDir, "sync_report.json");
