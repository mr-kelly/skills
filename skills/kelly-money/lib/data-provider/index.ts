...
//
// kelly-money is a read-mostly ledger dashboard whose unit of state is one
// authoritative snapshot plus config/onboarding/lock context. This module lets
...
function configCandidates(): string[] {
  return [
    process.env.KELLY_MONEY_CONFIG,
    path.join(skillDir, "config.local.json"),
    path.join(process.env.HOME || "", ".config", "kelly-money", "config.json"),
