import type { Account, DemoQuery, Invoice, InvoiceMatch, LedgerSnapshot, Transaction } from "./types.ts";
...
function localizeSnapshotZh(snapshot: LedgerSnapshot): LedgerSnapshot {
  ...
  const txText: Record<string, [string, string]> = {
...
function demoSnapshot(scenario: string): LedgerSnapshot {
  ...
  for (const item of transactions) {
    const totals = (byAccount.get(item.account_id) as Account).totals;
...
function demoInvoiceMatches(): InvoiceMatch[] {
  return [
    match(
      "match-northstar",
      "inv-northstar-20260629",
