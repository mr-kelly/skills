...
export async function loadState() {
  ...
  state.settings = data;
  applyDemoRoute();
  window.dispatchEvent(new CustomEvent("kelly-devops:state", { detail: data }));
  render();
}
...
function renderShell() {
  ...
  els.syncStatus.textContent =
    state.snapshot?.generated_at && Date.parse(state.snapshot.generated_at) > 0
      ? `${recordCountLabel("services", services().length)} ${t("services").toLowerCase()}`
      : t("empty");
...
const PAGE_BINDINGS = {
  "services": { key: "services", path: "snapshot.services" },
  "expiries": { key: "expiries", path: "snapshot.expiries" },
  "actions": { key: "actions", path: "snapshot.actions" },
};
