...
export function demoStatePayload(query = {}) {
  ...
  return {
    ...
    config_summary: {
      ...
      domains: snapshot.expiries.filter((item) => item.type === "domain").map((item) => ({
        domain: item.item,
      ...
      key_rotation: [
        { key_id: "relayapi-sendgrid", name: "RelayAPI SendGrid key", env: "RELAYAPI_SENDGRID_KEY", rotate_every_days: 90, env_ready: true }
      ],
      billing_sources: [
        { provider_id: "aws", name: "AWS", secret_envs: ["KELLY_DEVOPS_AWS_BILLING_KEY"], secrets_ready: true },
        { provider_id: "gcp", name: "Google Cloud", secret_envs: ["KELLY_DEVOPS_GCP_BILLING_KEY"], secrets_ready: true },
        { provider_id: "cloudflare", name: "Cloudflare", secret_envs: ["KELLY_DEVOPS_CLOUDFLARE_TOKEN"], secrets_ready: true }
      ]
    },
    snapshot
  };
...
function demoServices() {
  return [
    service("formkit-web", "FormKit Web App", "FormKit", "https://formkit.io", "up", 182, 99.98, cert("Let's Encrypt R11", "2026-09-03", 63), 200, [168, 175, 181, 172, 190, 178, 185, 174, 169, 188, 176, 182]),
    service("formkit-api", "FormKit API", "FormKit", "https://api.formkit.io/health", "up", 121, 100, cert("Let's Encrypt R11", "2026-09-03", 63), 200, [118, 112, 126, 120, 115, 129, 122, 117, 124, 119, 116, 121]),
    service("formkit-dash", "FormKit Dashboard", "FormKit", "https://dash.formkit.io", "up", 243, 99.95, cert("Let's Encrypt R11", "2026-09-03", 63), 200, [231, 246, 238, 252, 229, 241, 236, 249, 233, 244, 239, 243]),
    service("docsly-web", "Docsly Web", "Docsly", "https://docsly.app", "up", 205, 99.9, cert("Google Trust Services WE1", "2026-08-12", 41), 200, [196, 210, 201, 214, 198, 207, 203, 216, 199, 209, 202, 205]),
    service("docsly-cdn", "Docsly CDN", "Docsly", "https://cdn.docsly.app/ping", "degraded", 2280, 99.42, cert("Google Trust Services WE1", "2026-08-12", 41), 200, [242, 251, 246, 380, 940, 1480, 1820, 2110, 2350, 2190, 2410, 2280]),
    service("relayapi-web", "RelayAPI Site", "RelayAPI", "https://relayapi.dev", "up", 130, 100, cert("Let's Encrypt R10", "2026-09-14", 74), 200, [124, 133, 127, 138, 125, 131, 128, 136, 126, 132, 129, 130]),
    service("relayapi-api", "RelayAPI Core API", "RelayAPI", "https://api.relayapi.dev/v1/health", "up", 96, 99.99, cert("Let's Encrypt R10", "2026-09-14", 74), 200, [92, 98, 94, 101, 91, 97, 93, 100, 95, 99, 94, 96]),
    service("relayapi-hooks", "RelayAPI Webhooks", "RelayAPI", "https://hooks.relayapi.dev/health", "down", 0, 97.21, cert("Let's Encrypt R10", "2026-09-14", 74), 503, [108, 112, 105, 116, 109, 114, 107, 111, 0, 0, 0, 0]),
    service("relayapi-status", "RelayAPI Status Page", "RelayAPI", "https://status.relayapi.dev", "up", 111, 99.97, cert("Let's Encrypt R10", "2026-07-22", 20), 200, [104, 113, 107, 118, 105, 112, 108, 116, 106, 114, 109, 111])
  ];
}
...
function demoExpiries() {
  return [
    expiry("domain-formkit-io", "domain", "formkit.io", "FormKit", "2026-07-11", 9, false, "act-renew-formkit", "rdap", "Namecheap", "Primary product domain. Renew at Namecheap; auto-renew is off because the card on file expired."),
    expiry("cert-relayapi-status", "ssl_cert", "status.relayapi.dev", "RelayAPI", "2026-07-22", 20, false, "", "tls", "", "Let's Encrypt renewal has not fired. Check the certbot timer on the status-page host."),
    expiry("plan-vercel-pro", "plan_renewal", "Vercel Pro (Docsly)", "Docsly", "2026-07-28", 26, true, "", "config", "", "Annual plan renews automatically on the team card. Confirm the card is current."),
    expiry("key-relayapi-sendgrid", "api_key_rotation", "RELAYAPI_SENDGRID_KEY", "RelayAPI", "2026-05-21", -42, false, "act-rotate-sendgrid", "config", "", "Rotation policy is 90 days; the key is 132 days old. Rotate in SendGrid, then update the env var."),
    expiry("domain-docsly-io", "domain", "docsly.io", "Docsly", "2026-08-30", 59, false, "act-renew-docsly", "rdap", "GoDaddy", "Redirect domain. Registrar account access is pending a 2FA reset."),
    expiry("domain-formkit-dev", "domain", "formkit.dev", "FormKit", "2026-10-02", 92, true, "", "rdap", "Namecheap", "Docs subdomain host. Auto-renew is on."),
    expiry("domain-relayapi-dev", "domain", "relayapi.dev", "RelayAPI", "2026-11-08", 129, true, "", "rdap", "Cloudflare Registrar", "Auto-renew is on at Cloudflare Registrar."),
    expiry("domain-docsly-app", "domain", "docsly.app", "Docsly", "2027-01-15", 197, true, "", "rdap", "Cloudflare Registrar", "Auto-renew is on at Cloudflare Registrar."),
    expiry("domain-kellyhq-com", "domain", "kellyhq.com", "Portfolio", "2027-03-21", 262, true, "", "rdap", "Namecheap", "Personal portfolio domain. Auto-renew is on.")
  ];
}

function expiry(expiry_id, type, item, product, expires_on, days_left, auto_renew, action_id, source, registrar, detail) {
...
function demoSpend() {
  return {
    ...
    providers: [
      ...
      spendRow("gcp", "Google Cloud", 812.4, 501.42, 62, true, "act-investigate-gcp", "Cloud Run egress and BigQuery scans jumped after the June 24 deploy."),
...
function demoActions() {
  return [
    action("act-renew-formkit", 1, "renew_domain", "Renew formkit.io before July 11", "needs_review",
      ...
      "", null),
    action("act-rotate-sendgrid", 2, "rotate_key", "Rotate the RelayAPI SendGrid API key", "needs_review",
      ...
      "", null),
    action("act-investigate-gcp", 3, "investigate_spend", "Investigate the GCP spend spike (+62% MTD)", "changes_requested",
      ...
      { verdict: "request_changes", note: "Break it down per service and per day before I decide anything.", decided_at: "2026-07-01T21:12:00.000Z" }),
    action("act-renew-docsly", 4, "renew_domain", "Renew docsly.io redirect domain", "blocked",
      ...
      { verdict: "block", note: "Waiting on GoDaddy support ticket #48211 for the 2FA reset.", decided_at: "2026-06-30T10:05:00.000Z" }),
    action("act-restart-hooks", 5, "restart_service", "Restart the RelayAPI webhook worker", "approved",
      ...
      { verdict: "approve", note: "Approved. Restart it and add the memory alert.", decided_at: "2026-07-02T08:40:00.000Z" }),
    action("act-ack-cdn", 6, "ack_incident", "Acknowledge the Docsly CDN latency incident", "done",
      ...
      { verdict: "approve", note: "Acknowledged; upstream issue, nothing to do on our side.", decided_at: "2026-07-01T09:20:00.000Z" })
  ];
}
...
function demoEvents() {
  return [
    event("evt-hooks-down", "2026-07-02T08:00:00.000Z", "error", "incident", "RelayAPI Webhooks started returning HTTP 503.", "relayapi-hooks"),
    event("evt-service-check", "2026-07-02T09:30:00.000Z", "info", "check", "Service check completed: 7 up, 1 degraded, 1 down across 9 endpoints.", ""),
    event("evt-domain-check", "2026-07-02T07:30:00.000Z", "warning", "expiry", "Domain check: formkit.io expires in 9 days and auto-renew is off.", ""),
    event("evt-cert-status", "2026-07-02T07:30:00.000Z", "warning", "expiry", "TLS certificate for status.relayapi.dev expires in 20 days.", "relayapi-status"),
    event("evt-spend-anomaly", "2026-07-02T03:15:00.000Z", "warning", "spend", "GCP month-to-date spend is 62% above last month; anomaly flagged.", ""),
    event("evt-cdn-latency", "2026-06-30T22:04:00.000Z", "warning", "incident", "Docsly CDN latency degraded above 2s during an upstream edge incident.", "docsly-cdn"),
    event("evt-cdn-ack", "2026-07-01T09:20:00.000Z", "info", "action", "Action #6 acknowledged: Docsly CDN latency incident (upstream cause).", "docsly-cdn")
  ];
}
...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.services = snapshot.services.map((service) => ({
    ...
    meta: { ...service.meta, note: service.service_id === "relayapi-hooks" ? "最近一次成功检查在 2 小时前。" : service.meta.note }
  }));
  const expiryDetails = {
