// Backend-agnostic helpers shared by the data-provider layer and the scripts.
//
// These do not talk to any storage backend — they are pure JSON IO, config
// loading, dotenv parsing, and snapshot math (metrics/events). Providers layer
// their backend-specific reads/writes on top; the scripts call the math helpers
// directly while persisting through the provider.

import fs from "node:fs/promises";
import path from "node:path";
import { SKILL_DIR } from "./paths.ts";
import type { Config, ConfigResult, ConfigSummary, DevopsSnapshot, OpsEvent, Thresholds } from "./types.ts";
