...
const AIRAPP_ID = "kelly-api-contract-testing";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-api-contract-testing" }));
...
app.onError((error, context) => {
  console.error(
    "OpenAPI Consumer Contract & Breaking Change Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33379", 10);
