...
const AIRAPP_ID = "kelly-treasury-hedging";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-treasury-hedging" }));
...
app.onError((error, context) => {
  console.error(
    "Corporate FX & Interest Rate Hedging Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33324", 10);
