...
const AIRAPP_ID = "kelly-global-payroll";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-global-payroll" }));
...
app.onError((error, context) => {
  console.error("Multi-Country Global Payroll & Tax Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33341", 10);
