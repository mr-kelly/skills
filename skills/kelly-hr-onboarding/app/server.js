...
const AIRAPP_ID = "kelly-hr-onboarding";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-hr-onboarding" }));
...
app.onError((error, context) => {
  console.error(
    "HR Employee Onboarding & Provisioning Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3191", 10);
