...
const AIRAPP_ID = "kelly-sub-processor-dpa";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-sub-processor-dpa" }));
...
app.onError((error, context) => {
  console.error("DPA & Vendor Sub-Processor Audit Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33356", 10);
