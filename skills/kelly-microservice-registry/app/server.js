...
const AIRAPP_ID = "kelly-microservice-registry";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-microservice-registry" }));
...
app.onError((error, context) => {
  console.error(
    "Microservice Service Catalog & Graph Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33377", 10);
