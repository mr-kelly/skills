...
const AIRAPP_ID = "kelly-llm-fine-tune";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-llm-fine-tune" }));
...
app.onError((error, context) => {
  console.error("LLM Fine-Tuning & Model Evaluation Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33372", 10);
