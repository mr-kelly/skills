...
const AIRAPP_ID = "kelly-secops-soar";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-secops-soar" }));
...
app.onError((error, context) => {
  console.error(
    "SOAR Security Playbook Orchestration Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33331", 10);
