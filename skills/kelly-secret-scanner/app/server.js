...
const AIRAPP_ID = "kelly-secret-scanner";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-secret-scanner" }));
...
app.onError((error, context) => {
  console.error("Secret Scanner & Leakage Audit Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3180", 10);
