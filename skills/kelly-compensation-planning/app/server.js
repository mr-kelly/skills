...
const AIRAPP_ID = "kelly-compensation-planning";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-compensation-planning" }));
...
app.onError((error, context) => {
  console.error(
    "Merit Increase & Equity Bonus Planning Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33348", 10);
