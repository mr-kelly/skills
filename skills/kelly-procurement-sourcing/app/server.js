...
const AIRAPP_ID = "kelly-procurement-sourcing";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-procurement-sourcing" }));
...
app.onError((error, context) => {
  console.error(
    "Strategic Procurement & PO Approval Console server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3183", 10);
