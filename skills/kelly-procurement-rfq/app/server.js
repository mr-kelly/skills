...
const AIRAPP_ID = "kelly-procurement-rfq";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-procurement-rfq" }));
...
app.onError((error, context) => {
  console.error(
    "Strategic Sourcing RFQ Bidding & Award Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33319", 10);
