...
import { computeMetrics } from "./js/newsroom-model.js?v=0.1.0";
...
const state = {
  ...
  query: "",
  lang: normalizeLang(
    new URLSearchParams(location.search).get("lang") || localStorage.getItem("kelly-ai-newsroom-language") || "auto",
  ),
...
const SIDEBAR_COLLAPSED_STORAGE_KEY = "kelly-ai-newsroom.sidebarCollapsed";
...
async function loadState() {
  ...
  window.dispatchEvent(new CustomEvent("kelly-ai-newsroom:state", { detail: data }));
  applyDemoRoute();
  render();
}
...
els.language.addEventListener("change", () => {
  ...
  localStorage.setItem("kelly-ai-newsroom-language", state.lang);
  render();
});
