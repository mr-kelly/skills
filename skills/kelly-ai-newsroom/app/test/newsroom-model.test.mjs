...
test("baseSignalFields/normalizeSignal round trip preserves every field", () => {
  const signal = {
    ...
    kind: "signal",
    ref: 1,
    title: "Publisher licensing moves change AI citation sources",
    summary: "summary text",
    why_it_matters: "why text",
    buyer_intent: "High: creates a concrete sales trigger.",
    confidence: 0.82,
...
test("baseActionFields/normalizeAction round trip preserves linked_signal_ids array", () => {
  const fields = baseActionFields({
    action_id: "action-1",
    ref: 1,
    title: "Package one buyer-trigger brief",
    summary: "summary",
    status: "needs_review",
    priority: "high",
    owner: "operator",
...
test("baseDraftFields/normalizeDraft: effective_body prefers edited_body over body", () => {
  const fields = baseDraftFields({
    draft_id: "draft-1",
    ref: 1,
    channel: "sales opener",
    title: "sales opener: today's approved angle",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-ai-newsroom-20260806",
    ...
    offer: "daily buyer-trigger intelligence",
  };
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    drafts: [{ draft_id: "draft-1", ref: 1, channel: "sales opener", status: "blocked" }],
    sources: [{ source_id: "news", label: "News" }],
    meta: { batch_id: "b1", buyer: "founders" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "sales opener", title: "sales opener: today's angle", next_step: undefined };
