...
// local preview on loopback, before the Kelly AI Newsroom shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the Kelly AI Newsroom routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly AI Newsroom will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly AI Newsroom never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Kelly AI Newsroom shell mounts. Returns true once the app is clear to load
