...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly AI Newsroom must be Cloud-only");
if (/KELLY_AI_NEWSROOM_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly AI Newsroom AirApp checks OK (${files.length} files)`);
