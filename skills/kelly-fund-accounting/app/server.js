...
const AIRAPP_ID = "kelly-fund-accounting";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-fund-accounting" }));
...
app.onError((error, context) => {
  console.error("Fund Accounting NAV & Audit Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33329", 10);
