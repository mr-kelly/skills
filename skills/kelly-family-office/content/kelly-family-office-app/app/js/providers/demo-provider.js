...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.entities = snapshot.entities.map((item) => ({
    ...item,
    name: entityNames[item.entity_id] || item.name,
    member: entityMembers[item.entity_id] || item.member,
  }));
  const nameById = new Map(snapshot.entities.map((item) => [item.entity_id, item.name]));
  ...
  snapshot.accounts = snapshot.accounts.map((item) => ({
    ...item,
    display_name: accountNames[item.account_id] || item.display_name,
  }));
  ...
  snapshot.holdings = snapshot.holdings.map((item) => ({
    ...item,
    name: holdingNames[item.name] || item.name,
  }));
...
export const demoProvider = {
  ...
  async getState() {
    ...
    return {
      ...
      demo_visuals: demoVisualsForApp("kelly-family-office"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-family-office") },
    };
  },
