  ...
  return `
    <form class="image-config" data-image-config>
      ...
      <div class="field">
        ...
        <input id="imageModel" name="model" value="${escapeHtml(config.image_model || "gpt-image-2")}" />
      </div>
      <div class="field">
        <label for="imageSize">${t("image_config_size")}</label>
        <select id="imageSize" name="size">
          ${["1024x1024", "1536x1024", "1024x1536"].map((size) => `<option value="${size}" ${size === (config.image_size || "1024x1024") ? "selected" : ""}>${size}</option>`).join("")}
        </select>
      </div>
...
function bindImageConfigForm() {
  ...
  imageConfigForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      await provider.saveImageConfig({
