...
// local preview on loopback, before the app shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the app routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Drama will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Drama never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the app shell mounts. Returns true once the app is clear to load
