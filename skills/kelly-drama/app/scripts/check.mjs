...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Drama must be Cloud-only");
if (/KELLY_DRAMA_DATA_PROVIDER|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Drama AirApp checks OK (${files.length} files)`);
