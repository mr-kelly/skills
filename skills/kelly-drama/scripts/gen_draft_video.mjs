...
// restructuring Kelly MV's gen_draft_video.mjs did (this skill's closest
...
export async function generateDraftVideo({
  ...
  if (!fs.existsSync(PY)) {
    throw new Error(
      `Local LTX-Video is not installed at ${LTX_DIR} — see references/drama-workflow.md for setup. Draft video generation is a genuinely local capability, not a stub, but needs the model checkout present.`,
    );
  }
