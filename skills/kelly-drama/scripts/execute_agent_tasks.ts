...
import { generateCharacterVoice } from "../lib/generation/voice-service.ts";
...
for (const pending of approved) {
  ...
  try {
    if (action === "generate_storyboard_image") await generateStoryboardImage(target);
      ...
      if (task.backend === "ltx") await generateShotVideoDraft(target);
      else await generateShotVideoProd(target);
    ...
    else if (action === "generate_character_voice") await generateCharacterVoice(target);
