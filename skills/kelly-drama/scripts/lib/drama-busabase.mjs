// Trusted-process Busabase helper shared by every kelly-drama skill-root
...
export async function connect() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error(
      "Kelly Drama Busabase resources are not provisioned yet; open the AirApp once (or run its setup) first.",
    );
  }
...
export async function upsert(client, declaredBase, idFieldSlug, idValue, fields, message) {
  ...
  if (!existing) {
    return client.bases.createChangeRequest({
      ...
      fields: normalized,
      message,
      submittedBy: "kelly-drama-script",
      autoMerge: true,
    });
  }
