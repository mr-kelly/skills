...
import { DATA_DIR, GENERATED_DIR, SKILL_DIR } from "../../app/server/paths.ts";
...
export async function setCharacterVoiceActive(characterId, assetPath) {
  ...
  await saveProject(project);
  return import("../../app/server/state.ts").then((m) => m.statePayload());
}

function runWrapper(pyAbs, wrapperAbs, args): Promise<string> {
