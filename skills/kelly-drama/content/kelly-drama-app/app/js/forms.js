  ...
  return `
    <form class="detail-card" data-kind="series">
      ...
      <div class="form-grid">
        ...
        ${input("platform", t("field_platform"), series.platform)}
        ${input("format", t("field_episode_format"), series.format)}
        ${input("tone", t("field_tone"), series.tone)}
        ${input("audience", t("field_audience"), series.audience)}
        ${input("hyperframe_project_path", t("field_hyperframe_project"), series.hyperframe_project_path || "")}
  ...
  return `
    <section class="character-reference">
      <div>
        ...
        <p class="muted">${escapeHtml(localizeValue(reference.purpose) || t("char_ref_hint"))}</p>
...
function characterVoicePreview(item) {
  ...
  const summary = [vp.type, vp.pace, vp.accent, vp.signature].filter(Boolean).map(localizeValue).join(" · ");
  ...
  return `
    <form class="detail-card" data-kind="characters" data-id="${escapeHtml(item.id)}">
      ...
      <div class="form-grid">
        ...
        ${input("role", t("field_dramatic_role"), localizeValue(item.role))}
...
function episodeForm(item) {
  return `
    <form class="detail-card" data-kind="episodes" data-id="${escapeHtml(item.id)}">
      <h2>${t("episode_short")} ${escapeHtml(item.number || "")} — ${escapeHtml(item.title || "")}</h2>
      <div class="form-grid">
        ...
        ${input("hyperframe_video_asset", t("field_hyperframe_video"), item.hyperframe_video_asset || "")}
  ...
  return `
    <form class="detail-card" data-kind="shots" data-id="${escapeHtml(item.id)}">
      ...
      <div class="form-grid">
        ...
        ${textarea("camera", t("field_camera"), item.camera, false)}
  ...
  return `
    <form class="detail-card" data-kind="tasks" data-id="${escapeHtml(item.id)}">
      ...
      <div class="form-grid">
        ...
        ${input("kind", t("field_kind"), localizeValue(item.kind))}
