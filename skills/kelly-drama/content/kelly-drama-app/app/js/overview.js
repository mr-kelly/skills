  ...
  return `
    <section class="visual-bible-card">
      <div>
        ...
        <p>${escapeHtml(localizeValue(bible.format_note || ""))}</p>
        <p class="muted">${escapeHtml(localizeValue(bible.realism_target || ""))}</p>
      </div>
    ...
    <div class="hyperframe-list">
      ...
      ${
        ...
        <div class="hyperframe-row">
          ...
          <span>${escapeHtml(item.scenes?.length || 0)} ${t("hyperframe_scenes")}</span>
  ...
  return `
    <section class="hyperframe-card">
      <div class="section-head">
        ...
        ${series.hyperframe_status_updated_at ? `<span class="badge soft">${t("hyperframe_last_read")} ${escapeHtml(series.hyperframe_status_updated_at.slice(0, 10))}</span>` : ""}
      </div>
      ${!path ? `<div class="asset-placeholder">${t("hyperframe_set_path")}</div>` : ""}
      ${path && !hasStatus ? `<p class="muted">${t("hyperframe_not_read")} <code>scripts/read_hyperframe_status.mjs --apply</code></p>` : ""}
