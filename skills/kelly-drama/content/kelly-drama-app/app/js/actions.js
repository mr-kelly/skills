...
// back into render()/shotsForEpisode() — functions that live in modules which
// themselves depend on this one (bindForm). Rather than a direct circular
// import, app.js registers the real implementations once every module has
// loaded. See frontend-modules.md in kelly-app-skill-creator.
const hooks = {
  render: () => {},
  shotsForEpisode: () => [],
};
...
async function saveForm(form) {
  ...
  if (kind === "series") {
    ...
    store.state = await provider.saveSeries(payload);
  } else {
    payload = serializeItem(form, kind);
    store.state = await provider.saveItem(kind, payload);
...
export function bindForm() {
  ...
  document.querySelectorAll("[data-generate-character-card]").forEach((node) => {
    node.addEventListener("click", async () => {
      const id = node.dataset.generateCharacterCard;
      node.disabled = true;
      node.textContent = t("generating");
      try {
        const provider = await getProvider();
        ...
        node.disabled = false;
        node.textContent = t("generate_reference_card");
      }
    });
  });
  document.querySelectorAll("[data-set-voice-active]").forEach((node) => {
    node.addEventListener("click", async () => {
      try {
        const provider = await getProvider();
        store.state = await provider.setCharacterVoiceActive(node.dataset.char, node.dataset.setVoiceActive);
  ...
  document.querySelectorAll("[data-set-active-image]").forEach((node) => {
    node.addEventListener("click", async () => {
      try {
        const provider = await getProvider();
        store.state = await provider.setShotActive(node.dataset.shot, "image", node.dataset.setActiveImage);
  ...
  document.querySelectorAll("[data-set-active-video]").forEach((node) => {
    node.addEventListener("click", async () => {
      try {
        const provider = await getProvider();
        store.state = await provider.setShotActive(node.dataset.shot, "video", node.dataset.setActiveVideo);
  ...
  if (deleteButton) {
    deleteButton.addEventListener("click", async () => {
      ...
      const provider = await getProvider();
      store.state = await provider.deleteItem(kind, id);
