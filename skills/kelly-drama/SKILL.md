...
1. Open the AirApp (or `pnpm --dir content/kelly-drama-app dev` for local preview, which asks you to connect Busabase and select a Space — never an API key).
   - Run `node scripts/preflight.mjs` before the first generation pass. The same capability report appears in the app's **Capabilities** settings tab and controls whether generation buttons are enabled. It checks FFmpeg/FFprobe, HyperFrames, image/video credentials, local MLX runtimes, API TTS, and the Busabase 25 MB Asset limit without exposing credential values.
...
5. In Buda's Local Browser, a Generate click writes the Busabase request and emits a versioned `buda:agent-action-request`. Buda validates that it came from the current preview and asks the user before sending its exact target command to the current Agent. The app cannot silently start an Agent run. Outside Buda, execute the trusted command shown in the UI/chat handoff.
6. Assemble an approved episode with `node scripts/render_episode.mjs --project <project-id> --episode <episode-id> --apply`. It normalizes every ordered shot, supplies silent audio where needed, falls back to an approved storyboard still when no shot video exists, concatenates a rough cut, writes an SRT sidecar, uploads a size-safe review Asset, and indexes it on the episode. HyperFrame remains the final polish surface.
7. Export a readable bible with `node scripts/export_story_bible.mjs` when the user wants a handoff, pitch note, or production brief.
...
- **Character voices**: use local Qwen3-TTS (mlx-audio, Apple Silicon) when available, or set `KELLY_DRAMA_TTS_BACKEND=openai-compatible`, `KELLY_DRAMA_TTS_BASE_URL`, and `KELLY_DRAMA_TTS_API_KEY` for a cross-platform `/audio/speech` provider. Both paths build instructions from the character's `voice_profile`. Fit each final line into its shot window with ffmpeg `atempo` during final assembly.
...
- **Execution visibility:** trusted generation changes each requested asset to `running` before work starts, then to `generated` or `blocked`. Failure details live in that asset's generation metadata and are displayed in the app. Re-running the dispatcher can resume a stranded `running` request non-destructively.
- **Asset size preflight:** all trusted uploads check the 25 MB Busabase limit before requesting an upload URL. Oversized videos are automatically transcoded to a size-safe review proxy; oversized non-video files fail early with an actionable size message.
...
node skills/kelly-drama/scripts/preflight.mjs
node skills/kelly-drama/scripts/render_episode.mjs --project my-project --episode ep-001 --apply
