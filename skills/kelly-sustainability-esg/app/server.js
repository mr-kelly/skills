...
const AIRAPP_ID = "kelly-sustainability-esg";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-sustainability-esg" }));
...
app.onError((error, context) => {
  console.error(
    "Corporate ESG & Carbon Footprint Tracker server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3211", 10);
