...
const AIRAPP_ID = "kelly-employee-relations";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-employee-relations" }));
...
app.onError((error, context) => {
  console.error("ER Workplace Grievance & Incident Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33349", 10);
