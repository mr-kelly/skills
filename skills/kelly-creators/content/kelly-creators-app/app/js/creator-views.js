...
import { calcRoi } from "./creators-model.js?v=0.1.0";
      ...
      ${
        items
          .map((item) => {
            ...
            const draft = edits.draft ?? item.suggested_reply ?? "";
          ...
          <article class="queue-card status-${escapeHtml(status)}" data-creator="${escapeHtml(item.creator_id)}">
            ...
            <div class="queue-actions">
              ...
              ${item.decided_at ? `<span class="queue-decision muted">${t("decision")}: ${escapeHtml(new Date(item.decided_at).toLocaleString())}</span>` : ""}
            </div>
          </article>
        `;
          })
...
async function submitDecision(creatorId, action, card) {
  ...
  try {
    ...
    await provider.applyDecision(creatorId, { action, comment: note, draft });
  } catch (error) {
          ...
          <tbody>
            ${items
              .map((item) => {
                ...
                const roiValue = calcRoi(spend, estValue);
                const roi = roiValue === null ? "—" : `${roiValue.toFixed(0)}%`;
                return `
                <tr>
