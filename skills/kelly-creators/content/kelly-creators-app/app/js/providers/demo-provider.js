// Deterministic, explicitly-labeled, read-only demo data (invented brand
// "Aurelia Skincare"). Ported verbatim from the retired app/server/demo.ts —
// same creators, same wording, same C³ ACE fit breakdowns — only reshaped
// into the flat row format buildSnapshot() expects (matching what
// busabase-provider.js reads off a live Base) and with the old
// decisions.json / agent_tasks.json verdicts folded directly onto the item
...
function creator(input = {}) {
  return { item_type: "engagement", created_at: NOW, decision_note: "", decided_at: "", ...input };
}
...
function demoCreators() {
  return [
    ...
    creator({
      ...
      decided_at: "2026-06-27T10:05:00.000Z",
    }),
    creator({
    ...
    creator({
      ...
      est_value: 0,
      spend: 0,
      decision_note: "Her rate is 3x our per-post ceiling and she wants exclusivity. Pass for this quarter.",
    ...
    // Content-reviewer quality gate: a pre-publication decision gate (SHIP /
    // FIX / BLOCK) checking FTC disclosure and claim authenticity before a
    // live post goes out. Part of the Activate phase.
    creator({
...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.creators = snapshot.creators.map((item) => ({
    ...item,
    reason: reasons[item.creator_id] || item.reason,
  }));
...
function withWarnings(snapshot, scenario) {
  snapshot.warnings = ["outreach", "creators", "roi"].includes(scenario)
    ? [
        {
...
export const demoProvider = {
  ...
  async getState() {
    ...
    let snapshot = buildSnapshot({ creators: demoCreators() });
    snapshot.generated_at = NOW;
    snapshot.source = "kelly-creators-demo";
    snapshot.metrics.budget_total = 50000;
    ...
    return {
      ...
      decisions: {},
      demo_visuals: demoVisualsForApp("kelly-creators"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-creators") },
    };
  },
