// The data-provider contract for kelly-creators + a runtime consistency guard.
//
// kelly-creators' unit of work is a creator engagement under
// review-before-send. Every provider (local-file default, Busabase remote)
// implements this same DataProvider shape, so the Hono routes and scripts get
// one from createProvider() and call it without knowing the backend.
// `assertProvider()` is the runtime guard that fails loudly at registration
// instead of deep in a request with `provider.getState is not a function`.
//
...
export interface DataProvider {
  ...
  /** Apply a human verdict to a creator engagement; returns updated decisions. */
  ...
  /** Persist a creator snapshot (generate_demo_snapshot). */
