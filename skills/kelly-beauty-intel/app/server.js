...
const AIRAPP_ID = "kelly-beauty-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-beauty-intel" }));
...
app.onError((error, context) => {
  console.error("Kelly Beauty Intel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3123", 10);
