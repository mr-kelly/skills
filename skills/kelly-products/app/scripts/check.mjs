...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Products must be Cloud-only");
if (/KELLY_PRODUCTS_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Products AirApp checks OK (${files.length} files)`);
