...
const ENV = {
  BUDA_API_KEY: "user-key",
  BUDA_API_KEY_ALT: "alt-key",
  BUDA_SYSTEMADMIN_KEY: "admin-key",
};
...
describe("the publish gate", () => {
  ...
  test("`new --yes` posts once, to the right URL, with the user's key", async () => {
    ...
    assert.equal(calls[0].url, "https://buda.im/api/v1/community/posts");
...
describe("the moderation gate", () => {
  ...
  test("a hard delete with --yes puts the id in the path, not the body", async () => {
    ...
    assert.equal(
      calls[0].url,
      "https://buda.im/api/v1/system-admin/community/posts/cpost%201%2Fx",
  ...
  test("without the admin key, a moderation read refuses and explains the difference", async () => {
    const { calls, fetchImpl } = recorder();
    await assert.rejects(
      () => run(["admin", "overview"], { env: { BUDA_API_KEY: "user-key" }, fetchImpl }),
      (/** @type {any} */ error) => {
...
describe("accounts", () => {
  ...
  test("an unknown account refuses instead of falling back to the default", async () => {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => {
        assert.match(error.message, /BUDA_API_KEY_GHOST is not set/);
        return true;
      },
    );
  ...
  test("$BUDA_ACCOUNT moves the default, and --as still overrides it", async () => {
    const env = { ...ENV, BUDA_ACCOUNT: "alt" };
...
describe("failure modes", () => {
  test("no key at all produces the setup instructions, not a stack trace", async () => {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => {
        assert.ok(error instanceof CommunityCliError);
        assert.match(error.message, /BUDA_API_KEY is not set/);
  ...
  test("a 401 names the credential that was actually rejected", async () => {
    ...
    await assert.rejects(
      ...
      (/** @type {any} */ error) => {
        assert.match(error.message, /BUDA_API_KEY_ALT/);
        return true;
      },
    );
  });
...
describe("upload", () => {
  ...
  test("asks for a target, PUTs the bytes, then confirms", async () => {
    ...
    assert.equal(calls[0].url, "https://buda.im/api/v1/community/attachments/upload-urls");
    assert.equal(calls[0].auth, "Bearer user-key");
    ...
    assert.equal(calls[2].url, "https://buda.im/api/v1/community/attachments/confirmations");
  ...
  test("a relative upload target is resolved against the API origin", async () => {
    ...
    assert.equal(calls[1].url, "https://buda.im/api/dev/upload?key=attachments%2Fblobs%2Fsha256%2Fab%2Fabcd.png");
  });
