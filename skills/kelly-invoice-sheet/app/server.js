...
const AIRAPP_ID = "kelly-invoice-sheet";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-invoice-sheet" }));
...
app.onError((error, context) => {
  console.error("Kelly Invoice Sheet server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3158", 10);
