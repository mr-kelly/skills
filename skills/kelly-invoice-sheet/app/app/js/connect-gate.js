...
// local preview on loopback, before the Kelly Invoice Sheet shell mounts. A deployed AirApp
...
// setup-gate.js used, so index.html and the Kelly Invoice Sheet routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Invoice Sheet will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Invoice Sheet never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
  root.innerHTML = `<div class="ks-setup-overlay"><section class="ks-setup-panel" role="dialog" aria-modal="true" aria-labelledby="ksWorkspaceTitle"><div class="ks-setup-head"><div><h1 id="ksWorkspaceTitle">${title}</h1></div></div><div class="ks-setup-body">${body}<p class="ks-setup-error" data-workspace-status hidden></p></div><div class="ks-setup-footer ks-setup-footer-split"><a class="ks-text-link" href="?demo=1#/invoices/all">Open the read-only demo</a>${canProvision ? '<button class="ks-setup-primary" type="button" data-provision>Initialize workspace</button>' : retryOnly ? '<button class="ks-setup-primary" type="button" data-retry>Check again</button>' : ""}</div></section></div>`;
...
// Runs before the Kelly Invoice Sheet shell mounts. Returns true once the app is clear to
