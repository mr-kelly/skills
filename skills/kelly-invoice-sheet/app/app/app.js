...
function statusLabel(status) {
  return t(status) || String(status || "").replaceAll("_", " ");
}
...
async function loadState({ quiet = false } = {}) {
  ...
  window.dispatchEvent(new CustomEvent("kelly-invoice-sheet:state", { detail: data }));
  render();
}
...
function noticeHtml() {
  ...
  const parts = [];
  if (lock?.locked) parts.push(`<div class="lock-banner">${t("lockedBanner")}</div>`);
...
function uploadDialogHtml() {
  ...
  const used = state.data?.batch?.invoices?.length || 0;
  return `
    <div class="extract-modal" role="dialog" aria-label="Extract Data">
      ...
      <div class="extract-tabs">
        ...
        <span class="usage"><i></i>${used} invoices extracted</span>
...
function detailHtml() {
  ...
  const locked = Boolean(state.data?.lock?.locked);
      ...
      <section class="modal" role="dialog" aria-label="${t("settings")}">
        ...
        <div class="modal-body">
          <div class="settings-card">
            <h3>Extraction</h3>
            <p><strong>Default currency</strong><code>${summary.default_currency || "USD"}</code></p>
            <p><strong>Preferred OCR</strong><code>${summary.extraction?.preferred_ocr || "agent-provided"}</code></p>
            <p><strong>Low confidence threshold</strong><code>${summary.extraction?.low_confidence_threshold ?? 0.82}</code></p>
          </div>
          <div class="settings-card">
            <h3>Export</h3>
            <p><strong>Directory</strong><code>${summary.export?.directory || "exports/<batch-id>"}</code></p>
            <p><strong>Provider</strong><code>${state.data?.data_provider || "busabase"}</code></p>
          </div>
          <div class="settings-card" id="settingsContent"></div>
          <div class="settings-card">
            <h3>Language</h3>
...
async function submitDecision(action, patch = null, comment = "") {
  ...
  if (!invoice) return;
  if (state.data?.demo) {
    state.notice = "Demo mode is read-only.";
    render();
    return;
  }
  try {
    const provider = await getProvider();
    await provider.submitDecision({ item_id: invoice.id, action, comment, patch });
  } catch (error) {
...
async function boot() {
  ...
  try {
    await loadState();
    setInterval(() => loadState({ quiet: true }), 15000);
  } catch (error) {
    ...
    const app = document.getElementById("app");
    if (app) app.innerHTML = `<div class="empty-state">${error.message}</div>`;
  }
}
