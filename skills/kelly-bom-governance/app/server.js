...
const AIRAPP_ID = "kelly-bom-governance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-bom-governance" }));
...
app.onError((error, context) => {
  console.error(
    "Manufacturing Bill of Materials (BOM) Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33316", 10);
