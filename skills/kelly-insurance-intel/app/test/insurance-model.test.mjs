...
import {
  ...
  parseBatchMeta,
  statusForAction,
} from "../app/js/insurance-model.js";
...
test("baseDraftFields/normalizeDraft: effective_body prefers edited_body over body", () => {
  const fields = baseDraftFields({
    ...
    title: "client WhatsApp: today's approved angle",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-insurance-intel-20260806",
    generated_at: "2026-08-06T09:00:00.000Z",
    source: "kelly-insurance-intel",
    vertical: "insurance and wealth advisory",
    buyer: "insurance advisors and agency managers",
    offer: "daily advisor intelligence that becomes client reminders and meeting reasons",
  };
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    drafts: [{ draft_id: "draft-1", ref: 1, channel: "client WhatsApp", status: "blocked" }],
    sources: [{ source_id: "news", label: "News" }],
    meta: { batch_id: "b1", buyer: "insurance advisors" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "client WhatsApp", title: "client WhatsApp: today's angle", next_step: undefined };
