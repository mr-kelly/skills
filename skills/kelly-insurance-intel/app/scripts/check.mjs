...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Insurance Intel must be Cloud-only");
if (/KELLY_INSURANCE_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Insurance Intel AirApp checks OK (${files.length} files)`);
