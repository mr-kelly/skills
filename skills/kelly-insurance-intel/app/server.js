...
const AIRAPP_ID = "kelly-insurance-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-insurance-intel" }));
...
app.onError((error, context) => {
  console.error("Kelly Insurance Intel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3176", 10);
