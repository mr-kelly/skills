...
const AIRAPP_ID = "kelly-customer-feedback-voe";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-customer-feedback-voe" }));
...
app.onError((error, context) => {
  console.error("Voice of Customer (VoC) Feature Router server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33370", 10);
