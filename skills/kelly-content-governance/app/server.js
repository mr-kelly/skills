...
const AIRAPP_ID = "kelly-content-governance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-content-governance" }));
...
app.onError((error, context) => {
  console.error(
    "Brand Guidelines & Content Compliance Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3208", 10);
