...
const AIRAPP_ID = "kelly-clinical-trials";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-clinical-trials" }));
...
app.onError((error, context) => {
  console.error("Clinical Trial Protocol & Subject Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33301", 10);
