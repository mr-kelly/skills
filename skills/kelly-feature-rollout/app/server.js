...
const AIRAPP_ID = "kelly-feature-rollout";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-feature-rollout" }));
...
app.onError((error, context) => {
  console.error(
    "Feature Flag Canary & Guardrail Control Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33371", 10);
