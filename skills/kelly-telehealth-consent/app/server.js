...
const AIRAPP_ID = "kelly-telehealth-consent";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-telehealth-consent" }));
...
app.onError((error, context) => {
  console.error(
    "Telehealth Informed Consent & Provider Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33308", 10);
