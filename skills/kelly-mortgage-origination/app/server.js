...
const AIRAPP_ID = "kelly-mortgage-origination";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-mortgage-origination" }));
...
app.onError((error, context) => {
  console.error("Mortgage Origination & Title Audit Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33328", 10);
