...
const AIRAPP_ID = "kelly-benefits-admin";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-benefits-admin" }));
...
app.onError((error, context) => {
  console.error(
    "Employee Benefits & Open Enrollment Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33343", 10);
