...
const AIRAPP_ID = "kelly-app-analytics";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-app-analytics" }));
...
app.onError((error, context) => {
  console.error("App Product Analytics & Journey Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3216", 10);
