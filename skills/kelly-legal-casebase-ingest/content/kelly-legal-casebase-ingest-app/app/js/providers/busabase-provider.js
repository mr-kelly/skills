...
export const busabaseProvider = {
  ...
  async getState() {
    ...
    const snapshot = buildSnapshot({ items: itemPage.rows, entities: entityPage.rows, checks: checkPage.rows, workspace: buildWorkspace(settings) });
  ...
  // Human verdict on a case record, written directly onto the item record.
  // Ported from the retired local-file DataProvider's applyDecision(): every
  // action maps through statusFromDecision()'s table and is recorded
  // literally as decision_action — this simplifies away the retired
  // local-file provider's separate decisions.json bucket and agent_tasks
  // queue, since Busabase reads are always live and nothing in the UI ever
  // read agent_tasks.
  async decideItem({ id, action, comment = "", draft } = {}) {
    ...
    await runtimeClient.records.changeRequest({
      ...
      message: `Decision on case record ${id}: ${action}`,
