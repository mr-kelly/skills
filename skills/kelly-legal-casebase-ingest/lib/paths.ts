...
export function configSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-casebase-ingest", "config.json"));
...
export function envSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-casebase-ingest", ".env"));
  return paths;
}
