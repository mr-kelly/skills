...
import { computeMetrics } from "./js/ecommerce-model.js?v=0.1.0";
...
const state = {
  ...
  lang: normalizeLang(
    new URLSearchParams(location.search).get("lang") ||
      localStorage.getItem("kelly-ecommerce-intel-language") ||
      "auto",
  ),
...
const SIDEBAR_COLLAPSED_STORAGE_KEY = "kelly-ecommerce-intel.sidebarCollapsed";
...
async function loadState() {
  ...
  window.dispatchEvent(new CustomEvent("kelly-ecommerce-intel:state", { detail: data }));
  applyDemoRoute();
  render();
}
...
els.language.addEventListener("change", () => {
  ...
  localStorage.setItem("kelly-ecommerce-intel-language", state.lang);
  render();
});
