...
import {
  ...
  parseBatchMeta,
  statusForAction,
} from "../app/js/ecommerce-model.js";
...
test("baseSignalFields/normalizeSignal round trip preserves every field", () => {
  const signal = {
    ...
    kind: "signal",
    ref: 1,
    title: "A competitor price move creates a bundle or positioning response",
...
test("baseActionFields/normalizeAction round trip preserves linked_signal_ids array", () => {
  const fields = baseActionFields({
    action_id: "action-1",
    ref: 1,
    title: "Draft listing title/bullets for one product angle",
    summary: "summary",
    status: "needs_review",
    priority: "high",
    owner: "operator",
...
test("baseDraftFields/normalizeDraft: effective_body prefers edited_body over body", () => {
  const fields = baseDraftFields({
    draft_id: "draft-1",
    ref: 1,
    channel: "listing copy",
    title: "listing copy: today's approved angle",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-ecommerce-intel-20260806",
    generated_at: "2026-08-06T09:00:00.000Z",
    source: "kelly-ecommerce-intel",
    vertical: "e-commerce and cross-border sellers",
    buyer: "e-commerce founders and marketplace operators",
    offer: "daily e-commerce intelligence that becomes listing edits",
  };
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    drafts: [{ draft_id: "draft-1", ref: 1, channel: "listing copy", status: "blocked" }],
    sources: [{ source_id: "news", label: "News" }],
    meta: { batch_id: "b1", buyer: "e-commerce founders" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "listing copy", title: "listing copy: today's angle", next_step: undefined };
...
test("operationForDecision: approve on an action (has next_step, no channel) -> export_action_plan, target is the title", () => {
  const action = { title: "Draft listing title/bullets for one product angle", next_step: "Review and approve." };
  assert.deepEqual(operationForDecision(action, "approve"), {
    operation: "export_action_plan",
    target: "Draft listing title/bullets for one product angle",
  });
});

test("operationForDecision: approve on a plain signal (no channel, no next_step) -> mark_signal_approved", () => {
  const signal = { title: "A competitor price move creates a bundle or positioning response" };
  assert.deepEqual(operationForDecision(signal, "approve"), {
    operation: "mark_signal_approved",
    target: "A competitor price move creates a bundle or positioning response",
  });
});
