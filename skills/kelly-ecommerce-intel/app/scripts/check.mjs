...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Ecommerce Intel must be Cloud-only");
if (/KELLY_ECOMMERCE_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Ecommerce Intel AirApp checks OK (${files.length} files)`);
