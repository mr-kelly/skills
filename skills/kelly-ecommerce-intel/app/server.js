...
const AIRAPP_ID = "kelly-ecommerce-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-ecommerce-intel" }));
...
app.onError((error, context) => {
  console.error("Kelly Ecommerce Intel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3173", 10);
