...
const AIRAPP_ID = "kelly-mes-quality";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-mes-quality" }));
...
app.onError((error, context) => {
  console.error("MES Quality Inspection & SPC Console server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33311", 10);
