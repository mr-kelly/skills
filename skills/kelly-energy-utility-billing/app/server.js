...
const AIRAPP_ID = "kelly-energy-utility-billing";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-energy-utility-billing" }));
...
app.onError((error, context) => {
  console.error("Utility Smart Meter & Tariff Audit Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33296", 10);
