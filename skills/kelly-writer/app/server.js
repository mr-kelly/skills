...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-writer" }));
...
app.onError((error, context) => {
  console.error("Kelly Writer server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3133", 10);
