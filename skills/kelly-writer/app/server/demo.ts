...
// Accepts either a URLSearchParams-like object (with `.get`) or a plain query
// object, so callers can pass req.query or a parsed URLSearchParams.
interface DemoQuery {
  ...
  const state = {
    ...
    batch: {
      ...
      topics: [
        {
          ...
          evidence:
            "The original article highlights three proof points: local files, explicit approval, and channel-aware exports.",
          directions: [
            {
              id: "dir-trust-before-speed",
              title: "Build the approval desk before you automate the action",
              description:
                "Lead with the safety principle, then show how local review files let teams move quickly without handing over the final decision.",
            ...
            },
            {
              ...
              description:
                "Position the workflow as a privacy-preserving assistant that prepares work but waits for a human commit.",
        ...
        },
        {
          ...
          evidence:
            "The source has a clear claim, examples, and a workflow diagram that can be adapted for different channels.",
          directions: [
            {
              id: "dir-channel-native",
              title: "Keep the proof. Change the wrapper.",
              description:
                "Explain how to preserve the core argument while rewriting hooks, pacing, and media for each platform.",
        ...
        },
        {
          ...
          evidence:
            "Screenshots show reviewers editing copy, approving exports, and requesting changes before any external side effect.",
          directions: [
            {
              id: "dir-interface",
              title: "Make agent work visible before making it powerful",
              description:
                "Turn the post into a product lesson about queues, locks, and human-readable decision records.",
      ...
      main_content: {
        ...
        cover_brief:
          "A clean workspace showing one source article feeding approved drafts into email, PR review, and publishing queues.",
...
function localizeZh(state) {
  ...
  batch.source_summary =
    "源文章说明小团队如何用本地复核队列、审批门和按渠道生成的发布草稿，在不暴露私密客户数据的前提下使用 AI。";
  ...
  batch.topics[0].directions[0].description =
    "先讲安全原则，再展示本地复核文件如何让团队快速推进，同时不放弃最终决策权。";
  ...
  batch.main_content.html =
    "<p>最快的智能体工作流不是跳过复核，而是让复核足够轻，轻到每次都值得做。</p><h3>1. 让上下文留在本地</h3><p>用本地交接文件保存批次、草稿和决定。界面可以很丰富，但不需要把私密数据发出去。</p><h3>2. 把准备和执行分开</h3><p>智能体准备摘要、建议和草稿；人类批准最终动作。</p><h3>3. 只导出已批准内容</h3><p>输出步骤读取显式决定，再写出适合各渠道的成品。</p>";
  for (const item of batch.distribution) {
    if (item.id === "dist-blog") {
      item.title = "先搭审批台，再自动执行动作";
      item.summary = "给正在设计本地优先 AI 工作流团队的主文章。";
      item.body =
        "本地审批台让团队获得速度，同时不交出控制权。先建立队列，显示建议，保留本地编辑，只在明确批准后执行。";
      ...
      item.body =
        "我最近看到最好的 AI 工作流其实很朴素：生成批次，本地复核，批准下一步，再执行。这个小小的审批层，才让智能体从聪明演示变成可用运营工具。";
      ...
      item.body =
        "本周：一个不需要盲目信任的 AI 辅助工作模式。批次留在本地，展示每条建议，保留草稿，并把审批保存成文件记录。";
      ...
      item.body =
        "有用的智能体工作流有两个独立时刻：准备工作，然后执行已批准动作。把两者混在一起，团队很快会失去信任。";
      item.cta = "先搭复核队列。";
  ...
  state.decisions["dist-newsletter"].body =
    "本周：一个不需要盲目信任的 AI 辅助工作模式。批次留在本地，展示每条建议，保留草稿，并把审批保存成文件记录。";
