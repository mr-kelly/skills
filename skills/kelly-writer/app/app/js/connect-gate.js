...
// local preview on loopback, before the writer desk shell mounts. A deployed AirApp uses
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Writer will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Writer never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the writer shell mounts. Returns true once the app is clear to load
