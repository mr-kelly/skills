...
const AIRAPP_ID = "kelly-restaurant-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-restaurant-intel" }));
...
app.onError((error, context) => {
  console.error("Kelly Restaurant Intel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3253", 10);
