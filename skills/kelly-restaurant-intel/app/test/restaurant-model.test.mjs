...
import {
  ...
  parseBatchMeta,
  statusForAction,
} from "../app/js/restaurant-model.js";
...
test("baseSignalFields/normalizeSignal round trip preserves every field", () => {
  const signal = {
    ...
    kind: "signal",
    ref: 1,
    title: "A local event nearby changes expected customer flow",
...
test("baseActionFields/normalizeAction round trip preserves linked_signal_ids array", () => {
  const fields = baseActionFields({
    action_id: "action-1",
    ref: 1,
    title: "Pick today's hero menu item or set",
    summary: "summary",
    status: "needs_review",
    priority: "high",
    owner: "operator",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-restaurant-intel-20260806",
    generated_at: "2026-08-06T09:00:00.000Z",
    source: "kelly-restaurant-intel",
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    meta: { batch_id: "b1", buyer: "restaurant owners" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "IG post", title: "IG post: today's angle", next_step: undefined };
