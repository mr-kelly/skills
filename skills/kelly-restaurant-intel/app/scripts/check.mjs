...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Restaurant Intel must be Cloud-only");
if (/KELLY_RESTAURANT_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Restaurant Intel AirApp checks OK (${files.length} files)`);
