...
const AIRAPP_ID = "kelly-aml-compliance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-aml-compliance" }));
...
app.onError((error, context) => {
  console.error(
    "Anti-Money Laundering (AML) Monitoring Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33321", 10);
