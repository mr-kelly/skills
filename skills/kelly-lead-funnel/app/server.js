...
const AIRAPP_ID = "kelly-lead-funnel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-lead-funnel" }));
...
app.onError((error, context) => {
  console.error("Kelly Lead Funnel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3159", 10);
