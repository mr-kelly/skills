...
async function loadState() {
  ...
  state.settings = data;
  applyDemoRoute();
  window.dispatchEvent(new CustomEvent("kelly-lead-funnel:state", { detail: data }));
  render();
}
...
function funnelSummaryPanel() {
  ...
  return `
    ...
    <div class="metrics">
      <div class="metric"><span>${t("total")}</span><strong>${recordCountLabel("leads", state.summary?.total || 0)}</strong></div>
...
function renderBoard() {
  ...
  els.subtitle.textContent = `${recordCountLabel("leads", state.leads.length, Boolean(state.query))} ${t("leads")}`;
...
async function moveLeadStage(id, stage, reason) {
  try {
    const provider = await getProvider();
    await provider.moveStage(id, stage, reason);
  } catch (error) {
...
async function addLeadNote(id, text) {
  try {
    const provider = await getProvider();
    await provider.addNote(id, text);
  } catch (error) {
    alert(error?.message || "Request failed");
    return;
  }
