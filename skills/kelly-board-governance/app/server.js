...
const AIRAPP_ID = "kelly-board-governance";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-board-governance" }));
...
app.onError((error, context) => {
  console.error(
    "Board Resolution & Minutes Governance Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33357", 10);
