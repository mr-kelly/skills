...
- `scripts/generate_review_batch.ts` reads PRs with `gh` and writes `app/.cache/current_batch.json`.
- The UI lets you approve, comment, request changes, block, or leave notes.
- The UI writes `app/.cache/decisions.json`.
- `scripts/execute_decisions.ts` reads approved decisions and submits reviews with `gh pr review`.
