import type { DecisionBody, StateQuery } from "./types.ts";

const now = "2026-06-18T09:30:00.000Z";

export function isDemoQuery(query: StateQuery = {}): boolean {
  return Boolean(query.demo);
}

export function demoStatePayload(query: StateQuery = {}): Record<string, unknown> {
  ...
  return demoItems().map((item) => {
    const zh = {
      ...
      "northstar/sdk#31": {
        ...
        decision: {
          action: "block",
          comment: "批准前请重新生成 type definitions。",
...
function demoItems() {
  return [
    ...
    },
    {
      ...
      decision: {
        action: "block",
        comment: "Regenerate type definitions before approval.",
        decided_at: "2026-06-17T11:25:00.000Z",
      },
    },
    {
