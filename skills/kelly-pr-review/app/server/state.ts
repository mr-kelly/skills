...
import { publicConfigSummary } from "../../lib/data-reader/local-file-reader.ts";
import { loadBatch, normalizeItem } from "./batch-store.ts";
import { lockPayload } from "./lock.ts";
import { CURRENT_BATCH_PATH, DECISIONS_PATH, EXECUTION_REPORT_PATH, TESTED_PATH } from "./paths.ts";
import { applyTestedCache, loadTestedCache } from "./tested-cache.ts";
import type { StateQuery } from "./types.ts";
import { normalizeQueryValue, readJson } from "./utils.ts";
import { isApprovedForExecution, isBlocked, isDone, isNeedsReview, isToApprove } from "./workflow.ts";
...
export async function statePayload(query: StateQuery = {}): Promise<Record<string, unknown>> {
