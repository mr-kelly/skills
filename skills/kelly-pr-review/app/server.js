...
const AIRAPP_ID = "kelly-pr-review";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-pr-review" }));
...
app.onError((error, context) => {
  console.error("Kelly PR Review server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3132", 10);
