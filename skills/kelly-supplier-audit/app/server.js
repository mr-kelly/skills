...
const AIRAPP_ID = "kelly-supplier-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-supplier-audit" }));
...
app.onError((error, context) => {
  console.error(
    "Tier-1/2 Supplier ESG & Quality Audit Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33314", 10);
