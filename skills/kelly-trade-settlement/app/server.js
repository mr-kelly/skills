...
const AIRAPP_ID = "kelly-trade-settlement";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-trade-settlement" }));
...
app.onError((error, context) => {
  console.error("Securities Trade Settlement & Fail Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33327", 10);
