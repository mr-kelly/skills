...
function normalizeBrand(input: Partial<BrandProfile> = {}): BrandProfile {
  return {
    client_id: input.client_id || "client-demo-studio",
    name: input.name || "Demo Studio",
    audience: input.audience || "Founders, operators, sales teams, and stakeholders",
    language_mode: input.language_mode || "presentation",
    style_system_id: input.style_system_id || "style-clean-growth-system",
  };
}
...
export function emptySnapshot(): PptFactorySnapshot {
  return {
    schema_version: "1",
    ...
    warnings: [
      {
        ...
        severity: "info",
        message:
          "No PPT factory snapshot exists yet. Import source materials or ask the agent to create a starter PPTX project.",
      },
    ],
    metrics: {
      project_count: 0,
...
function configSearchPaths(): string[] {
  const paths: string[] = [];
  if (process.env.KELLY_PPT_FACTORY_CONFIG) paths.push(process.env.KELLY_PPT_FACTORY_CONFIG);
  paths.push(path.join(SKILL_DIR, "config.local.json"));
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-ppt-factory", "config.json"));
...
function summarizeConfig(configResult: ConfigResult): ConfigSummary {
  ...
  return {
    ...
    default_brand_id: config.default_brand_id || brands[0]?.client_id || "client-demo-studio",
...
export function createLocalFileProvider(_meta: ProviderMeta = {}) {
  const provider = {
    ...
    async readSnapshot(): Promise<PptFactorySnapshot> {
      return (await readJson<PptFactorySnapshot>(SNAPSHOT_PATH, emptySnapshot())) as PptFactorySnapshot;
    },
    ...
    async writeSnapshot(snapshot: PptFactorySnapshot): Promise<void> {
    ...
    async getState(): Promise<PptFactoryState> {
