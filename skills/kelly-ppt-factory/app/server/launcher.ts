...
const host = process.env.KELLY_PPT_FACTORY_UI_HOST || DEFAULT_HOST;
const explicitPort = process.env.KELLY_PPT_FACTORY_UI_PORT || "";
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`Kelly PPT Factory UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_PPT_FACTORY_UI_HOST: host, KELLY_PPT_FACTORY_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly PPT Factory UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly PPT Factory UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
