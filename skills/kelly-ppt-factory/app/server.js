...
const AIRAPP_ID = "kelly-ppt-factory";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-ppt-factory" }));
...
app.onError((error, context) => {
  console.error("Kelly PPT Factory server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3166", 10);
