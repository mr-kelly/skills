...
const fetchAuthInfo = async (target, spaceId = "") => {
  ...
  if (!response.ok) throw new Error(`Busabase 鉴权校验失败（HTTP ${response.status}）。`);
  const info = await response.json();
  if (!Array.isArray(info?.spaces)) throw new Error("Busabase 鉴权响应缺少 Space 列表。");
  return info;
};
  ...
  try {
    ...
    if (target) {
      return context.json({
        ...
        space: null,
        spaces: [],
        spaceError: error instanceof Error ? error.message : "暂时无法读取 Space，请重试。",
      });
    }
...
app.get("/auth/callback", async (context) => {
  ...
  try {
    ...
    return context.redirect("/#/to-send", 303);
  } catch (error) {
    const url = new URL("/", requestOrigin(context));
...
app.post("/auth/space", async (context) => {
  try {
    ...
    if (!target) throw new Error("需要先连接 Busabase。");
    const info = await fetchAuthInfo(target);
...
app.all("/api/v1/*", async (context) => {
  ...
  const spaceId = process.env.BUSABASE_SPACE_ID || cookieValue(context, SPACE_COOKIE);
...
app.onError((error, context) => {
  console.error("Kelly JobHunt server error", error instanceof Error ? error.message : error);
