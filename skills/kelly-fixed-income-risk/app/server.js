...
const AIRAPP_ID = "kelly-fixed-income-risk";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-fixed-income-risk" }));
...
app.onError((error, context) => {
  console.error(
    "Bond Portfolio Yield & Credit Analytics Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33325", 10);
