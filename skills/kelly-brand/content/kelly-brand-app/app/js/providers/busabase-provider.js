...
function normalizePageRows(key, rows) {
  if (key === "items") return rows.map(normalizeItemRow);
  if (key === "drift-alerts") return rows.map(normalizeDriftAlertRow);
  return rows;
}
...
function configSummaryFromProfile(profile = {}) {
  ...
  return {
    config_path: "busabase:base/kelly-brand-settings",
    is_example: false,
    brand: {
      name: brand.name || "",
...
export const busabaseProvider = {
  ...
  async applyDecision(itemId, payload = {}) {
    ...
    if (DRIFT_ACTION_SET.has(action)) {
      const existing = await findRecord("drift-alerts", "alert-id", itemId);
      if (!existing) throw new Error("not_found");
      const current = normalizeFields(existing.headCommit?.payload || existing.headCommit?.fields || existing.fields);
      const fields = {
        ...current,
      ...
      await upsert("drift-alerts", "alert-id", itemId, fields, `Decision on drift alert ${itemId}: ${action}`);
      return { ok: true };
    }
