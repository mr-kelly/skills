...
const AIRAPP_ID = "kelly-ats-candidate-score";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-ats-candidate-score" }));
...
app.onError((error, context) => {
  console.error(
    "Candidate Interview Scorecard Evaluation server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33350", 10);
