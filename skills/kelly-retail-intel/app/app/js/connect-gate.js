...
// local preview on loopback, before the Kelly Retail Intel shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the Kelly Retail Intel routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Retail Intel will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Retail Intel never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Kelly Retail Intel shell mounts. Returns true once the app is clear to load
