...
const AIRAPP_ID = "kelly-retail-intel";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-retail-intel" }));
...
app.onError((error, context) => {
  console.error("Kelly Retail Intel server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3260", 10);
