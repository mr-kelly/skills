...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Retail Intel must be Cloud-only");
if (/KELLY_RETAIL_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Retail Intel AirApp checks OK (${files.length} files)`);
