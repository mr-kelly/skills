...
const AIRAPP_ID = "kelly-treasury-desk";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-treasury-desk" }));
...
app.onError((error, context) => {
  console.error("Multi-Bank Treasury & FX Risk Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3206", 10);
