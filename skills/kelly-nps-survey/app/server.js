...
const AIRAPP_ID = "kelly-nps-survey";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-nps-survey" }));
...
app.onError((error, context) => {
  console.error(
    "Automated CSAT / NPS Survey & Sentiment Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3197", 10);
