...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Invest (Webull) must be Cloud-only");
if (!configText.includes("readOnly: true")) throw new Error("Kelly Invest (Webull) AirApp must declare readOnly: true");
if (!configText.includes("writeProcedures: []"))
  throw new Error("Kelly Invest (Webull) AirApp must declare no writeProcedures");
if (/KELLY_INVEST_WEBULL_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
  throw new Error("Retired provider/runtime contract remains in app config");
}
if (/records\.changeRequest|bases\.createChangeRequest/.test(providerText)) {
  throw new Error("busabase-provider.js must stay read-only (no record/base writes)");
}
...
console.log(`Kelly Invest (Webull) AirApp checks OK (${files.length} files)`);
