...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.positions = snapshot.positions.map((pos) => ({
    ...pos,
    name: posNames[pos.symbol] || pos.name,
  }));
...
function demoSnapshot(scenario, zh) {
  ...
  const accounts = rawAccounts.map((account) => {
    const owned = rawPositions.filter((p) => p.account_id === account.account_id);
  ...
  const warnings = ["overview", "positions", "detail"].includes(scenario)
    ? [
        {
  ...
  const snapshot = assembleSnapshot(accounts, rawPositions, {
...
export const demoProvider = {
  ...
  async getState() {
    ...
    return {
      ...
      demo_visuals: demoVisualsForApp("kelly-invest-webull"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-invest-webull") },
    };
  },
