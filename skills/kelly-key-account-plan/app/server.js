...
const AIRAPP_ID = "kelly-key-account-plan";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-key-account-plan" }));
...
app.onError((error, context) => {
  console.error("Key Account Strategy & Org Chart Matrix server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33369", 10);
