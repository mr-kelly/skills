...
const explicitPort = process.env.KELLY_FINANCE_UI_PORT || "";

function isReady(port: string): Promise<boolean> {
  return new Promise((resolve) => {
    const req = http.get(`http://${host}:${port}/api/state`, { timeout: 500 }, (res) => {
      let body = "";
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode ? res.statusCode < 300 && state.app === "kelly-finance" : false);
        } catch {
          resolve(false);
        }
      });
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`Kelly Finance UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_FINANCE_UI_HOST: host, KELLY_FINANCE_UI_PORT: port },
  });
  child.unref();
  await fsp.writeFile(PID_PATH, `${child.pid}\n`);
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly Finance UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly Finance UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
