...
const AIRAPP_ID = "kelly-pricing-margin";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-pricing-margin" }));
...
app.onError((error, context) => {
  console.error(
    "Dynamic Product Pricing & Margin Optimizer server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33368", 10);
