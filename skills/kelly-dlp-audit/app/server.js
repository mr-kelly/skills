...
const AIRAPP_ID = "kelly-dlp-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-dlp-audit" }));
...
app.onError((error, context) => {
  console.error("Data Loss Prevention & PII Auditor server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3186", 10);
