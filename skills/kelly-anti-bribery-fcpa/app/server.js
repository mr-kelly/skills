...
const AIRAPP_ID = "kelly-anti-bribery-fcpa";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-anti-bribery-fcpa" }));
...
app.onError((error, context) => {
  console.error(
    "FCPA Third-Party Anti-Bribery Audit Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33355", 10);
