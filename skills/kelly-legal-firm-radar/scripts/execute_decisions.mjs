#!/usr/bin/env node
// Trusted hand-off step. Legal Firm Radar's AirApp only ever proposes a
// review decision on a management insight (approve / request changes /
// revise / block); this script is the process authorized to act on that
// decision. It performs NO external side effects — it never publishes a
// management report, notifies downstream skills, or files anything itself.
// This mirrors kelly-legal-casebase-ingest's execute_decisions.mjs
// precedent: it only ever records a planned operation
// ("planned"/"ready_for_agent") and NEVER flips an item's workflow `status`
// itself, unlike the retired scripts/execute_decisions.ts (which set
// item.status = "done" directly on --apply for an "approve" decision) — the
// real follow-up (export the management report via
// scripts/export_management_report.mjs) is performed by the agent OUTSIDE
...
follow-up operation (export_management_report / request_revision) for each.
With --apply it writes an execution marker (execution-status:
"ready_for_agent", operation, target, detail) back onto each item — it
performs no export, no downstream notification, no filing itself, and never
changes the item's workflow status. The agent performs the real follow-up
outside the app after this report (scripts/export_management_report.mjs),
then records the real result.`);
}
...
async function main() {
  ...
  if (!decided.length) {
    console.log("No approved or changes-requested management insights to execute. Nothing written.");
    return;
  }
  ...
  for (const row of decided) {
    ...
    if (apply) {
      await client.records.changeRequest({
        ...
        author: "kelly-legal-firm-radar-execute-decisions",
