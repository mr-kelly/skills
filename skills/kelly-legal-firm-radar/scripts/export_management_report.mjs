#!/usr/bin/env node
// Trusted export step. Legal Firm Radar's AirApp only ever writes a review
// decision onto a management-insight record — the browser cannot write to
// the local filesystem. This script re-reads Busabase for genuinely
// approved insights and exports them as Markdown + JSON + CSV, preserving
// the exact document format (headings, field order, CSV columns) from the
...
// "approved"/"done". This closes the spoofed-import gap the retired script
// had (scripts/import_metrics.ts setting status: "approved" directly, with
// no real human decision, used to be enough to export). This script is also
// now the one that marks the item "done" after a real export —
// scripts/execute_decisions.mjs deliberately never touches workflow status
// (see its module doc comment), so export is the only write in this
// Busabase-only shape that finalizes an item.
//
// Usage: node scripts/export_management_report.mjs [--out /path/to/dir]
//
...
function help() {
  console.log(`Usage: node scripts/export_management_report.mjs [--out /path/to/dir]

Reads management insights with a genuine "approve" decision from Busabase
and writes approved-items.md, approved-items.json, and approved-items.csv
into --out (default: <skill>/exports, gitignored). Marks each exported
record "done" in Busabase. External brand use and any other external side
effect remain the agent's job outside the app after explicit approval.`);
}
...
function baseItemFields(row) {
  return {
    ...
    public_citable: row.public_citable ?? "",
...
// Ported verbatim from the retired scripts/export_management_report.ts
// (same heading shape, same field order).
function csv(value) {
...
async function main() {
  ...
  if (!rows.length) {
    console.log("No management insights found. Nothing to export.");
    return;
  }
  ...
  if (!exportableRows.length) {
    console.log("No approved management insights to export.");
    return;
  }
  ...
  for (const row of exportableRows) {
    await client.records.changeRequest({
      ...
      message: `Export management insight ${row.item_id}`,
      author: "kelly-legal-firm-radar-export",
  ...
  console.log(
    `Exported ${items.length} approved item(s) to ${outDir}. External brand use remains a separate approved step.`,
  );
}
