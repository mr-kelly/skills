// Pure domain logic for kelly-legal-firm-radar, ported verbatim (same field
// names, same order of operations, only TS types stripped) from the retired
// lib/common.ts (emptyMetrics/recomputeMetrics/statusFromDecision) and
...
// kelly-legal-casebase-ingest's items Base. `agent_tasks.json` (queued
...
// app's actual behavior (app/app.js's effectiveItem() used the identical
// statusByAction table, and lib/data-provider/local-file-provider.ts's
...
export function normalizeItemRow({
  ...
  return {
    ...
    fields: {
      sample_size: sample_size === "" ? undefined : Number(sample_size),
      period,
      visibility,
      lawyer_count: lawyer_count === "" ? undefined : Number(lawyer_count),
      public_citable: public_citable === "" ? undefined : Number(public_citable),
      quality_indicators: parseJsonValue(quality_indicators, []) || [],
    },
    decision_action,
...
// (firm_profile/analytics_policy/taxonomy/export).
/**
...
export function buildConfigSummary({ settings = {} } = {}) {
  ...
  const allowedViews = parseJsonValue(settings.allowed_views, []) || [];
  const practiceAreas = parseJsonValue(settings.practice_areas, []) || [];
  return {
    ...
    analytics_policy: {
      require_anonymized_metadata: boolField(settings.require_anonymized_metadata, true),
      minimum_sample_size_for_claim:
        settings.minimum_sample_size_for_claim === undefined || settings.minimum_sample_size_for_claim === ""
          ? 0
          : Number(settings.minimum_sample_size_for_claim),
      external_brand_claims_require_approval: boolField(settings.external_brand_claims_require_approval, true),
    ...
    export: {
      ...
      out_dir: settings.export_out_dir || "exports/management-reports",
    },
  };
}
...
// checks_failed) — case_samples/practice_groups/lawyers_profiled/
// public_citable/outcome_trends have no dedicated Busabase column on items
// (app.js's businessMetricsHtml already falls back to summing the item/
// entity fields when absent); `extra` lets a caller (e.g. the Settings
// row's outcome-trends field, or the demo dataset) merge in display-only
// metrics the same way the retired demo/server payload did. ----
...
// kelly-legal-casebase-ingest's deriveActivityLog.
  ...
  for (const item of items) {
    if (item.created_at) {
      entries.push({
        ...
        actor: "agent",
        action: "prepare",
        detail: `Prepared management insight "${item.title}" for review.`,
        count: 1,
      });
    }
    if (item.updated_at && item.updated_at !== item.created_at) {
      entries.push({
        ...
        actor: "agent",
        action: "update",
        detail: `Updated management insight "${item.title}".`,
        count: 1,
      });
    }
...
// kelly-legal-casebase-ingest's more conservative precedent instead — it
// never flips workflow status itself, only records an execution marker
// directly on the item record (execution_status/operation/target/detail/executed_at).
export function itemExecution(item, action, { apply = false } = {}) {
  if (action === "approve") {
    const target = `exports/management-reports/${slugify(item.title || item.id)}.md`;
    return {
      operation: "export_management_report",
      target,
      status: apply ? "ready_for_agent" : "planned",
      detail: `Run scripts/export_management_report.mjs to write ${target}. External brand use remains a separate explicit approval, never this script.`,
    };
  }
  if (action === "request_changes") {
    return {
      ...
      target: item.id,
      status: apply ? "ready_for_agent" : "planned",
      detail: "Redraft the management insight per the review note, then re-import with scripts/import_metrics.mjs.",
    };
  }
  return null;
}
