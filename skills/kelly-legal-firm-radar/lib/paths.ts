...
export const DEFAULT_PORT = 3130;
...
export function configSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-firm-radar", "config.json"));
...
export function envSearchPaths(): string[] {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "kelly-legal-firm-radar", ".env"));
  return paths;
}
