...
This schema describes `app/.data/firm_radar_snapshot.json`, the local handoff file shared by the agent, scripts, and the App UI.
...
| `ref` | yes | Human-friendly stable reference such as `Insight #1`. |
...
| `proposed_action` | no | Domain operation, usually `approve_management_report`. |
...
`scripts/import_metrics.ts` accepts a JSON payload with any of these keys:
