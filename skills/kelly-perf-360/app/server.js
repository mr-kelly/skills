...
const AIRAPP_ID = "kelly-perf-360";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-perf-360" }));
...
app.onError((error, context) => {
  console.error(
    "Performance Review & 360 Feedback Collector server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3194", 10);
