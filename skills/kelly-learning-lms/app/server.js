...
const AIRAPP_ID = "kelly-learning-lms";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-learning-lms" }));
...
app.onError((error, context) => {
  console.error(
    "Compliance Training & LMS Certification Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33346", 10);
