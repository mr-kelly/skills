...
async function loadState() {
  ...
  state.settings = data;
  window.dispatchEvent(new CustomEvent("kelly-deal-scorer:state", { detail: data }));
  render();
}
...
function renderCandidates() {
  ...
  els.subtitle.textContent = `${recordCountLabel("candidates", items.length, Boolean(state.query || state.route.statusFilter))} ${t("candidates")}`;
  if (!isConnected()) {
    els.content.innerHTML = `${emptyState(t("needsConnection"))}
    ${pagerControl("candidates")}`;
    return;
  }
...
async function submitDecision(id, action, comment) {
  ...
  try {
    const provider = await getProvider();
    await provider.submitReview({ id, action, comment });
    await loadState();
  } catch (error) {
...
const PAGE_BINDINGS = {
  candidates: { key: "candidates", path: "batch.items" },
};
