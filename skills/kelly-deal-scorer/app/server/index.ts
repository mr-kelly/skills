...
const port = Number.parseInt(process.env.KELLY_DEAL_SCORER_UI_PORT || process.env.PORT || String(DEFAULT_PORT), 10);

await ensureDirs();
await loadDotenvFiles(envSearchPaths());

serve({ fetch: app.fetch, hostname: host, port }, (info) => {
  console.log(`Deal Scoring Desk UI: http://${host}:${info.port}`);
});
