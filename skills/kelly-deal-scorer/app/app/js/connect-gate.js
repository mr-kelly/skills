...
// local preview on loopback, before the Deal Scoring Desk shell mounts. A deployed AirApp
...
// setup-gate.js used, so index.html and the Deal Scoring Desk routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Deal Scorer will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Deal Scorer never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Deal Scoring Desk shell mounts. Returns true once the app is clear to
