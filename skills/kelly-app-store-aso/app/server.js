...
const AIRAPP_ID = "kelly-app-store-aso";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-app-store-aso" }));
...
app.onError((error, context) => {
  console.error(
    "App Store Optimization (ASO) & Review Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33387", 10);
