...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Real Estate Intel must be Cloud-only");
if (/KELLY_REAL_ESTATE_INTEL_UI_PORT|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Kelly Real Estate Intel AirApp checks OK (${files.length} files)`);
