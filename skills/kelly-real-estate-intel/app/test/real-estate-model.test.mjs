...
import {
  ...
  parseBatchMeta,
  statusForAction,
} from "../app/js/real-estate-model.js";
...
test("baseSignalFields/normalizeSignal round trip preserves every field", () => {
  const signal = {
    ...
    kind: "signal",
    ref: 1,
    title: "A nearby transaction gives agents a timely reason to message owners and buyers",
...
test("baseActionFields/normalizeAction round trip preserves linked_signal_ids array", () => {
  const fields = baseActionFields({
    action_id: "action-1",
    ref: 1,
    title: "Pick today's hero listing and explain the buyer angle",
    summary: "summary",
    status: "needs_review",
    priority: "high",
    owner: "operator",
...
test("baseDraftFields/normalizeDraft: effective_body prefers edited_body over body", () => {
  const fields = baseDraftFields({
    ...
    title: "WhatsApp follow-up: today's approved angle",
...
test("parseBatchMeta/batchMetaPayload round trip", () => {
  const meta = {
    schema_version: "1",
    batch_id: "kelly-real-estate-intel-20260806",
...
test("buildSnapshot: assembles normalized batch and rollup metrics together", () => {
  const batch = buildSnapshot({
    ...
    drafts: [{ draft_id: "draft-1", ref: 1, channel: "WhatsApp follow-up", status: "blocked" }],
    sources: [{ source_id: "news", label: "News" }],
    meta: { batch_id: "b1", buyer: "property agency owners" },
  });
...
test("operationForDecision: approve on a draft (has channel) -> handoff_content_pack, target is the channel", () => {
  const draft = { channel: "WhatsApp follow-up", title: "WhatsApp follow-up: today's angle", next_step: undefined };
...
test("operationForDecision: approve on an action (has next_step, no channel) -> export_action_plan, target is the title", () => {
  const action = { title: "Pick today's hero listing and explain the buyer angle", next_step: "Review and approve." };
  assert.deepEqual(operationForDecision(action, "approve"), {
    operation: "export_action_plan",
    target: "Pick today's hero listing and explain the buyer angle",
  });
});

test("operationForDecision: approve on a plain signal (no channel, no next_step) -> mark_signal_approved", () => {
  const signal = { title: "A nearby transaction gives agents a timely reason to message owners and buyers" };
  assert.deepEqual(operationForDecision(signal, "approve"), {
    operation: "mark_signal_approved",
    target: "A nearby transaction gives agents a timely reason to message owners and buyers",
  });
});
