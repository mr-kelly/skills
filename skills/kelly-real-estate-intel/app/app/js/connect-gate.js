...
// local preview on loopback, before the Kelly Real Estate Intel shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the Kelly Real Estate Intel routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Real Estate Intel will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Real Estate Intel never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Kelly Real Estate Intel shell mounts. Returns true once the app is clear to load
