...
const AIRAPP_ID = "kelly-m-and-a-vdr";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-m-and-a-vdr" }));
...
app.onError((error, context) => {
  console.error(
    "M&A Virtual Data Room (VDR) Due Diligence server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33358", 10);
