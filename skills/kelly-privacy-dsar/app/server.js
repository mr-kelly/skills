...
const AIRAPP_ID = "kelly-privacy-dsar";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-privacy-dsar" }));
...
app.onError((error, context) => {
  console.error("Privacy DSAR Right-to-be-Forgotten Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33360", 10);
