...
const AIRAPP_ID = "kelly-container-security";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-container-security" }));
...
app.onError((error, context) => {
  console.error("Kubernetes Image Vulnerability Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33333", 10);
