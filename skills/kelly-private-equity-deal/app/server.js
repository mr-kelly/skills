...
const AIRAPP_ID = "kelly-private-equity-deal";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-private-equity-deal" }));
...
app.onError((error, context) => {
  console.error(
    "PE / VC Deal Sourcing & LBO Valuation Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33326", 10);
