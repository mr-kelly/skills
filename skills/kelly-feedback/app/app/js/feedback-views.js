...
import { statusForProposalAction, triageForFeedbackAction } from "./feedback-model.js?v=0.1.0";
...
export function renderInboxDetail() {
      ...
      <div class="detail-main">
        ...
        <div class="panel triage-panel">
          ...
          <div class="triage-actions">
            ...
            <button type="button" data-action="assign">${t("assignToRequest")}</button>
            <button type="button" data-action="ignore">${t("ignore")}</button>
            <button type="button" data-action="insight">${t("markInsight")}</button>
          </div>
        </div>
      </div>
  ...
  els.content.querySelectorAll("[data-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      decideFeedbackAction(
        item.feedback_id,
        action,
        action === "assign" ? (state.edits.assign[item.feedback_id] ?? item.request_id ?? "") : "",
      );
    });
  });
}
...
export function renderRequestDetail() {
  ...
  const quoteItems = (request.representative_feedback_ids || [])
    ...
    .filter(Boolean);
  const effortValue = state.edits.effort[request.request_id] ?? request.effort_estimate ?? "";
    ...
    <section class="detail">
      ...
      <aside class="detail-side">
        ...
        <div class="effort-field">
          ...
          <input id="effortInput" type="text" value="${escapeHtml(effortValue)}">
          <button type="button" id="effortSave">${t("save")}</button>
        </div>
  ...
  els.content.querySelector("#effortSave")?.addEventListener("click", () => {
    saveEffortAction(request.request_id, state.edits.effort[request.request_id] ?? effortValue);
  });
}
  ...
  return `
    <article class="proposal-card" data-proposal="${escapeHtml(proposal.proposal_id)}">
      ...
      ${
        ...
        <textarea class="proposal-draft" data-field="draft" rows="4" ${decided ? "disabled" : ""}>${escapeHtml(draftValue)}</textarea>
      `
          : ""
      }
      <label class="field-label">${t("reviewNote")}</label>
      <textarea class="proposal-note" data-field="note" rows="2" placeholder="${escapeHtml(t("reviewNotePlaceholder"))}">${escapeHtml(noteValue)}</textarea>
      <footer class="proposal-actions">
        <button type="button" data-verdict="approve" class="primary" ${decided ? "disabled" : ""}>${t("approve")}</button>
        <button type="button" data-verdict="request_changes" ${proposal.status === "done" ? "disabled" : ""}>${t("requestChanges")}</button>
        <button type="button" data-verdict="block" ${decided ? "disabled" : ""}>${t("block")}</button>
    ...
    card.querySelectorAll("button[data-verdict]").forEach((button) => {
      button.addEventListener("click", () => {
        decideProposalAction(
          ...
          state.edits.note[proposalId] ?? "",
          state.edits.draft[proposalId],
        );
      });
    });
  });
...
async function decideFeedbackAction(feedbackId, action, requestId) {
  if (state.settings?.demo) {
    const item = feedbackItems().find((entry) => entry.feedback_id === feedbackId);
    if (item) {
      item.triage = triageForFeedbackAction(action, item.triage);
      if (action === "assign") item.request_id = requestId || item.request_id;
    }
    delete state.edits.assign[feedbackId];
  ...
  try {
    ...
    await provider.decideFeedback({ feedback_id: feedbackId, action, request_id: requestId });
  } catch (error) {
    toast(error.message);
    return;
  }
...
async function saveEffortAction(requestId, effortEstimate) {
  if (state.settings?.demo) {
    const request = requests().find((entry) => entry.request_id === requestId);
    if (request) request.effort_estimate = effortEstimate;
    delete state.edits.effort[requestId];
    toast(t("demoNotPersisted"));
    render();
    return;
  }
  try {
    const provider = await getProvider();
    await provider.saveRequestEffort({ request_id: requestId, effort_estimate: effortEstimate });
  } catch (error) {
    toast(error.message);
    return;
  }
...
async function decideProposalAction(proposalId, action, reviewNote, draft) {
  if (state.settings?.demo) {
    const proposal = proposals().find((entry) => entry.proposal_id === proposalId);
    if (proposal) {
      proposal.status = statusForProposalAction(action, proposal.status);
      if (reviewNote) proposal.review_note = reviewNote;
      if (typeof draft === "string") proposal.draft = draft;
      if (action !== "revise") proposal.decided_at = new Date().toISOString();
    }
    delete state.edits.draft[proposalId];
    delete state.edits.note[proposalId];
    toast(t("demoNotPersisted"));
    render();
    return;
  }
  try {
    const provider = await getProvider();
    await provider.decideProposal({ proposal_id: proposalId, action, review_note: reviewNote, draft });
  } catch (error) {
    toast(error.message);
    return;
  }
