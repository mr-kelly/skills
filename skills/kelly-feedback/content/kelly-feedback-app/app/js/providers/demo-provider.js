...
// Ported verbatim from the retired app/server/demo.ts (same feedback/request/
// proposal ids, same copy, same personas PulseBoard/Formora). Derived fields
...
function product(product_id, display_name, tagline) {
  return { product_id, display_name, tagline };
}
...
function source(source_id, channel, name, collection, last_ingest_at, item_count) {
  return {
    ...
    name,
    collection,
    secret_envs: [`KELLY_FEEDBACK_${channel.toUpperCase()}_TOKEN_DEMO`],
    last_ingest_at,
    item_count,
    status: "ok",
  };
...
function demoProducts() {
  return [
    product("pulseboard", "PulseBoard", "Product analytics dashboards for small teams"),
    product("formora", "Formora", "Forms and surveys that feel handmade"),
  ];
}
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-feedback");
    return {
      demo: true,
