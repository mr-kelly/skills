...
const state = {
  ...
  query: "",
  anomalyFilter: "all",
  lang: normalizeLang(new URLSearchParams(location.search).get("lang") || localStorage.getItem("kelly-audit-language") || "auto"),
...
function money(value, currency) {
  ...
  try {
    return new Intl.NumberFormat(locale(), { style: "currency", currency: code, maximumFractionDigits: 2 }).format(Number(value || 0));
  } catch {
...
function date(value) {
  if (!value) return "";
  return new Intl.DateTimeFormat(locale(), { month: "short", day: "2-digit", year: "numeric" }).format(new Date(`${String(value).slice(0, 10)}T00:00:00`));
}
...
function renderShell() {
  ...
  els.syncStatus.textContent = snapshot && orderCount
...
function anomalyBadges(anomalyIds) {
  ...
  return list.map((anomaly) => `<a class="badge badge-${escapeHtml(anomaly.rule)}" href="#/anomalies/${encodeURIComponent(anomaly.id)}" title="${escapeHtml(anomaly.title)}">#${anomaly.ref} ${escapeHtml(enumLabel(anomaly.rule, "rule"))}</a>`).join(" ");
}
  ...
  els.content.innerHTML = `
    ...
    <section class="overview-grid">
      <div class="overview-panel">
        ...
        ${list.filter((item) => item.status !== "done").slice(0, 5).map((item) => `
        ...
        ${importLog.slice(0, 5).map((entry) => `
          <div class="import-row">
            <span>
              ...
              <small>${escapeHtml(Object.values(entry.files || {}).filter(Boolean).join(" · "))}</small>
...
function filteredOrders() {
  ...
  return orders().filter((order) => [order.order_no, order.customer, order.invoice_status, order.payment_status, order.currency]
    .filter(Boolean)
...
function renderOrderDetail() {
  ...
  for (const invoice of linkedInvoices) {
    chainRows.push(chainRow(invoiceTone(invoice.status),
    ...
    if (!(invoice.payment_ids || []).length && invoice.status !== "credit_note") {
      chainRows.push(chainRow(invoice.days_overdue ? "bad" : "warn",
    ...
    <section class="detail">
      <div class="detail-main">
        ...
        ${(order.anomaly_ids || []).length ? `
          <div class="overview-panel wide">
            <h2>${t("anomalies")}</h2>
            ${(order.anomaly_ids || []).map((id) => anomalyById(id)).filter(Boolean).map((anomaly) => `
...
function filteredInvoices() {
  ...
  return invoices().filter((invoice) => [invoice.invoice_no, invoice.order_no, invoice.customer, invoice.status, invoice.currency]
    .filter(Boolean)
    .some((value) => String(value).toLowerCase().includes(query)));
}
    ...
    <section class="detail">
      <div class="detail-main">
        ...
        ${(invoice.anomaly_ids || []).length ? `
          <div class="overview-panel wide">
            <h2>${t("anomalies")}</h2>
            ${(invoice.anomaly_ids || []).map((id) => anomalyById(id)).filter(Boolean).map((anomaly) => `
              <a class="health-row" href="#/anomalies/${encodeURIComponent(anomaly.id)}">
                <span><strong>${t("anomaly")} #${anomaly.ref} · ${escapeHtml(anomaly.title)}</strong><small>${escapeHtml(enumLabel(anomaly.rule, "rule"))}</small></span>
                <span class="num">${money(anomaly.amount_at_stake, anomaly.currency)}</span>
                ${statusBadge(anomaly.status)}
...
function renderSettings() {
  ...
  const tableRow = (name, table) => `
    <div class="settings-account">
      ...
      <span>${escapeHtml(Object.entries(table?.columns || {}).map(([key, value]) => `${key}←${value}`).join(", ") || t("setupNeeded"))}</span>
    </div>
  `;
  els.content.innerHTML = `
    <div class="settings">
