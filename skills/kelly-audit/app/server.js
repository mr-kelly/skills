...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-audit" }));
...
app.onError((error, context) => {
  console.error("kelly-audit server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3143", 10);
