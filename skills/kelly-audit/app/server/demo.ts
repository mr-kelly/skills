...
import { deriveSnapshot } from "./compute.ts";
import type {
  AgentTasksFile,
  Anomaly,
  AuditSnapshot,
...
const ORDER_ROWS: Array<[string, string, string, number, string]> = [
...
const INVOICE_ROWS: Array<[string, string, string, string, string, number, string, string]> = [
...
const PAYMENT_ROWS: Array<[string, string, string, string, number, string, string]> = [
...
function buildTables(): { orders: Order[]; invoices: Invoice[]; payments: Payment[] } {
  const orders: Order[] = ORDER_ROWS.map(([order_no, customer, order_date, amount, currency]) => ({
  ...
  const invoices: Invoice[] = INVOICE_ROWS.map(
    ([invoice_no, order_no, customer, issue_date, due_date, amount, currency, kind]) => ({
  ...
  const payments: Payment[] = PAYMENT_ROWS.map(
    ([payment_ref, invoice_no, payer, paid_date, amount, currency, method]) => ({
...
function demoAnomalies(): Anomaly[] {
  return [
    {
      ...
      aging_bucket: "61-90",
      reason:
        "Invoice INV-2026-041 (US$18,400.00) was due 2026-05-01 and has no matching payment. 62 days overdue puts it in the 61-90 aging bucket — the largest open receivable.",
      evidence: {
        ...
        rows: [
          ...
          {
            label: "Invoice INV-2026-041",
            detail: "Issued 2026-04-16 · due 2026-05-01",
            amount: 18400,
            currency: "USD",
          },
      ...
      agent_notes:
        "Harborline paid INV-2026-046 on time in May, so the account is active. No dispute is on file for this invoice.",
    ...
    },
    {
      ...
      currency: "USD",
      reason:
        "Order SO-2026-1002 totals US$9,600.00 but invoice INV-2026-042 was issued for US$9,120.00 — a −US$480.00 (5.0%) gap, beyond the 1% tolerance. The customer paid the invoice amount, leaving US$480.00 unbilled.",
      evidence: {
        ...
        rows: [
          {
            label: "Order SO-2026-1002",
            detail: "Northgate Manufacturing · 2026-04-05",
            amount: 9600,
            currency: "USD",
          },
          {
            label: "Invoice INV-2026-042",
            detail: "Issued 2026-04-12 · due 2026-05-12",
            amount: 9120,
            currency: "USD",
          },
    ...
    },
    {
      ...
      currency: "USD",
      reason:
        "Two payments of US$4,250.00 (RCP-5531 on 2026-05-06 and RCP-5538 on 2026-05-07) both reference invoice INV-2026-043. The invoice total is US$4,250.00, so the account is overpaid by US$4,250.00.",
      evidence: {
        ...
        rows: [
          {
            label: "Invoice INV-2026-043",
            detail: "Issued 2026-04-14 · due 2026-05-08",
            amount: 4250,
            currency: "USD",
          },
          { label: "Payment RCP-5531", detail: "Check · 2026-05-06", amount: 4250, currency: "USD" },
          {
            label: "Payment RCP-5538",
            detail: "Check · 2026-05-07 · same amount, 1 day apart",
            amount: 4250,
            currency: "USD",
          },
        ],
      ...
      agent_notes:
        "Both entries came from the June bank export with different bank references, so this is not an import artifact.",
      created_at: "2026-07-01T09:05:00.000Z",
      decision: null,
      execution: null,
    },
    {
      ...
      currency: "USD",
      reason:
        "Order SO-2026-1004 (US$7,800.00, 2026-06-05) has no invoice after 27 days; the policy threshold is 14 days. Revenue is unbilled and the payment clock has not started.",
      evidence: {
      ...
      agent_notes:
        "The order status in the export is 'fulfilled', so this looks like a billing miss rather than a hold.",
      created_at: "2026-07-01T09:05:00.000Z",
      decision: null,
      execution: null,
    },
    {
      ...
      currency: "USD",
      reason:
        "Order SO-2026-1005 (US$3,150.00, 2026-06-10) has no invoice after 22 days; the policy threshold is 14 days.",
      evidence: {
    ...
    },
    {
      ...
      currency: "USD",
      reason:
        "Payment RCP-5560 (US$2,900.00, 2026-06-15) references invoice INV-2026-058, which does not exist in the imported invoices. No open Bluecrest invoice matches the amount.",
      evidence: {
        order_id: "",
        invoice_id: "",
        ...
        rows: [
          {
            label: "Payment RCP-5560",
            detail: "Wire · 2026-06-15 · memo: INV-2026-058",
            amount: 2900,
            currency: "USD",
          },
          { label: "Invoice INV-2026-058", detail: "Not found in imported invoices", amount: 0, currency: "USD" },
          {
            label: "Open Bluecrest invoices",
            detail: "INV-2026-051 open at US$4,780.00 — amount does not match",
            amount: 4780,
            currency: "USD",
          },
        ],
    ...
    },
    {
      ...
      currency: "USD",
      reason:
        "Credit note INV-2026-052 (−US$1,200.00, 2026-05-22) for Summit Fabrication references no order and no original invoice. Negative entries without a paper trail need documentation before month close.",
      evidence: {
        order_id: "",
        ...
        rows: [
          {
            label: "Credit note INV-2026-052",
            detail: "Issued 2026-05-22 · no order reference",
            amount: -1200,
            currency: "USD",
          },
      ...
      agent_notes:
        "Blocked: sales says a contract amendment covering this credit is still being signed. Revisit after the amendment arrives.",
      created_at: "2026-07-01T09:05:00.000Z",
      decision: {
        action: "block",
    ...
    },
    {
      ...
      aging_bucket: "31-60",
      reason:
        "Invoice INV-2026-039 (US$12,600.00) was 45 days overdue when flagged in June. A chasing email was approved and sent; payment RCP-5502 arrived 2026-06-20 and the anomaly auto-resolved.",
      evidence: {
        ...
        rows: [
          {
            label: "Invoice INV-2026-039",
            detail: "Issued 2026-04-02 · due 2026-05-02",
            amount: 12600,
            currency: "USD",
          },
          {
            label: "Payment RCP-5502",
            detail: "Wire · 2026-06-20 · received after reminder",
            amount: 12600,
            currency: "USD",
          },
        ],
...
function demoImportLog(): ImportLogEntry[] {
  return [
    {
      import_id: "imp-20260701-0902",
      imported_at: "2026-07-01T09:02:00.000Z",
      files: {
        orders: "exports/orders_2026Q2.csv",
        invoices: "exports/invoices_2026Q2.csv",
        payments: "exports/payments_jun.csv",
      },
    ...
    },
    {
      ...
      files: {
        orders: "exports/orders_may.csv",
        invoices: "exports/invoices_may.csv",
        payments: "exports/payments_may.csv",
      },
    ...
    },
    {
      ...
      files: {
        orders: "exports/orders_apr.csv",
        invoices: "exports/invoices_apr.csv",
        payments: "exports/payments_apr.csv",
      },
...
export function buildDemoSnapshot(): AuditSnapshot {
...
function demoAgentTasks(zh: boolean): AgentTasksFile {
  return {
    updated_at: NOW,
    tasks: [
      {
...
function demoExecutionReport(zh: boolean): ExecutionReport {
  return {
...
const CUSTOMER_ZH: Record<string, string> = {
...
const ANOMALY_ZH: Record<string, AnomalyZh> = {
  "anom-overdue_receivable-inv-2026-041": {
    title: "明华贸易发票逾期 62 天未回款",
    reason:
      "发票 INV-2026-041（US$18,400.00）应于 2026-05-01 到期，至今没有匹配的回款。逾期 62 天，落入 61-90 天账龄区间，是当前最大的一笔未收应收款。",
  ...
  "anom-amount_mismatch-so-2026-1002": {
    ...
    reason:
      "订单 SO-2026-1002 总额 US$9,600.00，但发票 INV-2026-042 只开了 US$9,120.00，差 −US$480.00（5.0%），超过 1% 的容差。客户按发票金额付款，还有 US$480.00 未开票。",
  ...
  "anom-duplicate-rcp-5538": {
    title: "松岚商贸重复付款",
    reason:
      "两笔 US$4,250.00 的付款（RCP-5531，2026-05-06；RCP-5538，2026-05-07）都对应发票 INV-2026-043。发票总额 US$4,250.00，账户多收 US$4,250.00。",
  ...
  "anom-missing_invoice-so-2026-1004": {
    ...
    reason:
      "订单 SO-2026-1004（US$7,800.00，2026-06-05）已 27 天没有对应发票，超过 14 天的开票时限。收入未入账，账期也未开始计算。",
  ...
  "anom-unmatched_payment-rcp-5560": {
    ...
    reason:
      "回款 RCP-5560（US$2,900.00，2026-06-15）备注引用发票 INV-2026-058，但导入的发票里没有这张发票，也没有金额匹配的蓝峰未结发票。",
  ...
  "anom-irregular_entry-inv-2026-052": {
    ...
    reason:
      "峰达机械的贷项发票 INV-2026-052（−US$1,200.00，2026-05-22）没有引用任何订单或原始发票。月结前，负数分录必须补齐凭证。",
  ...
  "anom-overdue_receivable-inv-2026-039": {
    title: "松涛电子逾期发票已追回",
    reason:
      "发票 INV-2026-039（US$12,600.00）6 月被标记时已逾期 45 天。催款邮件经批准后发出；回款 RCP-5502 于 2026-06-20 到账，异常自动关闭。",
...
function localizeSnapshotZh(snapshot: AuditSnapshot): AuditSnapshot {
  snapshot.company = { name: "明威贸易" };
  const mapCustomer = (name: string): string => CUSTOMER_ZH[name] || name;
  ...
  snapshot.anomalies = snapshot.anomalies.map((anomaly) => {
    ...
    return {
      ...
      decision: anomaly.decision ? { ...anomaly.decision, note: zh.decision_note || anomaly.decision.note } : null,
  ...
  snapshot.import_log = snapshot.import_log.map((entry) => ({
    ...
    warnings: entry.warnings.map((warning) =>
      warning.replace(
        "Payment RCP-5560 references unknown invoice INV-2026-058.",
        "回款 RCP-5560 引用了不存在的发票 INV-2026-058。",
      ),
    ),
  }));
  return snapshot;
