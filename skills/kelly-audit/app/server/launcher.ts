...
const host = process.env.KELLY_AUDIT_UI_HOST || DEFAULT_HOST;
const explicitPort = process.env.KELLY_AUDIT_UI_PORT || "";
    ...
    const req = http.get(stateUrlFor(port), { timeout: 500 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode >= 200 && res.statusCode < 300 && state.app === "kelly-audit");
        } catch {
          resolve(false);
        }
      });
...
async function main() {
  ...
  if (await isReady(port)) {
    console.log(`Kelly Audit UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_AUDIT_UI_HOST: host, KELLY_AUDIT_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly Audit UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly Audit UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
