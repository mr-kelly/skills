// Shared types for the kelly-audit data-provider layer, server, and scripts.
// These model the ACTUAL shapes produced by app/server/compute.ts, demo.ts, the
// batch scripts, and the snapshot validated by scripts/validate_ui_schema.ts.
// Derived fields (populated by deriveSnapshot) are optional so raw imported rows
// also satisfy the interfaces.
...
// Busabase connection config, read from config.busabase (env overrides win).
...
// Result of loadConfig() in lib/data-provider/index.ts; passed to each provider.
...
// Input to a human verdict on one anomaly, applied by provider.applyDecision().
...
// Lock record written while a script/agent holds the write lock.
