...
// lib/ sits directly under the skill root, so SKILL_DIR is one level up.
...
// Handoff state is authoritative — keep it in app/.data (byte-for-byte the same
// files the original store.ts read/wrote), not the regenerable app/.cache.
export const DATA_DIR = path.join(APP_DIR, ".data");
export const CACHE_DIR = path.join(APP_DIR, ".cache");
export const SNAPSHOT_PATH = path.join(DATA_DIR, "audit_snapshot.json");
