...
export async function loadState() {
  ...
  state.settings = data;
  applyDemoRoute();
  window.dispatchEvent(new CustomEvent("kelly-audit:state", { detail: data }));
  render();
}
...
function renderShell() {
  ...
  const orderCount = recordCountLabel("orders", snapshot?.orders?.length || 0);
  els.syncStatus.textContent =
    snapshot && orderCount
      ? `${orderCount} ${t("orders").toLowerCase()} · ${recordCountLabel("invoices", snapshot?.invoices?.length || 0)} ${t("invoices").toLowerCase()}`
      : t("empty");
