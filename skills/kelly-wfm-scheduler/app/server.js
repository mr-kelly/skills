...
const AIRAPP_ID = "kelly-wfm-scheduler";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-wfm-scheduler" }));
...
app.onError((error, context) => {
  console.error("Workforce Management & Shift Scheduler server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3195", 10);
