...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Digital Human must be Cloud-only");
if (
  /KELLY_DIGITAL_HUMAN_UI_PORT|KELLY_DIGITAL_HUMAN_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(
    configText,
  )
) {
...
console.log(`Kelly Digital Human AirApp checks OK (${files.length} files)`);
