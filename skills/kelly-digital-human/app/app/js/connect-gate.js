...
// local preview on loopback, before the Kelly Digital Human shell mounts. A deployed AirApp uses
...
// setup-gate.js used, so index.html and the Kelly Digital Human routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Digital Human will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Digital Human never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Kelly Digital Human shell mounts. Returns true once the app is clear to load
