...
`app/app/js/digital-human-model.js`. The retired `app/server/index.ts`'s
...
`app/.data/decisions.json` handoff bucket, keyed by check id. This Busabase
...
`app/app/js/digital-human-model.js`). There is no delete operation anywhere
in this skill.

### QA Decisions (`kelly-digital-human-qa-decisions-v1`)
...
`app/app/js/digital-human-model.js` exports plain constants, shared verbatim
...
- `METRICS_STATIC`: `target_latency_ms`, `current_latency_ms`,
  ...
  `status` field (replaces the retired `app/server/demo.ts`'s hardcoded `6`/
...
`app/app/js/digital-human-model.js`:
...
- `effectiveStatus(check, decision)`: a recorded decision always wins
  ...
  `needs_review`). Ported verbatim from the retired `app/app.js`.
...
`app/app/js/providers/demo-provider.js` returns `buildSnapshot(QA_CHECKS,
...)` with an empty decisions map and a fixed `generated_at` of
`2026-07-07T09:30:00.000Z`. Matching the retired `app/app.js`'s behavior, a
