...
const AIRAPP_ID = "kelly-prompt-regression";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-prompt-regression" }));
...
app.onError((error, context) => {
  console.error("AI Prompt Regression & Cost Optimizer server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33373", 10);
