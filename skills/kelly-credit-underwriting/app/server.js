...
const AIRAPP_ID = "kelly-credit-underwriting";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-credit-underwriting" }));
...
app.onError((error, context) => {
  console.error("Commercial Loan Underwriting Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33322", 10);
