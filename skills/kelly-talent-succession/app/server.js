...
const AIRAPP_ID = "kelly-talent-succession";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-talent-succession" }));
...
app.onError((error, context) => {
  console.error(
    "Executive Succession & High-Po Talent Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33342", 10);
