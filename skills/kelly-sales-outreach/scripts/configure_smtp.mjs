#!/usr/bin/env node
// Stores the seller's SMTP credentials in the Busabase Vault, so the
...
if (profile) {
  await client.records.changeRequest({
    ...
    message: "Record SMTP Vault reference names on the sales outreach profile",
    author: appConfig.appId,
    autoMerge: true,
  });
}

console.log("\n已写入 Vault。产品与 ICP Base 里只记了引用名，没有记任何值。");
