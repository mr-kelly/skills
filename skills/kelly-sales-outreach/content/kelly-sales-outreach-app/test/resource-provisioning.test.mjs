...
test("builds one Folder and every declared Base in a single declared structure change", () => {
  ...
  assert.equal(firstBase.fields.find((field) => field.slug === "offer-name")?.required, true);
  assert.equal(firstBase.fields.find((field) => field.slug === "offer-summary")?.required, true);
  assert.equal(firstBase.metadata.appId, "kelly-sales-outreach");
  assert.ok(
    operations
      .slice(1)
      .every(
...
test("repairs the declared AirApp inside an exact legacy Folder", () => {
  ...
  assert.deepEqual(resolved.repairs.at(-1), {
    nodeId: "nod_airapp",
    resourceKey: "kelly-sales-outreach-app",
    metadata: {
