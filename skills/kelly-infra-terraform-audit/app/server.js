...
const AIRAPP_ID = "kelly-infra-terraform-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-infra-terraform-audit" }));
...
app.onError((error, context) => {
  console.error(
    "Terraform Infrastructure Drift & Security Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33378", 10);
