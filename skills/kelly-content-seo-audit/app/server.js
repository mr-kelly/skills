...
const AIRAPP_ID = "kelly-content-seo-audit";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-content-seo-audit" }));
...
app.onError((error, context) => {
  console.error(
    "Enterprise Technical SEO & Content Gap Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33386", 10);
