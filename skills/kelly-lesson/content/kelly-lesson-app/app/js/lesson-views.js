...
export function renderPlanDetail() {
  ...
  // The initial value and the save target are both the plan's decision_note
  // (the same field the review queue's "Review note" textarea edits) — a
  // "revise" decision only resaves this note/draft and never changes status.
  const noteValue = state.edits[`note:${plan.plan_id}`] ?? plan.decision_note ?? "";
  els.title.textContent = `${t("planRef")} #${plan.ref} · ${plan.title}`;
  els.subtitle.textContent = `${plan.subject} · ${plan.grade} · ${teacher?.name || ""} · ${enumLabel(plan.status)}`;
  els.content.innerHTML = `
    ${noticeBanner()}
    ...
    <section class="detail">
      ...
      <aside class="detail-side">
        <div>
          <h2>${t("planDetail")}</h2>
          <dl>
            <dt>${t("status")}</dt><dd>${statusBadge(plan.status)}</dd>
        ...
        <div class="notes-panel">
          <h2>${t("editNotes")}</h2>
          <textarea id="planNote" rows="3" data-plan="${escapeHtml(plan.plan_id)}" placeholder="${escapeHtml(t("reviewNotePlaceholder"))}" ${demo || !review ? "disabled" : ""}>${escapeHtml(noteValue)}</textarea>
          <div class="notes-actions">
            <button id="saveNote" type="button" ${demo || !review ? "disabled" : ""}>${t("saveNote")}</button>
  ...
  els.content.querySelector("#saveNote")?.addEventListener("click", () => {
    if (!review) return;
    submitDecision(plan.plan_id, "revise", { comment: noteField?.value ?? "" });
  });
}
      ...
      ${
        items
          .map((item) => {
            ...
            const draft = edits.draft ?? plan?.feedback_draft ?? item.feedback_draft ?? "";
            const note = edits.note ?? plan?.decision_note ?? "";
            return `
          <article class="queue-card status-${escapeHtml(item.status)}" data-review="${escapeHtml(item.review_id)}">
            ...
            <div class="queue-actions">
              ...
              ${plan?.decision_action ? `<span class="queue-decision muted">${t("decision")}: ${escapeHtml(enumLabel(plan.decision_action, "action"))} · ${escapeHtml(plan.decided_at ? new Date(plan.decided_at).toLocaleString() : "")}</span>` : ""}
            </div>
          </article>
        `;
          })
...
async function submitDecision(planId, action, { comment = "", draft } = {}) {
  ...
  try {
    ...
    await provider.decidePlan({ plan_id: planId, action, note: comment, draft });
  } catch (error) {
    state.notice = error?.message || `Decision failed: ${action}`;
    render();
    return;
  }
  delete state.edits[planId];
