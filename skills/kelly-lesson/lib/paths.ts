...
// lib/ lives directly under the skill root. Keep every path byte-identical to
// app/server/paths.ts so the data-provider reads/writes the same handoff files
// the store used before this layer existed.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "lesson_snapshot.json");
