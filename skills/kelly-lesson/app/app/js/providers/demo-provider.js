...
// Teachers/plans/rules/template_sections and buildChecks() below are ported
// verbatim (same ids, same copy, same numbers, same rule-evidence overrides)
// from the retired app/server/demo.ts; buildDemoSnapshot() calls
// assembleSnapshot() from lesson-model.js exactly like the retired demo.ts
// assembled its snapshot inline, so demo and Busabase mode always share the
// same metrics/scoring math.
//
// The retired demo.ts kept a separate, smaller review_items[] (5 of the 12
// plans) plus a separate decisions.json fixture. In the Busabase-only shape
// a review item is just its plan's own fields, so REVIEW_OVERLAY/
// DECISION_OVERLAY below merge that same curated narrative content directly
// onto the matching plan before assembleSnapshot() runs.
import { assembleSnapshot } from "../lesson-model.js?v=0.1.0";
...
// Curated review-queue narrative for 5 of the 12 demo plans, ported verbatim
// from the retired app/server/demo.ts's buildReviewItems().
function reviewOverlay(zh) {
  const L = (en, zhText) => (zh ? zhText : en);
  return {
    "plan-math-quadratic": {
...
// Ported verbatim from the retired app/server/demo.ts's demoDecisions() —
// merged directly onto the matching plan since there is no separate
// decisions.json bucket in the Busabase-only shape.
function decisionOverlay(zh) {
  const L = (en, zhText) => (zh ? zhText : en);
  return {
    "plan-chinese-beiying": {
    ...
    "plan-eng-listening": {
      ...
      decision_note: L(
        "Approved. Publish to the library and send the feedback note.",
        "已批准。归入教案库并发送反馈。",
      ),
...
// Ported verbatim from the retired app/server/demo.ts's buildChecks(): a
// default evidence sentence per rule, with a per-plan override table for the
// scenarios that need a specific warn/fail story.
...
function buildDemoSnapshot(zh) {
  const teachers = demoTeachers(zh);
  const { rules, template_sections } = demoRules(zh);
  const overlay = reviewOverlay(zh);
  const decisions = decisionOverlay(zh);
  const plans = demoPlans(zh).map((plan) => ({
    ...plan,
    ...(overlay[plan.plan_id] || {}),
    ...(decisions[plan.plan_id] || {}),
  }));
  ...
  const configSummary = {
    ...
    subjects: zh ? ["数学", "语文", "英语", "物理"] : ["Math", "Chinese", "English", "Physics"],
    grades: zh ? ["七年级", "八年级", "九年级"] : ["Grade 7", "Grade 8", "Grade 9"],
    template_sections,
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const { snapshot, configSummary } = buildDemoSnapshot(zh);
    if (["review", "checks", "detail"].includes(scenario)) {
      snapshot.warnings = [
        {
    ...
    const visuals = demoVisualsForApp("kelly-lesson");
    return {
      demo: true,
