...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Lesson must be Cloud-only");
if (/KELLY_LESSON_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Lesson AirApp checks OK (${files.length} files)`);
