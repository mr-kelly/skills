...
function demoRules(zh) {
  ...
  return {
    ...
    rules: [
      { rule_id: "measurable_objectives", name: L("Objectives are measurable", "教学目标可测量"), severity: "error", type: "deterministic" },
      { rule_id: "stage_count_timing", name: L("3+ stages with time allocation", "教学环节≥3个且标注时间"), severity: "error", type: "deterministic" },
      { rule_id: "duration_sum", name: L("Stage timing sums to class length", "环节时间合计等于课时长度"), severity: "error", type: "deterministic" },
      { rule_id: "homework_assigned", name: L("Homework assigned", "布置了课后作业"), severity: "warning", type: "deterministic" },
      { rule_id: "template_sections", name: L("Uses school template sections", "使用学校模板栏目"), severity: "error", type: "deterministic" },
      { rule_id: "safety_note_lab", name: L("Safety note for lab lessons", "实验课包含安全提示"), severity: "error", type: "deterministic" },
      { rule_id: "curriculum_alignment", name: L("Objectives align with curriculum refs", "目标对应课标条目"), severity: "warning", type: "agent_review" }
    ]
  };
}
...
function demoSnapshot(scenario, zh) {
  ...
  const plans = [
    plan({
      ...
      sections: {
        objectives: [
          L("Solve one-variable linear equations of the form ax + b = c using the properties of equality.", "运用等式的基本性质，解形如 ax+b=c 的一元一次方程。"),
          L("Explain the justification for each transformation step when solving.", "能说出解方程每一步变形所依据的等式性质。"),
          L("Apply a linear equation to model and solve one real-world word problem.", "能运用一元一次方程解决一个实际问题。")
        ],
        key_points: [
        ...
        stages: [
          stage(L("Warm-up review", "复习导入"), 5, L("Balance puzzles recap the properties of equality.", "用天平小谜题回顾等式的基本性质。")),
          stage(L("Concept building", "新知建构"), 12, L("Derive the solving steps from the balance model.", "由天平模型归纳解一元一次方程的步骤。")),
          stage(L("Guided practice", "例题精讲"), 10, L("Solve three equations together, verbalizing each property used.", "师生共解三道例题，说出每步依据。")),
          stage(L("Independent practice", "巩固练习"), 12, L("Worksheet 3.1 tiered problems with pair checking.", "完成练习卷 3.1 分层训练，同伴互查。")),
          stage(L("Summary & assignment", "小结与布置"), 6, L("Students summarize the procedure; assign homework.", "学生归纳解题步骤，布置课后作业。"))
        ],
        board_plan: L(
        ...
        curriculum_refs: [L("Standards: Expressions & Equations 7.EE.4a", "《义务教育数学课程标准（2022）》方程与不等式 7.2.1")],
        safety_notes: ""
      }
    }),
    plan({
      ...
      sections: {
        ...
        stages: [
          ...
          stage(L("Plotting y = x²", "描点画 y=x²"), 15, L("Students plot points and connect the curve.", "学生列表、描点、连线。")),
          stage(L("Comparing coefficients", "比较系数"), 14, L("Explore y = 2x² and y = -x² with software.", "用画板探究 y=2x² 与 y=-x²。")),
          stage(L("Summary", "课堂小结"), 8, L("Summarize opening direction and symmetry.", "归纳开口方向与对称性。"))
        ],
        board_plan: L("Three parabolas on one grid with labels for vertex and axis.", "同一坐标系中三条抛物线，标注顶点与对称轴。"),
    ...
    plan({
      ...
      sections: {
        objectives: [
          L("State the SSS criterion and use it to prove two triangles congruent.", "能叙述 SSS 判定并用其证明两个三角形全等。"),
        ...
        stages: [
          stage(L("Hands-on exploration", "动手探究"), 12, L("Build triangles from fixed side lengths; compare results.", "用固定边长拼三角形并比较结果。")),
          stage(L("Criterion statement", "归纳判定"), 10, L("Formalize SSS from the exploration.", "由操作归纳出 SSS 判定。")),
          stage(L("Proof practice", "证明练习"), 17, L("Two guided proofs, one independent.", "两道引导证明，一道独立完成。")),
          stage(L("Summary & assignment", "小结与布置"), 6, L("Recap proof format; assign homework.", "回顾证明格式，布置作业。"))
        ],
        board_plan: L("Criterion statement, model proof with numbered steps, common mistakes corner.", "判定内容、编号书写的示范证明、易错角落。"),
        homework: L("Textbook p.12 problems 2, 3, 5; prove one pair of triangles from the classroom poster.", "课本第 12 页第 2、3、5 题；证明教室挂图中的一组全等三角形。"),
    ...
    plan({
      ...
      sections: {
        objectives: [
          L("Summarize the four appearances of the father's retreating figure in the text.", "梳理并概括文中四次写“背影”的内容。"),
        ...
        stages: [
          stage(L("Author and context", "作者与背景"), 8, L("Introduce Zhu Ziqing and the family situation.", "介绍朱自清及写作背景。")),
          stage(L("First reading", "初读感知"), 12, L("Silent reading; mark the four 'bei ying' moments.", "默读课文，圈画四次“背影”。")),
          stage(L("Close reading", "精读品析"), 0, L("Analyze the platform scene.", "品析月台买橘场景。")),
          stage(L("Discussion", "讨论交流"), 0, L("Share a family memory sparked by the text.", "结合课文交流自己的亲情记忆。"))
        ],
    ...
    plan({
      ...
      sections: {
        objectives: [
          ...
          L("Explain the meaning of key images such as 'zaohua' and 'ceng yun'.", "能解释“造化”“层云”等关键意象的含义。"),
        ...
        stages: [
          stage(L("Image-led opening", "情境导入"), 6, L("Mount Tai photos; students describe the view.", "出示泰山组图，学生描述所见。")),
          stage(L("Reading aloud", "朗读感知"), 12, L("Model reading, choral reading, rhythm marking.", "范读、齐读，划分节奏。")),
          stage(L("Line-by-line study", "逐联品读"), 15, L("Unpack images couplet by couplet.", "逐联解读意象与情感。")),
          stage(L("Recitation & summary", "背诵与小结"), 12, L("Memory scaffold, group recitation, summary.", "支架式背诵，小组展示，课堂小结。"))
        ],
        board_plan: L("Poem structure ladder: view → distance → detail → aspiration.", "板书阶梯：远望—近望—细望—抒怀。"),
        homework: L("Recite to family; write four lines imitating the 'gazing' structure.", "背诵给家人听；仿“望”的结构写四行小诗。"),
    ...
    plan({
      ...
      ref: 6,
      title: L("Unit 4: My Daily Routine — Lesson 2 (Speaking)", "七年级英语 Unit 4《My Daily Routine》第二课时（口语）"),
      subject: ENGLISH,
      grade: G7,
      ...
      sections: {
        objectives: [
          L("Use frequency adverbs (always, usually, never) to describe daily routines in pair talk.", "能在对话中运用频度副词（always、usually、never）描述日常作息。"),
          L("Ask and answer 'What time do you…?' questions with correct third-person forms.", "能正确使用第三人称形式进行 What time do you…? 问答。")
        ],
        ...
        stages: [
          ...
          stage(L("Presentation", "呈现新知"), 12, L("Clock cards drill time expressions.", "用钟面卡片操练时间表达。")),
          stage(L("Pair interview", "同伴采访"), 16, L("Interview partner; fill the routine grid.", "采访同伴并填写作息表格。")),
          stage(L("Report out", "汇报展示"), 10, L("Report partner's routine in third person.", "用第三人称汇报同伴作息。"))
        ],
        board_plan: L("Timeline with frequency adverb ladder and sentence frames.", "时间轴 + 频度副词阶梯 + 句型框架。"),
        homework: L("Record a 60-second audio describing your family member's routine.", "录一段 60 秒音频，描述家人的日常作息。"),
        reflection: "",
    ...
    plan({
      ...
      sections: {
        ...
        stages: [
          stage(L("Read the passage", "阅读课文"), 0, L("Students read and underline new words.", "学生阅读并划出生词。")),
    ...
    plan({
      ...
      sections: {
        objectives: [
          L("Describe buoyant force and identify its direction on submerged and floating objects.", "能描述浮力，并指出浸没和漂浮物体所受浮力的方向。"),
          L("Measure the buoyant force on a block with a spring scale using the weight-difference method.", "会用弹簧测力计通过称重法测量物块所受浮力。"),
          L("Predict and verify how buoyant force changes with submerged depth.", "能预测并通过实验验证浮力随浸没深度的变化。")
        ],
        ...
        stages: [
          stage(L("Demo hook", "演示激趣"), 6, L("Floating and sinking objects; students predict.", "浮沉演示，学生先猜想。")),
          stage(L("Concept building", "建立概念"), 12, L("Define buoyant force; weight-difference method.", "定义浮力，讲解称重法。")),
          stage(L("Lab activity", "分组实验"), 18, L("Groups measure buoyant force at three depths.", "小组测量三个深度下的浮力。")),
          stage(L("Data discussion & summary", "数据讨论与小结"), 9, L("Compare group data; summarize findings.", "汇总各组数据，归纳结论。"))
        ],
        board_plan: L(
        ...
        reflection: "",
        curriculum_refs: [L("Standards: Mechanics — force and motion 2.2.9", "《义务教育物理课程标准（2022）》运动和相互作用 2.2.9")],
        safety_notes: L(
    ...
    plan({
      ...
      sections: {
        ...
        stages: [
          stage(L("Circuit review", "电路复习"), 8, L("Recall series circuit reading rules.", "回顾串联电路读数规则。")),
          stage(L("Experiment setup", "实验准备"), 10, L("Groups wire the measurement circuit.", "小组连接测量电路。")),
          stage(L("Data collection", "采集数据"), 18, L("Vary voltage; record current three times.", "改变电压，三次记录电流。")),
          stage(L("Graphing & conclusion", "作图与结论"), 9, L("Plot I–U graph; state the law.", "绘制 I–U 图象，得出结论。"))
        ],
    ...
    plan({
      ...
      sections: {
        objectives: [
          L("Extract destination, date, and transport details from two listening passages.", "能从两段听力材料中提取目的地、日期和交通方式等信息。"),
          L("Use 'be going to' to describe a travel plan in a two-minute pair talk.", "能用 be going to 在两分钟对话中描述旅行计划。")
        ],
        ...
        stages: [
          stage(L("Lead-in", "导入"), 5, L("Holiday photos; brainstorm travel words.", "假期照片导入，头脑风暴旅行词汇。")),
          stage(L("Listening tasks", "听力任务"), 15, L("Two tiered listening passes with grids.", "两轮分层听力，完成信息表。")),
          stage(L("Language focus", "语言聚焦"), 10, L("'be going to' forms from the transcript.", "从听力文本归纳 be going to 用法。")),
          stage(L("Pair speaking", "结对口语"), 15, L("Plan a trip with cards; report to class.", "抽卡片设计行程并汇报。"))
        ],
        board_plan: L("Information grid, 'be going to' frames, useful travel phrases.", "信息表格、be going to 句型框架、旅行常用语。"),
    ...
    plan({
      ...
      sections: {
        ...
        stages: [
          ...
          stage(L("Sharing", "交流点评"), 6, L("Peer sharing with one-star-one-wish feedback.", "同伴互评：一颗星一个建议。"))
        ],
    ...
    plan({
      ...
      sections: {
        objectives: [
          L("Read and interpret bar, line, and pie charts from real data.", "能读取并解释条形图、折线图和扇形图中的真实数据。"),
          L("Choose an appropriate chart type for a given data set and justify the choice.", "能为给定数据选择合适的统计图并说明理由。")
        ],
        ...
        stages: [
          ...
          stage(L("Chart choice debate", "选图辩论"), 15, L("Teams justify chart choices for four data sets.", "小组为四组数据选图并辩护。")),
          stage(L("Summary & assignment", "小结与布置"), 12, L("Summarize criteria; assign chart-making task.", "归纳选图依据，布置绘图任务。"))
        ],
        board_plan: L("Three chart sketches with 'best for…' labels.", "三种统计图简图并标注适用场景。"),
        homework: L("Chart the class's one-week reading minutes; one paragraph of interpretation.", "把全班一周阅读时长绘制成统计图，并写一段解读。"),
        reflection: L("Debate format surfaced misconceptions about pie charts.", "辩论环节暴露了对扇形图的误解，效果好。"),
        curriculum_refs: [L("Standards: Statistics & Probability 8.3", "《义务教育数学课程标准（2022）》统计与概率 8.3")],
        safety_notes: ""
      }
    })
  ];
...
function buildChecks(plans, rules, zh) {
  ...
  const overrides = {
    ...
    "plan-chinese-beiying": {
      ...
      duration_sum: ["fail", L("Timed stages total 20 min of a 45 min class.", "已标时环节合计 20 分钟，课时为 45 分钟。")],
    ...
    "plan-eng-daily": {
      ...
      curriculum_alignment: ["agent_review", L("Awaiting agent judgement against Unit 4 standard descriptors.", "待智能体对照第四单元课标描述判断。")]
    },
    "plan-eng-reading": {
    ...
    "plan-phys-buoyancy": {
      curriculum_alignment: ["agent_review", L("Awaiting agent judgement against mechanics descriptor 2.2.9.", "待智能体对照课标 2.2.9 条目判断。")]
    },
    "plan-phys-ohm": {
      measurable_objectives: ["warn", L("'Understand Ohm's law' has no observable behavior.", "“理解欧姆定律”缺少可观察行为。")],
      safety_note_lab: ["fail", L("Lab lesson with beakers and circuits but no safety note.", "涉及电路实验，但没有任何安全提示。")]
    },
    "plan-chinese-writing": {
      measurable_objectives: ["warn", L("'Feel the charm' and 'improve ability' are not measurable.", "“感受魅力”“提高能力”不可测量。")],
      curriculum_alignment: ["warn", L("No curriculum standard refs listed.", "未填写课标依据。")]
    }
  };
  ...
  for (const plan of plans) {
    ...
    const defaults = {
      ...
      safety_note_lab: plan.sections.safety_notes
        ? L(`Safety note present: "${plan.sections.safety_notes.slice(0, 60)}"`, `安全提示：“${plan.sections.safety_notes.slice(0, 40)}”`)
        : L("Not a lab lesson; no safety note required.", "非实验课，无需安全提示。"),
      curriculum_alignment: plan.sections.curriculum_refs?.length
        ? L(`Refs: ${plan.sections.curriculum_refs.join("; ")}`, `课标依据：${plan.sections.curriculum_refs.join("；")}`)
        : L("No curriculum standard refs listed.", "未填写课标依据。")
    };
    for (const rule of rules) {
      const override = overrides[plan.plan_id]?.[rule.rule_id];
      const result = override ? override[0] : (rule.type === "agent_review" && plan.sections.curriculum_refs?.length ? "pass" : rule.type === "agent_review" ? "warn" : "pass");
...
function buildReviewItems(zh) {
  ...
  return [
    reviewItem({
      ...
      plan_id: "plan-math-quadratic",
      status: "needs_review",
      compliance_summary: L("Score 74 — 2 warnings: objectives not measurable, no curriculum refs.", "得分 74——2 项提醒：目标不可测量、缺少课标依据。"),
      suggestions: [
        L("Rewrite both objectives with observable verbs, e.g. 'sketch the graph of y = ax² and identify vertex and axis of symmetry'.", "将两条目标改为可观察行为动词，如“会画 y=ax² 的图象，并说出顶点和对称轴”。"),
    ...
    reviewItem({
      ...
      plan_id: "plan-chinese-beiying",
      status: "changes_requested",
      compliance_summary: L("Score 61 — 3 failures: 2 stages untimed, timing sums to 20/45, no homework.", "得分 61——3 项不合格：2 个环节未标时、时间合计 20/45、未布置作业。"),
      suggestions: [
    ...
    reviewItem({
      ...
      plan_id: "plan-phys-ohm",
      status: "needs_review",
      compliance_summary: L("Score 68 — 1 failure: lab lesson without a safety note; 1 warning on objectives.", "得分 68——1 项不合格：实验课缺少安全提示；1 项目标提醒。"),
      suggestions: [
        L("Add a safety note covering battery handling and not exceeding rated voltage.", "补充安全提示：电池使用规范、不得超过额定电压。"),
        L("Restate 'Understand Ohm's law' as an observable outcome, e.g. 'state and apply I = U/R to compute one quantity'.", "将“理解欧姆定律”改为可观察结果，如“能陈述并运用 I=U/R 计算其中一个量”。")
      ],
      feedback_draft: L(
    ...
    reviewItem({
      ...
      plan_id: "plan-eng-listening",
      status: "approved",
      compliance_summary: L("Score 93 — all checks pass; tiered listening tasks are a model example.", "得分 93——全部检查通过，分层听力任务可作范例。"),
      suggestions: [
        L("Consider sharing the tiered listening grid at the next English teaching group meeting.", "建议在下次英语教研组会议上分享分层听力表格设计。")
      ],
      feedback_draft: L(
    ...
    reviewItem({
      ...
      plan_id: "plan-eng-reading",
      status: "blocked",
      compliance_summary: L("Score 42 — 3 failures: not on school template, 2 untimed stages, no board plan or reflection.", "得分 42——3 项不合格：未使用学校模板、环节未标时、缺少板书设计与教学反思。"),
      suggestions: [
        L("Rewrite the plan on the school template with all required sections.", "按学校模板重写教案，补齐所有必填栏目。"),
...
function buildActivityLog(zh) {
  const L = (en, zhText) => (zh ? zhText : en);
  return [
    activity("act-1", "2026-07-02T08:20:00.000Z", "agent", L("Drafted 'Buoyancy — Lesson 1' from Chapter 10 materials and the school template.", "根据第十章材料和学校模板起草《浮力》第一课时教案。"), "plan-phys-buoyancy"),
    activity("act-2", "2026-07-01T17:05:00.000Z", "agent", L("Imported Henry Ma's Ohm's Law lab plan and ran compliance checks: 1 failure.", "导入马宏老师《欧姆定律》实验课教案并运行合规检查：1 项不合格。"), "plan-phys-ohm"),
    activity("act-3", "2026-07-01T16:20:00.000Z", "dean", L("Blocked 'Great Inventions' — must be rewritten on the school template.", "退回《Great Inventions》——需按学校模板重写。"), "plan-eng-reading"),
    activity("act-4", "2026-07-01T16:05:00.000Z", "dean", L("Approved 'Travel Plans'; feedback note queued for sending.", "批准《Travel Plans》，反馈已进入待发送队列。"), "plan-eng-listening"),
    activity("act-5", "2026-07-01T15:52:00.000Z", "dean", L("Requested changes on 'Bei Ying' Lesson 1: timing and homework.", "对《背影》第一课时提出修改：补时间分配和作业。"), "plan-chinese-beiying"),
    activity("act-6", "2026-07-01T14:40:00.000Z", "agent", L("Imported Brian Zhao's quadratic functions plan; checks found 2 warnings.", "导入赵斌老师二次函数教案，检查发现 2 项提醒。"), "plan-math-quadratic"),
    activity("act-7", "2026-06-30T11:20:00.000Z", "dean", L("Approved 'Viewing the Mountain' Lesson 1.", "批准《望岳》第一课时。"), "plan-chinese-poetry"),
    activity("act-8", "2026-06-28T10:05:00.000Z", "agent", L("Exported 2 approved Math plans to the library as Markdown.", "将 2 份已批准的数学教案导出为 Markdown 归入教案库。"), "plan-math-congruent")
  ];
}
