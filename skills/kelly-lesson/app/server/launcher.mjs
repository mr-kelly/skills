...
const explicitPort = process.env.KELLY_LESSON_UI_PORT || "";
    ...
    const req = http.get(stateUrlFor(port), { timeout: 500 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode >= 200 && res.statusCode < 300 && state.app === "kelly-lesson");
        } catch {
          resolve(false);
        }
      });
...
async function main() {
  ...
  if (await isReady(port)) {
    console.log(`Kelly Lesson UI already running: http://${host}:${port}`);
    return;
  }
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly Lesson UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly Lesson UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
