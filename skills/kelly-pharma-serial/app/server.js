...
const AIRAPP_ID = "kelly-pharma-serial";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-pharma-serial" }));
...
app.onError((error, context) => {
  console.error(
    "Pharmaceutical Track & Trace Serialization Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33303", 10);
