...
const AIRAPP_ID = "kelly-knowledge-ingest";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-knowledge-ingest" }));
...
app.onError((error, context) => {
  console.error("Enterprise Knowledge RAG Ingestion Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3212", 10);
