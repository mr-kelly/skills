...
const AIRAPP_ID = "kelly-patient-triage";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-patient-triage" }));
...
app.onError((error, context) => {
  console.error(
    "Patient Intake Triage & Care Pathway Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33307", 10);
