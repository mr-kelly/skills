...
const AIRAPP_ID = "kelly-k8s-cost-allocator";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-k8s-cost-allocator" }));
...
app.onError((error, context) => {
  console.error("Kubernetes Pod Cost & Right-Sizing Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33374", 10);
