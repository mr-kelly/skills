...
export async function readSnapshot(): Promise<InquirySnapshot> {
...
export function mergeInquiries(snapshot: InquirySnapshot, incoming: Inquiry[] = []) {
...
export function refreshInquiryDerived(inquiry: Inquiry) {
  const messages = Array.isArray(inquiry.messages) ? inquiry.messages : [];
...
export function applyMinPriceGuard(quote: Quote, products: Product[]) {
