...
// Ported verbatim from the retired app/server/demo.ts (same inquiry_id/quote_id/
// approval ref values, same copy, same persona "Lumina Lighting Co."). Derived
// fields (last_message_at/last_incoming_at, the stage heuristic, quote totals,
// the min-price guard, metrics) are no longer hardcoded/duplicated here — they
// come from the same ported model functions (refreshInquiryDerived/
// recomputeQuoteTotals/applyMinPriceGuard/recomputeMetrics) the busabase
// provider uses, computed the same way for both. The separate decisions.json
// bucket the retired local-file provider kept is gone: the human verdict lives
// directly on each approval item's `decision`, as it always did in the
// snapshot itself.
...
function demoSnapshot() {
  ...
  for (const item of inquiries) refreshInquiryDerived(item);
  const quotes = demoQuotes();
  for (const item of quotes) {
    recomputeQuoteTotals(item);
    applyMinPriceGuard(item, products);
  }
...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.approvals = snapshot.approvals.map((item) => ({
    ...
    decision: item.decision ? { ...item.decision, comment: comments[item.ref] ?? item.decision.comment } : null,
  }));
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-inquiry");
    return {
      demo: true,
