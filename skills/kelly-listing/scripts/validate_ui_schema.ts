#!/usr/bin/env node
// Validates app/.data/listing_snapshot.json (or a path passed as argv[2])
// against the schema in references/listing-schema.md. Run this before relying
// on a snapshot in the UI or executing decisions.
import fs from "node:fs/promises";

const target = process.argv[2] || new URL("../app/.data/listing_snapshot.json", import.meta.url).pathname;

const PLATFORMS = new Set(["amazon", "shopify", "tiktok_shop", "ebay"]);
