...
// lives in app/.data/*.json exactly as before, so /api/state in local mode is
// byte-identical to the pre-retrofit desk — plus the new `claims` block. The
// same review model is served remotely by the Busabase provider, so
// KELLY_LISTING_DATA_PROVIDER=local|busabase is a config switch, not a UI or
// script rewrite.
  ...
  const provider = {
    ...
    async saveClaim(payload: ClaimPayload) {
      if (await this.readLock()) {
        const error: HttpError = new Error("Listing files are locked while the agent is writing.");
