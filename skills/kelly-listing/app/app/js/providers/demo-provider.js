...
// demoProducts/demoDrafts/demoClaims/demoConfig are ported verbatim (same
// ids, same copy, same numbers) from the retired app/server/demo.ts's
// demoProducts()/demoDrafts()/demoClaims()/demoConfig(); checks are computed
// live by evaluateDraft() from the actual mock draft content (same as the
// retired demo.ts did through rules.ts), so the failing spice-rack draft
// really fails the banned-word / claims-registry checks.
import { assembleSnapshot, evaluateDraft, scoreChecks } from "../listing-model.js?v=0.1.0";

const NOW = "2026-07-03T08:30:00.000Z";
const FEATURED_DRAFT_ID = "d-lunchbox-amazon-us";
...
function demoDrafts(zh) {
  ...
  const raw = [
    ...
    },
    {
      ...
      suggestions: [],
      decision_action: "approve",
      decision_note: l("Approved after the handle-stitch claim was verified.", "核实提手缝线卖点后批准。"),
    ...
    },
    {
      ...
      suggestions: [],
      decision_action: "approve",
      decision_note: l("Approved. Export and hand off publishing.", "已批准。导出并交接发布。"),
  ...
  return raw.map((draft) => ({ ...draft, compliance_score: 0 }));
}
...
function buildDemoSnapshot(zh, scenario) {
  ...
  for (const draft of drafts) {
    ...
    for (const result of evaluateDraft(draft, product, config, zh ? "zh" : "en", claims)) {
      checks.push({
  ...
  const snapshot = assembleSnapshot({ products, drafts, checks, configSummary, lang: zh ? "zh" : "en", now: NOW });
    ...
    snapshot.warnings = [
      {
        ...
        detail: zh ? "演示提醒，未读取任何真实平台数据。" : "Demo warning; no live marketplace data was read.",
      },
    ];
  }
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-listing");
    return {
      demo: true,
...
export const FEATURED_DEMO_DRAFT_ID = FEATURED_DRAFT_ID;
