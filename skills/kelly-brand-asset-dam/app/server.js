...
const AIRAPP_ID = "kelly-brand-asset-dam";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-brand-asset-dam" }));
...
app.onError((error, context) => {
  console.error(
    "Digital Asset Management (DAM) Governance server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33382", 10);
