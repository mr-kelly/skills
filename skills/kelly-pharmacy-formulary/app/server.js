...
const AIRAPP_ID = "kelly-pharmacy-formulary";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-pharmacy-formulary" }));
...
app.onError((error, context) => {
  console.error(
    "Hospital Pharmacy Formulary & Safety Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33309", 10);
