...
const BROWSED_KEYS = ["sites", "queries", "pages", "opportunities", "geo-opportunities"];
...
export const busabaseProvider = {
  ...
  async getState() {
    ...
    const [sites, queries, pages, opportunities, geoOpportunities, entitySignals, aiVisibilityPrompts, settings] =
      await Promise.all([
        readPageRows("sites"),
        readPageRows("queries"),
        readPageRows("pages"),
        readPageRows("opportunities"),
        readPageRows("geo-opportunities"),
        readPageRows("entity-signals"),
        readPageRows("ai-visibility"),
        readSettingsRow(),
      ]);
  ...
  async decideGeoOpportunity({ geo_opportunity_id, action, note = "", draft } = {}) {
    ...
    await ensureResources();
    const existing = await findRecord("geo-opportunities", "geo-opportunity-id", geo_opportunity_id);
  ...
  async updateEntitySignal({ signal_id, status, note = "" } = {}) {
    ...
    await ensureResources();
    const existing = await findRecord("entity-signals", "signal-id", signal_id);
    ...
    await upsert("entity-signals", "signal-id", signal_id, nextFields, `Update entity signal ${signal_id}: ${status}`);
    return { ok: true };
  },
  ...
  async fetchPage(key, cursor) {
    ...
    const normalize = {
      sites: normalizeSiteRow,
      queries: normalizeQueryRow,
      pages: normalizePageRow,
      opportunities: normalizeOpportunityRow,
      "geo-opportunities": normalizeGeoOpportunityRow,
    }[key];
