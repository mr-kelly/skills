// Deterministic demo scenes for Kelly SEO. Never reads or writes Busabase.
// Persona: Featherlog, an indie SaaS for changelogs and release notes, with a
// marketing site and a docs subdomain, both verified in Google Search
// Console. Ported verbatim (same ids, same copy, same numbers) from the
// retired app/server/demo.ts; only the demo-specific fixture builders
// (queryEntry/pageEntry/dailySeries/trendSeries/seededRandom/dateRange)
// live here since they are demo-only, while the shared pure math
...
const GEO_ENGINES = ["chatgpt", "perplexity", "gemini", "claude", "copilot"];
...
function demoAiVisibilityPrompts() {
  const prompts = [
    {
...
function demoGeoOpportunities() {
  const raw = [
    ...
    },
    {
      ...
      execution: {
        ...
        target: "https://featherlog.app/blog/how-to-write-release-notes",
  ...
  return raw.map((entry) => {
    const gate = evaluateGeoGate({
      ...
      has_qa_block: entry.has_qa_block,
    });
    // A shipped change stays done; a geo-qa BLOCK is a hard gate otherwise —
    // matches busabase-provider.js's normalizeGeoOpportunityRow so the
    // effective status is identical regardless of provider.
    const executed = entry.execution?.status === "executed";
    const status = gate.verdict === "BLOCK" && !executed ? "blocked" : entry.status;
    return { ...entry, gate, status };
  });
}
...
function buildDemoSnapshot() {
  ...
  const geoOpportunities = demoGeoOpportunities();
  const entitySignals = demoEntitySignals();
  const prompts = demoAiVisibilityPrompts();
  ...
  return assembleSnapshot({
    ...
    settings: {
      ...
      range: RANGE,
      warnings: [],
      ai_visibility_prev_score: null, // computed below once the score is known
      ai_visibility_engines: GEO_ENGINES,
      prompts,
    },
    now: NOW,
  });
...
// The prev_score baseline is "score - 9" (never below 0), matching the
// retired demo.ts's demoAiVisibility(); computed after assembleSnapshot so
// it can read the just-computed score.
function withAiVisibilityPrevScore(snapshot) {
  if (!snapshot.ai_visibility) return snapshot;
  const prevScore = Math.max(0, snapshot.ai_visibility.score - 9);
  return { ...snapshot, ai_visibility: { ...snapshot.ai_visibility, prev_score: prevScore } };
}
...
function localizeGeoZh(snapshot) {
  ...
  const entityZh = {
    ...
    "ent-schema-org": { detail: "首页存在 Organization JSON-LD，含 name、url 和 logo。", fix: "" },
    "ent-sameas": {
...
function buildSnapshotForLang(zh) {
  const snapshot = withAiVisibilityPrevScore(buildDemoSnapshot());
  return zh ? localizeSnapshotZh(snapshot) : snapshot;
}
...
export const demoProvider = {
  ...
  async getState() {
    ...
    const visuals = demoVisualsForApp("kelly-seo");
    return {
      demo: true,
