...
// Path anchors for the kelly-seo data layer. This module lives in lib/, so the
// skill root is one directory up. The .data/*.json layout is byte-compatible
// with the original app/server/paths.ts — the local-file provider must read and
// write exactly the same files so switching to the provider layer is invisible.
...
export const SNAPSHOT_PATH = path.join(DATA_DIR, "seo_snapshot.json");
