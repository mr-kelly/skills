...
const AIRAPP_ID = "kelly-freight-tms";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-freight-tms" }));
...
app.onError((error, context) => {
  console.error(
    "Freight Dispatch & Customs Clearance Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33313", 10);
