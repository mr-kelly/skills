...
const AIRAPP_ID = "kelly-inventory-reorder";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-inventory-reorder" }));
...
app.onError((error, context) => {
  console.error("Supply Chain Inventory & Reorder Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3190", 10);
