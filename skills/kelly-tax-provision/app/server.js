...
const AIRAPP_ID = "kelly-tax-provision";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-tax-provision" }));
...
app.onError((error, context) => {
  console.error(
    "Corporate Income Tax Provision & VAT Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33323", 10);
