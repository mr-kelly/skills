...
const AIRAPP_ID = "kelly-db-privacy-masking";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-db-privacy-masking" }));
...
app.onError((error, context) => {
  console.error("Database Sensitive Data Masking Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33336", 10);
