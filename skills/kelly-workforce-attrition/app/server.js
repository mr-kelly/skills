...
const AIRAPP_ID = "kelly-workforce-attrition";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-workforce-attrition" }));
...
app.onError((error, context) => {
  console.error(
    "Employee Retention Risk & Exit Analytics server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33344", 10);
