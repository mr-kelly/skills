// The data-provider contract for kelly-family-fund + its runtime guard.
//
// kelly-family-fund is a READ-MOSTLY pooled-pension ledger dashboard: the agent
// (or an import script) writes one authoritative snapshot, and the UI reads it
// back with config, onboarding, and lock context. There is no review queue — so
// this interface mirrors that store surface, not a change-request model:
//
//   getState()          -> { onboarding, lock, config_summary, snapshot }  (for /api/state)
//   readSnapshot()      -> the built fund snapshot
...
import type { ConfigResult, ConfigSummary, FamilyFundState, FundSnapshot } from "../types.ts";
...
export interface DataProvider {
  ...
  getState(): Promise<FamilyFundState>;
  /** The built fund snapshot. */
  readSnapshot(): Promise<FundSnapshot>;
  /** Persist a snapshot (import_csv / generate_demo_snapshot scripts). */
  writeSnapshot(snapshot: FundSnapshot): Promise<{ ok: boolean; path?: string | null }>;
