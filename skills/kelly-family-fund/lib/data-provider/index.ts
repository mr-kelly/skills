...
//
// kelly-family-fund is a read-mostly pooled-pension ledger dashboard whose unit
// of state is one authoritative snapshot plus config/onboarding/lock context.
// This module lets the same UI and scripts run against either backend:
//
...
function configCandidates(): string[] {
  return [
    process.env.KELLY_FAMILY_FUND_CONFIG,
    path.join(skillDir, "config.local.json"),
    path.join(process.env.HOME || "", ".config", "kelly-family-fund", "config.json"),
...
export async function createProvider(): Promise<DataProvider> {
  ...
  throw new Error(`Unknown KELLY_FAMILY_FUND_DATA_PROVIDER: "${kind}" (expected "local" or "busabase")`);
}
