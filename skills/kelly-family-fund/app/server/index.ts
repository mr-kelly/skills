...
const port = Number.parseInt(process.env.KELLY_FAMILY_FUND_UI_PORT || process.env.PORT || String(DEFAULT_PORT), 10);

await ensureDirs();
await loadDotenvFiles(envSearchPaths());

serve({ fetch: app.fetch, hostname: host, port }, (info) => {
  console.log(`Kelly Family Fund UI: http://${host}:${info.port}`);
});
