...
const AIRAPP_ID = "kelly-slo-monitor";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-slo-monitor" }));
...
app.onError((error, context) => {
  console.error(
    "Service Level Objective & Error Budget Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3213", 10);
