...
import {
  ...
  normalizeIssueRow,
  statusForVerdict,
} from "../contracts-model.js?v=0.1.0";
import { assembleSnapshot } from "../contracts-model.js?v=0.1.0";
...
// workbench's "Save edits" button; it is merged onto the issue's current
// fields before re-encoding the array fields back to JSON.
function baseIssueFields(row, fieldsOverride) {
  const currentFields = normalizeIssueRow(row).fields;
...
export const busabaseProvider = {
  ...
  async getState() {
    ...
    const [contracts, issues, checks, claims, claimRules, settings] = await Promise.all([
      ...
      readAllRecords("claim_rules"),
      readSettingsRow(),
    ]);
    const normalizedContracts = contracts.map(normalizeContractRow);
    const normalizedIssues = issues.map(normalizeIssueRow);
    ...
    return {
      ...
      onboarding: { completed: normalizedIssues.length > 0, config_version: "1" },
      lock: null,
  ...
  // Ported from the retired local-file DataProvider's applyDecision(): every
  // action (including "revise") maps through statusForVerdict()'s table and
  // is recorded literally as decision_action — this simplifies away the
  // retired local-file provider's "revise preserves the prior real verdict"
  // special case, since Busabase reads are always live and there is no
  // staleness left to paper over. "revise" also carries edited fields from
  // the review workbench's field editor (title/bullets/description/...).
  async decideIssue({ issue_id, action, note = "", fields } = {}) {
    if (!issue_id || typeof issue_id !== "string") throw new Error("issue_id is required");
    if (!action || !DECISION_ACTIONS.has(action)) {
      throw new Error(`action must be one of: ${[...DECISION_ACTIONS].join(", ")}`);
    }
    await ensureResources();
    const existing = await findRecord("issues", "issue-id", issue_id);
    if (!existing) throw new Error(`Unknown issue: ${issue_id}`);
    const current = normalizeFields(existing.headCommit?.fields || existing.fields);
    const now = new Date().toISOString();
    const nextFields = {
      ...baseIssueFields(current, fields),
      issue_id,
