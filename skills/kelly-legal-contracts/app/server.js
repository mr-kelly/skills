...
const AIRAPP_ID = "kelly-legal-contracts";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-legal-contracts" }));
...
app.onError((error, context) => {
  console.error("Kelly Legal Contracts server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3148", 10);
