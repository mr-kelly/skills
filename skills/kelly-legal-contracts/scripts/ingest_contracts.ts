#!/usr/bin/env node
// Single write path for contract + issue payload JSON. Takes a payload file
// containing { "products": [...], "drafts": [...] } (or a single draft
// object), validates the structure per workstream shape, and merges it into
// app/.data/contract_snapshot.json. Refuses to write while agent.lock exists.
//
// Internal products[] are contracts and drafts[] are clause issues. Set
// product "source" to "manual" or "kelly_picks" (business handoff). Run
// scripts/run_checks.ts after ingesting to refresh risk checks and scores.
//
// Usage: node scripts/ingest_contracts.ts payload.json
...
if (!payloadPath) {
  console.error("Usage: node scripts/ingest_contracts.ts <payload.json>");
  process.exit(1);
}
...
for (const input of incomingDrafts) {
  ...
  if (!productId) {
    console.error(
      `drafts: cannot resolve contract for ${JSON.stringify(input.product_id || input.product)}; ingest the contract first.`,
    );
    process.exit(1);
  }
  ...
  snapshot.activity_log.unshift({
    ...
    at: now,
    actor: "agent",
    detail: `${mergedDrafts[mergedDrafts.length - 1].created ? "Ingested" : "Updated"} ${draft.platform} ${draft.locale} issue for ${snapshot.products.find((product) => product.product_id === productId)?.name || productId}.`,
    draft_id: draft.draft_id,
  });
}
...
for (const { draft, created } of mergedDrafts) {
  console.log(
    `${created ? "Created" : "Updated"} ${draft.draft_id} (Issue #${draft.ref}) — ${draft.platform} ${draft.locale}`,
  );
}
console.log(
  "Wrote contract review snapshot via the data provider. Run scripts/run_checks.ts to refresh compliance results.",
);
