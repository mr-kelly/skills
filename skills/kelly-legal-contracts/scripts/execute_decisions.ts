...
// - approve          -> "export_issue_list" (target: the path export_issues.ts
//                       will write) plus "handoff_redline" marked
//                       handoff_to_agent: external sending/redline delivery is
...
for (const item of snapshot.review_items || []) {
  ...
  if (decision.action === "approve") {
    ...
    pushResult(item, "export_issue_list", exportTarget, {
      detail: "Run scripts/export_issues.ts to write the Markdown issue memo and CSV row.",
    });
    pushResult(item, "handoff_redline", `${draft.platform}:${draft.locale || ""}`, {
      handoff_to_agent: true,
      detail: "External redline delivery or counsel handoff is executed by the agent outside the app, after approval.",
    });
...
for (const result of results) {
  console.log(
    `Issue #${result.ref} ${result.review_id}: ${result.status} ${result.operation}${result.target ? ` -> ${result.target}` : ""}`,
  );
}
...
console.log(
  "Wrote the execution report via the data provider. Exporting is done by scripts/export_issues.ts; external sends/redlines are delegated to the agent per SKILL.md.",
);
