// Data-provider selector for kelly-legal-contracts.
//
// kelly-legal-contracts is an App-in-Skill whose unit of work — a contract
// issue under review-before-handoff — maps onto Busabase's review model (record +
// change_request + operation + review + merge). This module lets the same UI
// and scripts run against either backend:
//
//   KELLY_LEGAL_CONTRACTS_DATA_PROVIDER=local     (default) JSON files in app/.data/
//   KELLY_LEGAL_CONTRACTS_DATA_PROVIDER=busabase  HTTP client to a Busabase base
//
...
function configCandidates() {
  return [
    process.env.KELLY_LEGAL_CONTRACTS_CONFIG,
    path.join(SKILL_DIR, "config.local.json"),
    path.join(process.env.HOME || "", ".config", "kelly-legal-contracts", "config.json"),
...
export function resolveProviderKind(config: Record<string, unknown> = {}) {
  return String(process.env.KELLY_LEGAL_CONTRACTS_DATA_PROVIDER || config.data_provider || "local").toLowerCase();
}
...
export async function createProvider(): Promise<DataProvider> {
  ...
  throw new Error(`Unknown KELLY_LEGAL_CONTRACTS_DATA_PROVIDER: "${kind}" (expected "local" or "busabase")`);
}
