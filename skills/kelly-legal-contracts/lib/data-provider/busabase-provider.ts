...
// not a backend. A contract issue's fields (title/risk notes/fallback text/...)
// live in a Busabase record's commit `fields`; an agent issue draft is a
// change_request, the human verdict is a review, and approval is a merge. The change-request
// status maps back onto kelly-legal-contracts's draft statuses so the UI is identical
// to local mode.
//
...
//   base_url      KELLY_LEGAL_CONTRACTS_BUSABASE_URL       e.g. http://127.0.0.1:3000
//   base_id       KELLY_LEGAL_CONTRACTS_BUSABASE_BASE_ID   the target Busabase base
...
export function createBusabaseProvider(meta: ProviderMeta = {}) {
  ...
  const baseUrl = (process.env.KELLY_LEGAL_CONTRACTS_BUSABASE_URL || busa.base_url || "").replace(/\/$/, "");
  const baseId = process.env.KELLY_LEGAL_CONTRACTS_BUSABASE_BASE_ID || busa.base_id || "";
  const apiKey = busa.api_key_env
    ? process.env[busa.api_key_env] || process.env.KELLY_LEGAL_CONTRACTS_BUSABASE_API_KEY || ""
    : process.env.KELLY_LEGAL_CONTRACTS_BUSABASE_API_KEY || "";

  function requireConfig() {
    if (!baseUrl || !baseId) {
      throw new Error(
        "Busabase provider needs base_url and base_id. Set config.busabase.{base_url,base_id} " +
          "or KELLY_LEGAL_CONTRACTS_BUSABASE_URL / KELLY_LEGAL_CONTRACTS_BUSABASE_BASE_ID.",
      );
    }
  }
      ...
      if (action === "approve") {
        if (edited && op) {
          await api("POST", `/api/v1/operations/${encodeURIComponent(op.id)}/revisions`, {
            payload: { fields: nextFields, message: "Edited before approval", author: "kelly-legal-contracts" },
          });
        }
        ...
        await api("POST", `/api/v1/operations/${encodeURIComponent(op.id)}/revisions`, {
          payload: { fields: nextFields, message: payload.comment || "Saved edits", author: "kelly-legal-contracts" },
        });
...
function notSupported(detail: string): HttpError {
  const error: HttpError = new Error(
    `Not supported in busabase mode: ${detail}. Use KELLY_LEGAL_CONTRACTS_DATA_PROVIDER=local for offline authoring.`,
  );
  error.statusCode = 400;
  return error;
}
