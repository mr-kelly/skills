...
const AIRAPP_ID = "kelly-construction-project";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-construction-project" }));
...
app.onError((error, context) => {
  console.error(
    "Commercial Construction Job Costing Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33299", 10);
