...
const followup = (
  ...
  status,
  suggested_reply,
  decision_comment: DECISIONS[followup_id]?.comment || "",
  decided_at: DECISIONS[followup_id]?.decided_at || "",
  decided_by: DECISIONS[followup_id] ? "operator" : "",
  created_at: NOW,
});
...
function localizeSnapshotZh(snapshot) {
  ...
  snapshot.followups = snapshot.followups.map((item) => ({
    ...item,
    reason: reasons[item.followup_id] || item.reason,
  }));
  snapshot.warnings = snapshot.warnings.map((item) => ({
    ...item,
...
export const demoProvider = {
  ...
  async getState() {
    ...
    return {
      ...
      demo_visuals: demoVisualsForApp("kelly-crm"),
      snapshot: { ...snapshot, demo_visuals: demoVisualsForApp("kelly-crm") },
    };
  },
