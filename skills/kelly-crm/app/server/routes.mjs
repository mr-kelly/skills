...
async function state() {
  ...
  return {
    app: "kelly-crm",
    data_provider: process.env.KELLY_CRM_DATA_PROVIDER || configResult.config.data_provider || "local",
    onboarding,
    lock,
...
async function handleDecision(req, res) {
  ...
  if (lock) {
    sendJson(res, 423, { error: "Agent lock is active; the queue is read-only right now.", lock });
    return;
  }
  let payload;
  try {
