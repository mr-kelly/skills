// Data-provider selector for kelly-crm.
//
// kelly-crm is an App-in-Skill whose unit of work — a follow-up under
// review-before-send — maps cleanly onto Busabase's review model
// (record + change_request + operation + commit + review + merge). This module
// lets the same UI and scripts run against either backend:
//
//   KELLY_CRM_DATA_PROVIDER=local     (default) JSON files in app/.data/
//   KELLY_CRM_DATA_PROVIDER=busabase  HTTP client to a Busabase base
//
...
export function resolveProviderKind(config: Config = {}) {
  return String(process.env.KELLY_CRM_DATA_PROVIDER || config.data_provider || "local").toLowerCase();
}
...
export async function createProvider(): Promise<DataProvider> {
  ...
  throw new Error(`Unknown KELLY_CRM_DATA_PROVIDER: "${kind}" (expected "local" or "busabase")`);
}
