...
const AIRAPP_ID = "kelly-medical-adverse-event";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-medical-adverse-event" }));
...
app.onError((error, context) => {
  console.error(
    "Medical Device & Drug Adverse Event Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33310", 10);
