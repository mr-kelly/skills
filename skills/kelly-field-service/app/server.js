...
const AIRAPP_ID = "kelly-field-service";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-field-service" }));
...
app.onError((error, context) => {
  console.error(
    "Field Service Technician Dispatch Console server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33366", 10);
