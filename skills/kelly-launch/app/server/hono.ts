...
// cloud provider (Busabase) — on other fetch-based runtimes.
//
...
app.get("/api/state", async (c) => {
  ...
  if (isDemoQuery(query)) {
    return c.json(demoStatePayload(query), 200, { "cache-control": "no-store" });
  }
  const state = await provider.getState();
  return c.json({ app: "kelly-launch", ...state }, 200, { "cache-control": "no-store" });
});
