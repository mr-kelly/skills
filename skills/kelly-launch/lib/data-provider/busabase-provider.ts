...
// layer, not a backend. A launch item's fields (phase, title, channel_id,
// readiness, proposed_action, status, draft, reason) live in a Busabase record's
// commit `fields`; an agent-prepared launch item is a change_request, the human
// verdict is a review, an edited draft is an operation revision, "shipping"
// (submit/pitch/publish) is a merge. The change-request status maps back onto
// kelly-launch's item workflow statuses so the UI is identical to local mode.
//
...
import { summarizeConfig } from "../common.ts";
import type { DecisionBody, ProviderMeta } from "../types.ts";

// Busabase change-request status -> kelly-launch item workflow status.
...
export function createBusabaseProvider(meta: ProviderMeta = {}) {
  ...
  const baseUrl = (process.env.KELLY_LAUNCH_BUSABASE_URL || busa.base_url || "").replace(/\/$/, "");
  const baseId = process.env.KELLY_LAUNCH_BUSABASE_BASE_ID || busa.base_id || "";
  const apiKey = busa.api_key_env
    ? process.env[busa.api_key_env] || process.env.KELLY_LAUNCH_BUSABASE_API_KEY || ""
  ...
  function crToItem(cr: Record<string, unknown>) {
    ...
    const draft = (fields.draft as string) || "";
    return {
      item_id: String(cr.id ?? ""),
      ref: typeof fields.ref === "number" ? fields.ref : 0,
      phase: (fields.phase as string) || "assemble",
      title: (fields.title as string) || "",
      channel_id: (fields.channel_id as string) || "",
      readiness: (fields.readiness as string) || "FIX",
  ...
  function countStatuses(items: Array<{ readiness: string; status: string }>) {
    let needsReview = 0;
    let approved = 0;
    let blocked = 0;
    let ship = 0;
    ...
    for (const item of items) {
      ...
      if (item.readiness === "SHIP") ship += 1;
      if (item.readiness === "FIX") fix += 1;
      if (item.readiness === "BLOCK") block += 1;
    }
    return { needs_review: needsReview, approved, blocked, ship, fix, block };
  }
  ...
  return {
    ...
    async getState() {
      ...
      try {
        ...
        const blockers = items
          .filter((item) => item.readiness === "BLOCK" || (item.readiness === "FIX" && item.status === "blocked"))
          .map((item) => ({ item_id: item.item_id, ref: item.ref, title: item.title, phase: item.phase }));
        const verdict = blockers.length ? "FIX" : counts.block ? "BLOCK" : "SHIP";
        const lqs = items.length ? Math.round(((counts.ship + counts.fix * 0.5) / items.length) * 100) : 0;
        return {
          ...
          snapshot: {
            ...
            source: "busabase",
            product: (summary.product as Record<string, unknown>) || {},
            launch: (summary.launch as Record<string, unknown>) || { target_date: "", timezone: "UTC" },
            phases: ["research", "assemble", "mobilize", "prove"],
            readiness: { verdict, lqs, ship: counts.ship, fix: counts.fix, block: counts.block, blockers },
            metrics: {
      ...
      if (action === "approve") {
        if (edited && op) {
          await api("POST", `/api/v1/operations/${encodeURIComponent(op.id as string)}/revisions`, {
            payload: { fields: nextFields, message: "Edited before approval", author: "kelly-launch" },
          });
        }
        ...
        await api("POST", `/api/v1/operations/${encodeURIComponent(op.id as string)}/revisions`, {
          payload: { fields: nextFields, message: payload.comment || "Saved edits", author: "kelly-launch" },
        });
      ...
      for (const item of (snapshot.items as Array<Record<string, unknown>>) || []) {
        const cr = await api("POST", `/api/v1/bases/${encodeURIComponent(baseId)}/change-requests`, {
          payload: {
            ...
            message: `Launch item ${item.title || item.item_id}`,
            submittedBy: "kelly-launch",
          },
        });
    ...
    async writeExecutionReport() {
      // "Shipping" an approved launch item is a merge in Busabase; there is no
