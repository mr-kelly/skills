...
const AIRAPP_ID = "kelly-fleet-telematics";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-fleet-telematics" }));
...
app.onError((error, context) => {
  console.error(
    "Fleet Telematics & Vehicle Maintenance Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33317", 10);
