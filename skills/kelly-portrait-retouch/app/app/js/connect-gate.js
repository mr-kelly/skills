...
// local preview on loopback, before the Kelly Portrait Retouch shell mounts. A deployed AirApp
// uses its ambient Busabase session and never reaches this module's fetch calls.
...
// setup-gate.js used, so index.html and the Kelly Portrait Retouch routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Portrait Retouch will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Portrait Retouch never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
  root.innerHTML = `<div class="ks-setup-overlay"><section class="ks-setup-panel" role="dialog" aria-modal="true" aria-labelledby="ksWorkspaceTitle"><div class="ks-setup-head"><div><h1 id="ksWorkspaceTitle">${title}</h1></div></div><div class="ks-setup-body">${body}<p class="ks-setup-error" data-workspace-status hidden></p></div><div class="ks-setup-footer ks-setup-footer-split"><a class="ks-text-link" href="?demo=queue#/queue">Open the read-only demo</a>${canProvision ? '<button class="ks-setup-primary" type="button" data-provision>Initialize workspace</button>' : retryOnly ? '<button class="ks-setup-primary" type="button" data-retry>Check again</button>' : ""}</div></section></div>`;
...
// Runs before the Kelly Portrait Retouch shell mounts. Returns true once the app is clear to
// load state from the provider (standalone local runtime is connected + Space
...
export function renderProductOnboarding(data, onRetry) {
  const root = overlayRoot();
  const readiness = data?.readiness || {};
  const settings = data?.snapshot?.settings || {};
  const waiting = readiness.action === "review_change_request";
  const cr = readiness.change_request_id || "pending";
  root.innerHTML = `<div class="ks-setup-overlay"><section class="ks-setup-panel" role="dialog" aria-modal="true" aria-labelledby="ksOnboardingTitle">
    <div class="ks-setup-head"><div><h1 id="ksOnboardingTitle">${waiting ? "Review the configuration request" : "Set portrait defaults"}</h1><p>Runtime resources are ready. Complete the separate product setup before portrait records are shown.</p></div></div>
    <div class="ks-setup-body">
      ${
        waiting
          ? `<p>The onboarding ChangeRequest <strong>${escapeHtml(cr)}</strong> is waiting for review. This app will not treat submitted settings as materialized.</p>`
