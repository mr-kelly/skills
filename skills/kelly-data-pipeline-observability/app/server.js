...
const AIRAPP_ID = "kelly-data-pipeline-observability";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-data-pipeline-observability" }));
...
app.onError((error, context) => {
  console.error(
    "Data Pipeline Lineage & Quality Audit Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33380", 10);
