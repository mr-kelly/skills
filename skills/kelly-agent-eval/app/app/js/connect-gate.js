...
// local preview on loopback, before the Tickets shell mounts. A deployed AirApp
...
// setup-gate.js used, so index.html and the Tickets routes need no change.
...
function renderWorkspaceScreen(error, onRetry) {
  ...
  const body = canProvision
    ? "<p>Kelly Agent Eval will create its Folder and Bases in the current Space.</p><p>Submitted as one idempotent Busabase ChangeRequest; nothing existing is deleted or repurposed.</p>"
    : `<p>${escapeHtml(reason)}</p><p>Kelly Agent Eval never asks you to create Nodes/Bases by hand or switches to local data.</p>`;
...
// Runs before the Tickets shell mounts. Returns true once the app is clear to
