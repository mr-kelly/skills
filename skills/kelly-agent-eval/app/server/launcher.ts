...
const explicitPort = process.env.KELLY_AGENT_EVAL_UI_PORT || "";
    ...
    const req = http.get(stateUrlFor(port), { timeout: 500 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          resolve(res.statusCode >= 200 && res.statusCode < 300 && state.app === "kelly-agent-eval");
        } catch {
          resolve(false);
        }
      });
...
async function main(): Promise<void> {
  ...
  if (await isReady(port)) {
    console.log(`Agent Eval & Regression Board already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.ts"], {
    ...
    env: { ...process.env, KELLY_AGENT_EVAL_UI_HOST: host, KELLY_AGENT_EVAL_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Agent Eval & Regression Board started: http://${host}:${port}`);
  ...
  console.error(`Agent Eval & Regression Board did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
