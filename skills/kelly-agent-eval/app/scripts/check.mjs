...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly Agent Eval must be Cloud-only");
if (/KELLY_AGENT_EVAL_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
...
console.log(`Kelly Agent Eval AirApp checks OK (${files.length} files)`);
