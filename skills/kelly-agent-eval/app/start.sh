...
if [ ! -d "$SKILL_DIR/node_modules/hono" ] || [ ! -d "$SKILL_DIR/node_modules/@hono/node-server" ]; then
  echo "Installing kelly-agent-eval server dependencies (first run)…"
  (cd "$SKILL_DIR" && npm install)
fi

if [ ! -f "$APP_DIR/.data/eval_run.json" ]; then
  echo "No eval run found — generating the mock suite…"
  (cd "$SKILL_DIR" && node scripts/generate_eval_run.ts)
fi
