...
const explicitPort = process.env.KELLY_MV_UI_PORT || "";
...
async function main() {
  ...
  if (await isReady(port)) {
    console.log(`Kelly MV UI already running: http://${host}:${port}`);
    return;
  }
  ...
  const child = spawn(process.execPath, ["index.mjs"], {
    ...
    env: { ...process.env, KELLY_MV_UI_HOST: host, KELLY_MV_UI_PORT: port },
  });
  child.unref();
  ...
  for (let i = 0; i < 40; i += 1) {
    if (await isReady(port)) {
      console.log(`Kelly MV UI started: http://${host}:${port}`);
  ...
  console.error(`Kelly MV UI did not become ready. See ${LOG_PATH}`);
  process.exitCode = 1;
}
