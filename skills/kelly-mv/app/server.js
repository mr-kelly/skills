...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-mv" }));
...
// Kelly MV-specific addition (not part of the shared template every other
...
app.onError((error, context) => {
  console.error("Kelly MV server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "3139", 10);
