...
const cli = [
  ...
  "--num_frames", String(args.frames),
  "--frame_rate", String(args.fps),
  "--pipeline_config", CONFIG,
  "--output_path", outDir,
  "--seed", String(args.seed ?? 171198),
];
...
child.on("close", (code) => {
  if (code !== 0) { console.error("inference.py exit", code); process.exit(code || 1); }
  const after = fs.readdirSync(outDir).filter((f) => f.endsWith(".mp4") && !before.has(f));
  if (!after.length) { console.error("no new mp4 produced in", outDir); process.exit(2); }
  // newest new mp4
