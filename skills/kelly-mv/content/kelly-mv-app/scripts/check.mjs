...
if (!configText.includes('deployment: "cloud"')) throw new Error("Kelly MV must be Cloud-only");
if (/KELLY_MV_DATA_PROVIDER|local-file-provider|config\.local\.json/.test(configText)) {
  throw new Error("Retired provider/runtime contract remains in app config");
}
if (/\b(?:href|src)="\/(?!api\/v1|api\/storage)/.test(index)) throw new Error("AirApp assets must use relative URLs");

const forbidden = ["local-file-provider.ts", "launcher.ts", "start.sh", "setup-gate.js", "setup-gate.css"];
...
console.log(`Kelly MV AirApp checks OK (${files.length} files)`);
