...
const AIRAPP_ID = "kelly-customer-cdp-segment";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-customer-cdp-segment" }));
...
app.onError((error, context) => {
  console.error("CDP Audience Segmentation & Sync Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33388", 10);
