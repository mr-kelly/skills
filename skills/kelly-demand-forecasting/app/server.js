...
const AIRAPP_ID = "kelly-demand-forecasting";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-demand-forecasting" }));
...
app.onError((error, context) => {
  console.error(
    "Supply Chain Demand & S&OP Planning Desk server error",
    error instanceof Error ? error.message : error,
  );
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33315", 10);
