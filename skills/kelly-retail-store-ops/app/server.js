...
const AIRAPP_ID = "kelly-retail-store-ops";
...
app.get("/health", (context) => context.json({ ok: true, app: "kelly-retail-store-ops" }));
...
app.onError((error, context) => {
  console.error("Retail Store Audit & Merchandising Desk server error", error instanceof Error ? error.message : error);
  return context.json({ error: "Internal server error" }, 500);
});

const port = Number.parseInt(process.env.PORT || "33291", 10);
