...
SKILL_ROOT = REPO_ROOT / "skills" / "kelly-family-fund"
APP_ROOT = SKILL_ROOT / "content" / "kelly-family-fund-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-family-fund"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.goto(f"{base_url}/?demo=1#/ledger")
    page.wait_for_load_state("networkidle")
    page.locator("#monthSelect").select_option("")
    assert page.locator("table tbody tr").count() == 40

    page.goto(f"{base_url}/?demo=1#/family")
    page.wait_for_load_state("networkidle")
    assert page.locator(".fairness-row").count() == 4

    page.goto(f"{base_url}/?demo=1#/family/fam-01")
    page.wait_for_load_state("networkidle")
    assert page.locator(".detail-side").is_visible()

    page.goto(f"{base_url}/?demo=1#/category")
    page.wait_for_load_state("networkidle")
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-family-fund-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-family-fund-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    try:
                        ...
                        page.get_by_text("No ledger entries yet").wait_for(timeout=15_000)
                    except Exception:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                families_base = find_resource(nodes, "families")
                expenses_base = find_resource(nodes, "expenses")
                assert families_base and expenses_base, nodes

                family_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{families_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "family-id": "fam-fixture",
                            "name": "Integration Fixture Family",
                            "head": "Tester",
                            "members-count": 2,
                        },
                        "message": "Seed Kelly Family Fund integration fixture family",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [family_cr["id"]]})

                expense_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{expenses_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "expense-id": "exp-fixture",
                            "month": "2026-01",
                            "date": "2026-01-15",
                            "category": "gift",
                            "amount": 300,
                            "payee": "Integration Fixture Reimbursement",
                            "occasion": "Test",
                            "family-id": "fam-fixture",
                            "shared": "false",
                        },
                        "message": "Seed Kelly Family Fund integration fixture expense",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [expense_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert page.get_by_text("Integration Fixture Reimbursement").is_visible()
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
            ...
            assert sorted(keys) == sorted(
                ["app-root", "beneficiaries", "families", "income", "expenses", "settings"]
            ), nodes
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            ...
            expenses_base = find_resource(nodes, "expenses")
            records = read_json(f"{busabase_url}/api/v1/records?baseId={expenses_base['baseId']}")
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
                (record.get("headCommit", {}).get("payload") or record.get("headCommit", {}).get("fields", {})).get("payee") == "Integration Fixture Reimbursement"
