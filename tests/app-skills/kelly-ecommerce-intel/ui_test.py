...
APP_ROOT = REPO_ROOT / "skills" / "kelly-ecommerce-intel" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-ecommerce-intel"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.wait_for_load_state("networkidle")
    assert "A competitor price move" in page.locator("#page-title").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-ecommerce-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-ecommerce-intel-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{signals_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": 1,
                            "title": "Fixture competitor price move",
                        ...
                        "message": "Seed Kelly Ecommerce Intel integration fixture signal",
