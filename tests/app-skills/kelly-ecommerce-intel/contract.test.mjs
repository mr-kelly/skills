...
const appRoot = join(repoRoot, "skills", "kelly-ecommerce-intel", "content", "kelly-ecommerce-intel-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-ecommerce-intel");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-ecommerce-intel-${base.key}`, base.key);
...
test("a trusted skill-root decision-execution script exists, dry-run gated by --apply", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-ecommerce-intel");
