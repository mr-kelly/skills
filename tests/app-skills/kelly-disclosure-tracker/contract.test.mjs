...
const skillRoot = join(repoRoot, "skills", "kelly-disclosure-tracker");
const appRoot = join(skillRoot, "content", "kelly-disclosure-tracker-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-disclosure-tracker");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-disclosure-tracker-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 3, "expected vehicles/items/settings");
  for (const base of appConfig.bases) {
    assert.ok(base.readLimit <= 100, `${base.key} readLimit ${base.readLimit} exceeds 100`);
  }
});

test("the fixed mock portfolio stays within the vehicles/items Bases' readLimits", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  const { buildSeedData } = await import(join(browserRoot, "js", "tracker-model.js"));
  const { vehicles, items } = buildSeedData({ now: "2026-01-01T00:00:00.000Z" });
  const vehiclesBase = appConfig.bases.find((base) => base.key === "vehicles");
  const itemsBase = appConfig.bases.find((base) => base.key === "items");
  assert.ok(vehicles.length <= vehiclesBase.readLimit, `${vehicles.length} exceeds readLimit`);
  assert.ok(items.length <= itemsBase.readLimit, `${items.length} exceeds readLimit`);
});
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_DISCLOSURE_TRACKER_DATA_PROVIDER|local-file-provider|config\.local\.json/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp itself never reads a local file", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/tracker-model.js"].map(
...
test("deterministic domain math: tracker-model never calls a real LLM/HTTP judge", async () => {
  const source = await readFile(join(browserRoot, "js", "tracker-model.js"), "utf8");
  assert.doesNotMatch(source, /fetch\(|openai|anthropic|XMLHttpRequest/i);
  assert.match(source, /computeItemStatus/);
  assert.match(source, /buildSeedData/);
});

test("this skill never files, submits, or transmits anything externally", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/tracker-model.js"].map(
      (path) => readFile(join(browserRoot, path), "utf8"),
    ),
  ...
  const source = sources.join("\n");
  assert.doesNotMatch(source, /\bfile(?:d|ing)?\s+(?:with|to)\s+(?:a\s+)?(?:regulator|exchange)/i);
});
...
test("ships both trusted scripts (generate_batch and execute_decisions)", async () => {
  ...
  assert.match(generateSource, /buildSeedData/);
  ...
  assert.doesNotMatch(executeSource, /\bfile(?:d|ing)?\s+(?:with|to)\s+(?:a\s+)?(?:regulator|exchange)/i);
