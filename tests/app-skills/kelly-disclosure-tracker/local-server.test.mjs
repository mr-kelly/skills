...
const appRoot = join(repoRoot, "skills", "kelly-disclosure-tracker", "content", "kelly-disclosure-tracker-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-disclosure-tracker-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-disclosure-tracker" });
