...
const appRoot = join(repoRoot, "skills", "kelly-insure-data", "content", "kelly-insure-data-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-insure-data-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-insure-data" });
