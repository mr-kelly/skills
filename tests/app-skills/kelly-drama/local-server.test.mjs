...
const appRoot = join(repoRoot, "skills", "kelly-drama", "content", "kelly-drama-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-drama-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-drama" });

  for (const path of ["/", "/styles/layers.css", "/app.js", "/js/config.js"]) {
...
test("proxies /api/storage/* (Busabase Asset upload/read relay) same as /api/v1/*, requiring a connection", async () => {
  // Kelly Drama's server.js adds this second relay beyond the shared template
  // (see server.js's header comment) because busabase-sdk's real `assets`
  // client returns root-relative URLs outside /api/v1/* for the actual
  // upload PUT target and file bytes. Without a Busabase connection it must
