...
const appRoot = join(skillRoot, "content", "kelly-drama-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-drama");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-drama-${base.key}`, base.key);
