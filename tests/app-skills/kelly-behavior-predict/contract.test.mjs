...
const appRoot = join(skillRoot, "content", "kelly-behavior-predict-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-behavior-predict");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-behavior-predict-${base.key}`, base.key);
