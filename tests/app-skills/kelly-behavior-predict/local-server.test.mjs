...
const appRoot = join(repoRoot, "skills", "kelly-behavior-predict", "content", "kelly-behavior-predict-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-behavior-predict-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-behavior-predict" });
