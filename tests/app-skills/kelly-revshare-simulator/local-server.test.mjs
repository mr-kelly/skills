...
const appRoot = join(repoRoot, "skills", "kelly-revshare-simulator", "content", "kelly-revshare-simulator-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-revshare-simulator-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-revshare-simulator" });
