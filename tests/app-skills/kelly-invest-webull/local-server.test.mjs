...
const appRoot = join(repoRoot, "skills", "kelly-invest-webull", "content", "kelly-invest-webull-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-invest-webull-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-invest-webull" });
