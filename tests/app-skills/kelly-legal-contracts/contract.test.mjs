...
const skillRoot = join(repoRoot, "skills", "kelly-legal-contracts");
const appRoot = join(skillRoot, "content", "kelly-legal-contracts-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-legal-contracts");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-legal-contracts-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 6, "expected contracts/issues/checks/claims/claim_rules/settings");
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LEGAL_CONTRACTS_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/contracts-model.js"].map(
...
test("has a live decision workflow: the review write path goes through records.changeRequest", async () => {
  ...
  assert.match(source, /decideIssue/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/ingest_contracts.ts",
    "scripts/run_checks.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_issues.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all four trusted scripts: ingest_contracts, run_checks, execute_decisions, export_issues", async () => {
  const ingestSource = await readFile(join(skillRoot, "scripts", "ingest_contracts.mjs"), "utf8");
  assert.match(ingestSource, /createBusabaseClient/);
  assert.match(ingestSource, /BUSABASE_BASE_URL/);
  assert.match(ingestSource, /--apply/);
  assert.match(ingestSource, /validateIssue/);
  ...
  assert.match(checksSource, /evaluateIssue/);
  ...
  assert.match(executeSource, /issueExecution/);

  const exportSource = await readFile(join(skillRoot, "scripts", "export_issues.mjs"), "utf8");
  assert.match(exportSource, /createBusabaseClient/);
  assert.match(exportSource, /BUSABASE_BASE_URL/);
  assert.match(exportSource, /issueMarkdown/);
  // Export only counts a genuine human "approve" decision as exportable —
  // never bare status, which an ingest payload could spoof.
