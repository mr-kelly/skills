...
APP_ROOT = REPO_ROOT / "skills" / "kelly-money" / "content" / "kelly-money-app"
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-money-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-money-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    try:
                        ...
                        page.get_by_text("No ledger snapshot yet").wait_for(timeout=15_000)
                    except Exception:
                ...
                transactions_base = find_resource(nodes, "transactions")
                assert accounts_base and transactions_base, nodes
                ...
                account_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{accounts_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "provider": "mercury",
                            "display-name": "Integration Fixture Account",
                            "currency": "USD",
                            "status": "ok",
                            "balance-current": 1000,
                        },
                        "message": "Seed Kelly Money integration fixture account",
                ...
                tx_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{transactions_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "transaction-id": "tx-fixture",
                            "account-id": "acct-fixture",
                            "description": "Integration Fixture Transaction",
                            "provider": "mercury",
                            "occurred-at": "2026-01-01T00:00:00.000Z",
                            "currency": "USD",
                            "gross": 500,
                            "net": 500,
                            "direction": "in",
                        ...
                        "message": "Seed Kelly Money integration fixture transaction",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [tx_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert page.get_by_text("Integration Fixture Transaction").is_visible()
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
            ...
            assert sorted(keys) == sorted(
                ["app-root", "accounts", "transactions", "invoices", "invoice-matches", "settings"]
            ), nodes
