...
const appRoot = join(repoRoot, "skills", "kelly-digital-human", "content", "kelly-digital-human-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-digital-human-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-digital-human" });
