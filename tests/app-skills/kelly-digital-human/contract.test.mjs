...
const appRoot = join(skillRoot, "content", "kelly-digital-human-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-digital-human");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-digital-human-${base.key}`, base.key);
