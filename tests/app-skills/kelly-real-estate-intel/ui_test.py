...
APP_ROOT = REPO_ROOT / "skills" / "kelly-real-estate-intel" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-real-estate-intel"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # throwaway `node --input-type=module -e` probe against
    # demoProvider.getState() before writing these assertions: 3+3+3 = 9
    # total needs_review, 0 approved, 0 blocked.
    ...
    page.wait_for_load_state("networkidle")
    assert "A nearby transaction gives agents a timely reason to message owners and buyers" in page.locator(
        "#page-title"
    ).inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-real-estate-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-real-estate-intel-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{signals_base['baseId']}/change-requests",
                    {
                        ...
                        "message": "Seed Kelly Real Estate Intel integration fixture signal",
