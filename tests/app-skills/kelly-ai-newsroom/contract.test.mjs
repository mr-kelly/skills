...
const appRoot = join(repoRoot, "skills", "kelly-ai-newsroom", "content", "kelly-ai-newsroom-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-ai-newsroom");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-ai-newsroom-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_AI_NEWSROOM_UI_PORT|KELLY_AI_NEWSROOM_DATA_PROVIDER|local-file-provider/);
...
test("a trusted skill-root decision-execution script exists, dry-run gated by --apply", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-ai-newsroom");
