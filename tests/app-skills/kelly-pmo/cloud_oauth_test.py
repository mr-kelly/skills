...
def run_cloud_oauth(config: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-pmo-cloud-home-") as app_home:
        env = {
            "HOME": app_home,
            "PORT": str(port),
            "BUSABASE_BASE_URL": "",
            "BUSABASE_API_KEY": "",
            # Space is selected only after OAuth from the server-reported list;
            # never preselect it in the app process.
            "BUSABASE_SPACE_ID": "",
        }
