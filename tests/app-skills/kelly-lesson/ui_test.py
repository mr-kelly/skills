...
APP_ROOT = REPO_ROOT / "skills" / "kelly-lesson" / "content" / "kelly-lesson-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-lesson"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (12 plans, 84 checks [12 plans x 7 rules], every plan
    # folded into review_items) is ported verbatim (same ids, same copy) from
    # the retired app/server/demo.ts — counts verified against
    ...
    # count-failed=7, count-revision=2 (changes_requested).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-review").inner_text() == "4"
    assert page.locator("#count-failed").inner_text() == "7"
    assert page.locator("#count-revision").inner_text() == "2"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=plans&lang=en#/plans")
    page.wait_for_load_state("networkidle")
    ...
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 84

    page.goto(f"{base_url}/?demo=review&lang=en#/review")
    page.wait_for_load_state("networkidle")
    assert page.locator(".queue-card").count() == 12
    assert "4" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert "Plan #1" in page.locator("#page-title").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-lesson-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-lesson-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                plans_base = find_resource(nodes, "plans")
                settings_base = find_resource(nodes, "settings")
                assert plans_base and plans_base.get("baseId"), nodes
                assert settings_base and settings_base.get("baseId"), nodes

                plan_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{plans_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "grade": "Grade 7",
                            "unit": "Unit 1",
                            "teacher-id": "t-fixture",
                            "source": "agent_draft",
                            "status": "needs_review",
                            "compliance-score": 80,
                            "class-length-minutes": 45,
                            "duration-minutes": 45,
                            "objectives": json.dumps(["Fixture objective"]),
                            "stages": json.dumps([{"name": "Intro", "minutes": 45, "activities": "Fixture"}]),
                            "compliance-summary": "Fixture summary",
                            "suggestions": json.dumps([]),
                            "feedback-draft": "Fixture draft",
                            "created-at": "2026-01-01T00:00:00.000Z",
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly Lesson integration fixture plan",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [plan_cr["id"]]})
                ...
                records = read_json(f"{busabase_url}/api/v1/records?baseId={plans_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("plan-id") == "fixture-plan"
                )
            ...
            assert sorted(keys) == sorted(["app-root", "teachers", "plans", "checks", "settings"]), nodes
