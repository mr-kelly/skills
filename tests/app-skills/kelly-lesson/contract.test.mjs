...
const appRoot = join(skillRoot, "content", "kelly-lesson-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-lesson");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-lesson-${base.key}`, base.key);
