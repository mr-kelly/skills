...
APP_ROOT = REPO_ROOT / "skills" / "kelly-messenger" / "content" / "kelly-messenger-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-messenger"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (5 accounts, 12 conversations, 65 messages, 6 outbox
    # replies) is ported verbatim from the retired app/server/demo.ts — counts
    # verified against app/app/js/providers/demo-provider.js with a throwaway
    # Node probe before writing these assertions: unread_count=6,
    # awaiting_reply_count=7 (== needsReply on the overview), outbox replies
    # by status: 2 needs_review, 1 approved, 1 blocked, 1 done, 1 changes_requested.
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-needs-reply").inner_text() == "7"
    assert page.locator("#count-approved").inner_text() == "1"
    assert page.locator("#count-blocked").inner_text() == "1"
    assert page.locator(".platform-row").count() == 5
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=inbox&lang=en#/inbox")
    page.wait_for_load_state("networkidle")
    assert page.locator(".conv-row").count() == 12
    assert "12" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=chat&lang=en#/inbox/wa-lena-pricing")
    page.wait_for_load_state("networkidle")
    assert page.locator("#page-title").inner_text() == "Lena Ortiz"
    composer_value = page.locator("#composer-text").input_value()
    assert "12 editor seats" in composer_value

    page.goto(f"{base_url}/?demo=outbox&lang=en#/outbox")
    page.wait_for_load_state("networkidle")
    assert page.locator(".outbox-card").count() == 6

    page.goto(f"{base_url}/?demo=accounts&lang=en#/accounts")
    page.wait_for_load_state("networkidle")
    assert page.locator(".account-card").count() == 5
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-messenger-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-messenger-home-") as app_home:
                ...
                conversations_base = find_resource(nodes, "conversations")
                replies_base = find_resource(nodes, "replies")
                assert accounts_base and accounts_base.get("baseId"), nodes
                assert conversations_base and conversations_base.get("baseId"), nodes
                assert replies_base and replies_base.get("baseId"), nodes

                account_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{accounts_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "account-id": "fixture-wa",
                            "platform": "whatsapp",
                            "connector": "manual",
                            "display-name": "Fixture WhatsApp",
                            "workspace": "Fixture Co",
                            "status": "ok",
                        },
                        "message": "Seed Kelly Messenger integration fixture account",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [account_cr["id"]]})

                conversation_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{conversations_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "conversation-id": "fixture-conv",
                            "account-id": "fixture-wa",
                            "platform": "whatsapp",
                            "kind": "dm",
                            "title": "Fixture Contact",
                            "provider-conversation-id": "15550001234@wa",
                            "unread": "true",
                            "awaiting-reply": "true",
                        },
                        "message": "Seed Kelly Messenger integration fixture conversation",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(
                    f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [conversation_cr["id"]]}
                )

                reply_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{replies_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "reply-id": "reply-fixture",
                            "conversation-id": "fixture-conv",
                            "account-id": "fixture-wa",
                            "platform": "whatsapp",
                            "conversation-title": "Fixture Contact",
                            "text": "Thanks, we'll get back to you shortly.",
                            "reason": "Integration fixture reply",
                            "suggested-by": "human",
                            "status": "needs_review",
                            "created-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly Messenger integration fixture reply",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [reply_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator("[data-reply-card='reply-fixture']")
                ...
                records = read_json(f"{busabase_url}/api/v1/records?baseId={replies_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("reply-id") == "reply-fixture"
                )
            ...
            assert sorted(keys) == sorted(
                ["app-root", "accounts", "conversations", "messages", "sync-log", "replies", "settings"]
            ), nodes
