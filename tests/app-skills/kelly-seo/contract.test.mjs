...
const skillRoot = join(repoRoot, "skills", "kelly-seo");
const appRoot = join(skillRoot, "content", "kelly-seo-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-seo");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-seo-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  ...
  assert.ok(
    ...
    "expected sites/queries/pages/opportunities/geo_opportunities/ai_visibility/entity_signals/settings",
  );
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_SEO_DATA_PROVIDER|local-file-provider|config\.local\.json/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp itself never reads a local file or performs the real GSC sync/execution", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/seo-model.js"].map((path) =>
      readFile(join(browserRoot, path), "utf8"),
    ),
  );
  const source = sources.join("\n");
  ...
  assert.doesNotMatch(source, /googleapis\.com/);
});

test("has a live decision workflow: opportunity/GEO/entity review writes go through records.changeRequest", async () => {
  ...
  assert.match(source, /decideOpportunity/);
  assert.match(source, /decideGeoOpportunity/);
  assert.match(source, /updateEntitySignal/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.equal(appConfig.readOnly, false);
  assert.ok(appConfig.permissions.writeProcedures.includes("records.changeRequest"));
});

test("geo-qa is a hard gate: approving a BLOCKed GEO opportunity is rejected before any write", async () => {
  const source = await readFile(join(browserRoot, "js", "providers", "busabase-provider.js"), "utf8");
  assert.match(source, /geo-qa BLOCK/);
  assert.match(source, /evaluateGeoGate/);
});
...
test("ships both trusted scripts: sync_gsc and execute_decisions", async () => {
  const syncSource = await readFile(join(skillRoot, "scripts", "sync_gsc.mjs"), "utf8");
  assert.match(syncSource, /createBusabaseClient/);
  assert.match(syncSource, /BUSABASE_BASE_URL/);
  assert.match(syncSource, /googleapis\.com/);
  assert.match(syncSource, /serviceAccountToken|KELLY_SEO_GSC_SERVICE_ACCOUNT_FILE/);
  ...
  assert.match(executeSource, /operationForOpportunity/);
