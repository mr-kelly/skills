...
APP_ROOT = REPO_ROOT / "skills" / "kelly-seo" / "content" / "kelly-seo-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-seo"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (Featherlog persona: 2 sites, 30 queries, 15 pages, 6
    # SEO opportunities, 6 GEO opportunities, 5 tracked AI-visibility
    # prompts, 6 entity-readiness signals) is ported verbatim (same ids, same
    # copy, same numbers) from the retired app/server/demo.ts — counts
    # verified against app/app/js/providers/demo-provider.js with a
    # throwaway `node -e` probe before writing these assertions:
    # count-review=2 (needs_review opportunities), count-approved=1,
    # count-blocked=1; AI-visibility score=68 (prev 59); entity score=50
    # (4/6 signals fully/partially present); one GEO opportunity
    # (geo-opp-quotable-stats-headway) is BLOCKed by geo-qa for a fabricated
    # migration stat.
    ...
    page.goto(f"{base_url}/?demo=queries&lang=en#/queries")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 30

    page.goto(f"{base_url}/?demo=pages&lang=en#/pages")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 15

    page.goto(f"{base_url}/?demo=opportunities&lang=en#/opportunities")
    page.wait_for_load_state("networkidle")
    assert page.locator(".opp-card").count() == 6
    assert "2" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=geo&lang=en#/geo")
    page.wait_for_load_state("networkidle")
    assert page.locator(".geo-matrix tbody tr").count() == 5

    page.goto(f"{base_url}/?demo=optimize&lang=en#/optimize")
    page.wait_for_load_state("networkidle")
    assert page.locator(".geo-card").count() == 6
    assert page.locator(".gate-panel-BLOCK").count() == 1

    page.goto(f"{base_url}/?demo=entity&lang=en#/entity")
    page.wait_for_load_state("networkidle")
    assert page.locator(".entity-card").count() == 6

    page.goto(f"{base_url}/?demo=overview&lang=en#/sites")
    page.wait_for_load_state("networkidle")
    assert page.locator(".account-card").count() == 2

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert page.locator("#page-title").inner_text() == "release notes vs changelog"
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-seo-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-seo-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/opportunities")
                    page.wait_for_load_state("networkidle")
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                opportunities_base = find_resource(nodes, "opportunities")
                settings_base = find_resource(nodes, "settings")
                assert opportunities_base and opportunities_base.get("baseId"), nodes
                assert settings_base and settings_base.get("baseId"), nodes

                opp_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{opportunities_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "opportunity-id": "fixture-opp",
                            "ref": 1,
                            "site-id": "fixture-site",
                            "type": "fix_page_issue",
                            "title": "Fixture opportunity",
                            "target-page": "https://example.com/fixture",
                            "target-query": "",
                            "reason": "Fixture reason",
                            "expected-impact": "Fixture expected impact",
                            "draft": "Fixture draft",
                            "status": "needs_review",
                            "agent-notes": "",
                            "created-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly SEO integration fixture opportunity",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [opp_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    page.locator("button[data-decision='approve']").click()
                    page.wait_for_timeout(800)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={opportunities_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("opportunity-id") == "fixture-opp"
                )
