...
APP_ROOT = REPO_ROOT / "skills" / "kelly-ads" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-ads"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (persona "Nimbus Home": 4 platforms, 10 campaigns, 7
    # anomalies, 6 adjustment cards) is ported verbatim from the retired
    # app/server/demo.ts — counts verified against
    # app/app/js/providers/demo-provider.js with a throwaway Node probe
    # (stubbing window/navigator) before writing these assertions:
    # count-review=2, count-alerts=4 (open anomalies), count-budget=2
    # (campaigns >=85% of daily budget spent today), 10 campaign rows, 7
    # anomaly rows, 6 adjustment cards (2 needing review).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-review").inner_text() == "2"
    assert page.locator("#count-alerts").inner_text() == "4"
    assert page.locator("#count-budget").inner_text() == "2"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=campaigns&lang=en#/campaigns")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 10
    assert "10" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=alerts&lang=en#/alerts")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 7

    page.goto(f"{base_url}/?demo=adjustments&lang=en#/adjustments")
    page.wait_for_load_state("networkidle")
    assert page.locator(".action-card").count() == 6
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-ads-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-ads-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/adjustments")
                    page.wait_for_load_state("networkidle")
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                adjustments_base = find_resource(nodes, "adjustments")
                campaigns_base = find_resource(nodes, "campaigns")
                assert adjustments_base and adjustments_base.get("baseId"), nodes
                assert campaigns_base and campaigns_base.get("baseId"), nodes

                adjustment_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{adjustments_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "adjustment-id": "fixture-neg-keyword",
                            "ref": 1,
                            "type": "negative_keyword",
                            "title": "Add fixture term as a negative keyword",
                            "status": "needs_review",
                            "campaign-id": "fixture-campaign",
                            "platform": "amazon",
                            "reason": "Integration fixture reason",
                            "evidence": json.dumps(["fixture evidence"]),
                            "target": json.dumps({"kind": "term", "id": "fixture-term", "text": "fixture term"}),
                            "current-value": "broad match, enabled",
                            "proposed-value": "negative exact",
                            "created-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly Ads integration fixture adjustment",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [adjustment_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/adjustments/fixture-neg-keyword")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    page.locator("[data-verdict='approve']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={adjustments_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if r.get("headCommit", {}).get("fields", {}).get("adjustment-id") == "fixture-neg-keyword"
                )
                assert fixture["headCommit"]["fields"]["status"] == "approved", fixture
                assert fixture["headCommit"]["fields"]["decision-verdict"] == "approve", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(
                ["app-root", "platforms", "campaigns", "anomalies", "adjustments", "sync_log", "settings"]
            ), nodes
