...
APP_ROOT = REPO_ROOT / "skills" / "kelly-radar" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-radar"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (5 watchlist targets, 10 signals, 4 research
    # questions, 4 briefs, 2 reports, 8 trend movers, 2 opportunities) is
    # ported verbatim from the retired app/server/demo.ts — counts verified
    # against app/app/js/providers/demo-provider.js with a throwaway grep
    # probe before writing these assertions: count-triage=5 (signals
    # needs_review), count-briefs=1 (briefs needs_review), count-reports=1
    # (questions report_ready).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-triage").inner_text() == "5"
    assert page.locator("#count-briefs").inner_text() == "1"
    assert page.locator("#count-reports").inner_text() == "1"
    ...
    page.goto(f"{base_url}/?demo=overview&lang=en#/watchlist")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 5

    page.goto(f"{base_url}/?demo=research&lang=en#/research")
    page.wait_for_load_state("networkidle")
    ...
    page.goto(f"{base_url}/?demo=detail&lang=en#/signals/sig-formora-pricing")
    page.wait_for_load_state("networkidle")
    assert page.locator("#page-title").inner_text() == "Formora raised Pro from $12 to $15/month"
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-radar-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-radar-home-") as app_home:
                ...
                # and a human triage verdict must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    page.locator("button[data-action='approve'][data-kind='signal']").click()
                ...
                fixture = next(
                    ...
                    if r.get("headCommit", {}).get("fields", {}).get("signal-id") == "fixture-signal"
                )
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            assert len(resource_keys(nodes)) == 10, nodes
            signals_base = find_resource(nodes, "signals")
            records = read_json(f"{busabase_url}/api/v1/records?baseId={signals_base['baseId']}")
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
