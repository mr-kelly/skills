...
const appRoot = join(repoRoot, "skills", "kelly-homework-coach", "content", "kelly-homework-coach-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-homework-coach-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-homework-coach" });
