...
const skillRoot = join(repoRoot, "skills", "kelly-homework-coach");
const appRoot = join(skillRoot, "content", "kelly-homework-coach-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-homework-coach");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-homework-coach-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 5, "expected questions/mistakes/papers/reviews/settings");
  for (const base of appConfig.bases) {
    assert.ok(base.readLimit <= 100, `${base.key} readLimit ${base.readLimit} exceeds 100`);
  }
});

test("the demo dataset stays within each Base's readLimit", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  const { demoSnapshot } = await import(join(browserRoot, "js", "homework-model.js"));
  const snapshot = demoSnapshot("en");
  const byKey = Object.fromEntries(appConfig.bases.map((base) => [base.key, base]));
  assert.ok(snapshot.questions.length <= byKey.questions.readLimit);
  assert.ok(snapshot.mistakes.length <= byKey.mistakes.readLimit);
  assert.ok(snapshot.papers.length <= byKey.papers.readLimit);
  assert.ok(snapshot.review_items.length <= byKey.reviews.readLimit);
});
...
test("the AirApp itself never reads a local file", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/homework-model.js"].map(
...
test("deterministic domain math: homework-model never calls a real LLM/HTTP judge", async () => {
  const source = await readFile(join(browserRoot, "js", "homework-model.js"), "utf8");
  assert.doesNotMatch(source, /fetch\(|openai|anthropic|XMLHttpRequest/i);
  assert.match(source, /computeMetrics/);
  assert.match(source, /demoSnapshot/);
});

test("this skill never uploads a photo or exports a paper from the browser", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/homework-model.js"].map(
      (path) => readFile(join(browserRoot, path), "utf8"),
    ),
...
test("wraps the review-decision execution step and the homework-recording step in trusted .mjs scripts", async () => {
  ...
  const recordSource = await readFile(join(skillRoot, "scripts", "record_homework.mjs"), "utf8");
  assert.match(recordSource, /createBusabaseClient/);
  assert.match(recordSource, /BUSABASE_BASE_URL/);
  assert.match(recordSource, /--apply/);
  // Never lets an ingestion payload set a review's decision fields.
