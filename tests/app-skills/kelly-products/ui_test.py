...
APP_ROOT = REPO_ROOT / "skills" / "kelly-products" / "content" / "kelly-products-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-products"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (5 products, 7 channel rows, 5 inventory rows, 3
    # review items) is ported verbatim (same ids, same copy, same numbers)
    # from the retired app/server/demo.ts. Counts verified against
    # app/app/js/providers/demo-provider.js with a throwaway Node probe
    # before writing these assertions: count-review=3 (all needs_review),
    # count-stock=2 (low_stock/stockout_risk), count-channel=6 (non-empty
    # issue notes -- only the live Amazon lunchbox listing has none).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-review").inner_text() == "3"
    assert page.locator("#count-stock").inner_text() == "2"
    assert page.locator("#count-channel").inner_text() == "6"
    ...
    page.goto(f"{base_url}/?demo=inventory&lang=en#/inventory")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 5

    page.goto(f"{base_url}/?demo=channels&lang=en#/channels")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 7

    page.goto(f"{base_url}/?demo=review&lang=en#/review")
    page.wait_for_load_state("networkidle")
    assert page.locator(".review-item").count() == 3
    assert "3" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert "Aurora Gradient Desk Lamp" in page.locator("#page-title").inner_text()

    page.goto(f"{base_url}/?demo=overview&lang=zh#/overview")
    page.wait_for_load_state("networkidle")
    assert "极光渐变桌面灯" in page.content()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-products-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-products-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                review_base = find_resource(nodes, "review")
                products_base = find_resource(nodes, "products")
                settings_base = find_resource(nodes, "settings")
                assert review_base and review_base.get("baseId"), nodes
                assert products_base and products_base.get("baseId"), nodes
                assert settings_base and settings_base.get("baseId"), nodes

                # Review items enter Busabase only through the agent's ingest
                # workflow (scripts/ingest_products.mjs), never the browser --
                # seed one fixture row directly via the Busabase HTTP API,
                # mirroring kelly-legal-contracts' ui_test.py precedent for
                # the same decision-only shape.
                review_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{review_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "item-id": "fixture-review",
                            "ref": 1,
                            "product-id": "prod-fixture",
                            "type": "publish_approval",
                            "status": "needs_review",
                            "title": "Fixture publish approval",
                            "summary": "Fixture review item summary",
                            "risk": "medium",
                        ...
                        "message": "Seed Kelly Products integration fixture review item",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [review_cr["id"]]})

                # A fresh app process must discover the existing resources
                # and records, then a human verdict must write straight to
                # Busabase (records.changeRequest directly on the review
                # item's own record -- no separate decisions bucket).
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.once("dialog", lambda dialog: dialog.accept("Approved via integration test"))
                    page.locator("[data-action='approve'][data-item='fixture-review']").click()
                    page.wait_for_timeout(1000)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={review_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("item-id") == "fixture-review"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["status"] == "approved", fixture
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["decision-note"] == "Approved via integration test", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(
                ["app-root", "products", "channels", "inventory", "review", "settings"]
            ), nodes
