...
const appRoot = join(skillRoot, "content", "kelly-products-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-products");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-products-${base.key}`, base.key);
