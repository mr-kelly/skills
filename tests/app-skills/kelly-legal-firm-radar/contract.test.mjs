...
const skillRoot = join(repoRoot, "skills", "kelly-legal-firm-radar");
const appRoot = join(skillRoot, "content", "kelly-legal-firm-radar-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-legal-firm-radar");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-legal-firm-radar-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LEGAL_FIRM_RADAR_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/firm-radar-model.js"].map(
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/import_metrics.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_management_report.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all three trusted scripts: import_metrics, execute_decisions, export_management_report", async () => {
  const importSource = await readFile(join(skillRoot, "scripts", "import_metrics.mjs"), "utf8");
  ...
  const exportSource = await readFile(join(skillRoot, "scripts", "export_management_report.mjs"), "utf8");
