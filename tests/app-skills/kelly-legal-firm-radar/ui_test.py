...
APP_ROOT = REPO_ROOT / "skills" / "kelly-legal-firm-radar" / "content" / "kelly-legal-firm-radar-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-legal-firm-radar"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.wait_for_load_state("networkidle")
    assert "商业租赁争议增长与团队配置建议" in page.locator(".detail h2").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-legal-firm-radar-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-legal-firm-radar-home-") as app_home:
                ...
                item_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{items_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": "Insight #1",
                            "title": "Fixture practice mix insight",
                            "category": "Fixture",
                            "status": "needs_review",
                        ...
                        "message": "Seed Legal Firm Radar integration fixture item",
