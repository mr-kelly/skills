...
const appRoot = join(repoRoot, "skills", "kelly-llm-gateway", "content", "kelly-llm-gateway-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-llm-gateway");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-llm-gateway-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LLM_GATEWAY_DATA_PROVIDER|local-file-provider/);
  assert.match(source, /createBusabaseClient/);
});

test("this is a direct-manipulation dashboard: the browser is allowed to write records directly", async () => {
  // Unlike a read-only intel/reporting skill (e.g. kelly-money, which only
  // ever calls records.list), Kelly LLM Gateway's rollout promote/rollback/
  // hold decisions and anomaly acknowledgements are applied straight from
  // the browser through records.changeRequest — there is no separate
  // review/approval queue and no trusted execute_decisions script, since
  // SKILL.md confirms the app itself never calls a live gateway or changes
  // a real routing config.
  ...
  assert.match(providerSource, /async decideRollout\(/);
  assert.match(providerSource, /async ackAnomaly\(/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-llm-gateway");
  for (const path of [
    "lib",
    "config.example.json",
    "app/setup-gate.js",
...
test("no skill-root trusted script: scripts/seed_snapshot.ts was a demo/dev fixture generator, folded into demo-provider.js", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-llm-gateway");
  await assert.rejects(readFile(join(skillRoot, "scripts", "seed_snapshot.mjs")));
  // The skill-root package.json that does exist is scoped to publishing the
  // AirApp, not to seeding data from outside it.
  ...
  assert.match(demoSource, /ROUTE_SEEDS/);
});
