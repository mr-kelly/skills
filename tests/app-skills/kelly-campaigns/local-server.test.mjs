...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-campaigns-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-campaigns" });

  for (const path of ["/", "/styles/shell.css", "/app.js", "/js/config.js"]) {
