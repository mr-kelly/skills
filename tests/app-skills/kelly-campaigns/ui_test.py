...
APP_ROOT = REPO_ROOT / "skills" / "kelly-campaigns" / "content" / "kelly-campaigns-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-campaigns"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset has 15 sends, all with an Approve/Request changes/Block
    # decision surface in the review queue.
    page.goto(f"{base_url}/?demo=campaigns#/campaigns")
    page.wait_for_load_state("networkidle")
    assert page.locator(".queue-card").count() == 15
    assert page.locator("[data-action='approve']").count() == 15

    # 4 suppression entries plus the 15-row deliverability QA table.
    page.goto(f"{base_url}/?demo=deliverability#/deliverability")
    page.wait_for_load_state("networkidle")
    assert page.locator(".suppression-facet table tbody tr").count() == 4
    assert page.locator(".content > .table-wrap table tbody tr").count() == 15

    # Only 2 demo sends carry performance data (the ones marked "done").
    page.goto(f"{base_url}/?demo=performance#/performance")
    page.wait_for_load_state("networkidle")
    assert page.locator("table tbody tr").count() == 2

    page.goto(f"{base_url}/?demo=detail#/campaigns/send-summer-launch")
    page.wait_for_load_state("networkidle")
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-campaigns-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-campaigns-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                sends_base = find_resource(nodes, "sends")
                assert sends_base and sends_base.get("baseId"), nodes
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{sends_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "send-id": "send-fixture",
                            "ref": 1,
                            "type": "campaign",
                            "phase": "deliver",
                            "subject": "Integration Fixture Send",
                            "segment-id": "all-subscribers",
                            "audience-size": 100,
                            "status": "needs_review",
                            "proposed-action": "schedule_send",
                            "body": "Draft for the integration fixture.",
                        },
                        "message": "Seed Kelly Campaigns integration fixture",
                ...
                # and a human verdict on the seeded send must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".queue-card", has_text="Integration Fixture Send")
                ...
                records = read_json(f"{busabase_url}/api/v1/records?baseId={sends_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("send-id") == "send-fixture"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["status"] == "approved", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(["app-root", "segments", "sends", "suppression", "settings"]), nodes
