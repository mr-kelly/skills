...
const appRoot = join(repoRoot, "skills", "kelly-campaigns", "content", "kelly-campaigns-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-campaigns");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-campaigns-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_CAMPAIGNS_DATA_PROVIDER|local-file-provider/);
  assert.match(source, /createBusabaseClient/);
});

test("retires the pre-Busabase local-file provider layer", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-campaigns");
  for (const path of [
    "lib",
    "config.example.json",
    "app/setup-gate.js",
...
test("ships a trusted execute-decisions script that never sends mail itself", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-campaigns");
