...
const appRoot = join(repoRoot, "skills", "kelly-demo-video-factory", "content", "kelly-demo-video-factory-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-demo-video-factory-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-demo-video-factory" });

  for (const path of ["/", "/app.js", "/js/config.js", "/styles.css"]) {
