...
APP_ROOT = REPO_ROOT / "skills" / "kelly-audit" / "content" / "kelly-audit-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-audit"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (20 orders, 16 invoices [1 credit note], 14 payments,
    # 8 anomalies) is ported verbatim from the retired app/server/demo.ts —
    ...
    # count-review=4 (needs_review), count-approved=1, count-blocked=1.
    ...
    page.goto(f"{base_url}/?demo=orders&lang=en#/orders")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 20
    assert "20" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=invoices&lang=en#/invoices")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 16

    page.goto(f"{base_url}/?demo=anomalies&lang=en#/anomalies")
    page.wait_for_load_state("networkidle")
    assert page.locator(".opp-card").count() == 8

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert page.locator(".opp-card").count() == 1
    assert "amount mismatch" in page.locator(".opp-card").inner_text().lower()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-audit-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-audit-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                anomalies_base = find_resource(nodes, "anomalies")
                orders_base = find_resource(nodes, "orders")
                assert anomalies_base and anomalies_base.get("baseId"), nodes
                assert orders_base and orders_base.get("baseId"), nodes

                anomaly_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{anomalies_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "anomaly-id": "fixture-overdue",
                            "ref": 1,
                            "rule": "overdue_receivable",
                            "severity": "high",
                            "status": "needs_review",
                            "title": "Fixture invoice overdue",
                            "customer": "Fixture Customer",
                            "amount-at-stake": 1000,
                            "currency": "USD",
                            "aging-bucket": "1-30",
                            "reason": "Integration fixture reason",
                            "evidence": json.dumps({"order_id": "", "invoice_id": "", "payment_ids": [], "rows": [], "computed": ""}),
                            "proposed-action": "chase_receivable",
                            "draft": "Fixture draft",
                            "created-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly Audit integration fixture anomaly",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [anomaly_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/anomalies/fixture-overdue")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    page.locator("[data-decision='approve']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={anomalies_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("anomaly-id") == "fixture-overdue"
                )
            ...
            assert sorted(keys) == sorted(
                ["app-root", "orders", "invoices", "payments", "anomalies", "import-log", "settings"]
            ), nodes
