...
const appRoot = join(repoRoot, "skills", "kelly-agent-observability", "content", "kelly-agent-observability-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-agent-observability-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-agent-observability" });

  for (const path of ["/", "/app.js", "/js/config.js", "/styles/shell.css"]) {
