...
def run_cloud_oauth(config: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-email-cloud-home-") as app_home:
        env = {
            "HOME": app_home,
            "PORT": str(port),
            "BUSABASE_BASE_URL": "",
        ...
        with managed_process(["node", "server.js"], APP_ROOT, env, f"{app_url}/health"):
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    # kelly-email provisions its Folder/Bases/Drive through the trusted
                    # scripts/init_busabase_schema.ts CLI, not a browser button — the
                    # AirApp only ever reads and reports readiness, so there is no
                    # in-browser mutation path to exercise here even in mutation mode.
