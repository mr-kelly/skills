...
APP_ROOT = REPO_ROOT / "skills" / "kelly-pr-review" / "content" / "kelly-pr-review-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-pr-review"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset has 6 pull requests: 1 needs_review, 1 to_approve,
    # 1 needs_test, 1 tested, 1 blocked, 1 done — verified against the ported
    # demo-provider.js dataset with a throwaway Playwright probe before
    # writing these assertions.
    page.goto(f"{base_url}/?demo=1&lang=en#/all")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-all").inner_text() == "6"
    assert page.locator("#count-needs_review").inner_text() == "1"
    assert page.locator("#count-to_approve").inner_text() == "1"
    assert page.locator("#count-needs_test").inner_text() == "1"
    assert page.locator("#count-tested").inner_text() == "1"
    assert page.locator("#count-blocked").inner_text() == "1"
    assert page.locator("#count-done").inner_text() == "1"
    assert page.locator(".pr-row").count() == 6
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=1&lang=en#/needs_review")
    page.wait_for_load_state("networkidle")
    assert page.locator(".pr-row").count() == 1
    page.locator(".pr-row").first.click()
    page.wait_for_timeout(200)
    assert page.locator("[data-action='approve']").count() == 1
    assert page.locator(".detail-title").is_visible()

    # Repo filter reflects the 3 repos in the demo dataset.
    options = page.locator("#repoFilter option").all_inner_texts()
    assert any("northstar/app (3)" in text for text in options), options
    assert any("northstar/docs (2)" in text for text in options), options
    assert any("northstar/sdk (1)" in text for text in options), options

    page.goto(f"{base_url}/?demo=1&lang=en#/needs_test")
    page.wait_for_load_state("networkidle")
    page.locator(".pr-row").first.click()
    page.wait_for_timeout(200)
    assert page.locator("[data-tested]").is_visible()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-pr-review-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-pr-review-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/needs_review")
                    page.wait_for_load_state("networkidle")
                ...
                record_cr = post_json(
                    ...
                    {
                        "fields": {
                            "item-id": "octo/fixture#1",
                            "repo": "octo/fixture",
                            "number": 1,
                            "title": "Integration Fixture PR",
                            "author": "octocat",
                            "url": "https://github.com/octo/fixture/pull/1",
                            "status": "needs_review",
                            "proposed-action": "comment",
                            "summary": "Draft for the integration fixture.",
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly PR Review integration fixture",
                ...
                # and a human verdict on the seeded PR must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    row = page.locator(".pr-row", has_text="Integration Fixture PR")
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("item-id") == "octo/fixture#1"
                )
            ...
            assert sorted(keys) == sorted(["app-root", "reviews", "settings"]), nodes
