...
const skillRoot = join(repoRoot, "skills", "kelly-pr-review");
const appRoot = join(skillRoot, "content", "kelly-pr-review-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-pr-review");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-pr-review-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_PR_REVIEW_DATA_PROVIDER|local-file-provider/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp never shells out to gh itself", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/pr-review-model.js"].map(
      (path) => readFile(join(browserRoot, path), "utf8"),
    ),
  );
  const source = sources.join("\n");
  assert.doesNotMatch(source, /child_process|execFile|\bexeca\b|"gh"|'gh'/);
});
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/generate_review_batch.ts",
...
test("ships both trusted gh CLI scripts: ingestion and execution", async () => {
  const generateSource = await readFile(join(skillRoot, "scripts", "generate_review_batch.mjs"), "utf8");
  assert.match(generateSource, /createBusabaseClient/);
  assert.match(generateSource, /BUSABASE_BASE_URL/);
  assert.match(generateSource, /execFile.*"gh"|"gh".*args|await gh\(/s);
  ...
  assert.match(executeSource, /pr.*review|reviewFlag/s);
  assert.match(executeSource, /--apply/);
  assert.doesNotMatch(executeSource, /\bmerge\b|--close|git push/i);
