...
const skillRoot = join(repoRoot, "skills", "kelly-legal-casebase-ingest");
const appRoot = join(skillRoot, "content", "kelly-legal-casebase-ingest-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-legal-casebase-ingest");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-legal-casebase-ingest-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LEGAL_CASEBASE_INGEST_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/casebase-model.js"].map(
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/ingest_documents.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_case_records.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all three trusted scripts: ingest_documents, execute_decisions, export_case_records", async () => {
  const ingestSource = await readFile(join(skillRoot, "scripts", "ingest_documents.mjs"), "utf8");
  assert.match(ingestSource, /createBusabaseClient/);
  assert.match(ingestSource, /BUSABASE_BASE_URL/);
  assert.match(ingestSource, /--apply/);
  assert.match(ingestSource, /validateItem/);
  ...
  const exportSource = await readFile(join(skillRoot, "scripts", "export_case_records.mjs"), "utf8");
