...
const appRoot = join(repoRoot, "skills", "kelly-legal-casebase-ingest", "content", "kelly-legal-casebase-ingest-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-legal-casebase-ingest-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-legal-casebase-ingest" });
