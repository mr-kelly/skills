...
APP_ROOT = REPO_ROOT / "skills" / "kelly-legal-casebase-ingest" / "content" / "kelly-legal-casebase-ingest-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-legal-casebase-ingest"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # 3 checks [1 warn, 2 pass]) is ported verbatim (same ids, same copy)
    ...
    page.wait_for_load_state("networkidle")
    assert "深圳商业租赁欠租解除案" in page.locator(".detail h2").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-legal-casebase-ingest-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-legal-casebase-ingest-home-") as app_home:
                ...
                item_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{items_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": "Intake #1",
                            "title": "Fixture lease-arrears case",
                            "category": "Fixture",
                            "status": "needs_review",
                        ...
                        "message": "Seed Legal Casebase Ingest integration fixture item",
