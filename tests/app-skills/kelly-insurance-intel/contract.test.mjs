...
const appRoot = join(repoRoot, "skills", "kelly-insurance-intel", "content", "kelly-insurance-intel-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-insurance-intel");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-insurance-intel-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(
    source,
    /KELLY_FINANCIAL_SERVICES_INTEL_UI_PORT|KELLY_FINANCIAL_SERVICES_INTEL_DATA_PROVIDER|local-file-provider/,
  );
...
test("a trusted skill-root decision-execution script exists, dry-run gated by --apply", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-insurance-intel");
