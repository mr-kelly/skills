...
const appRoot = join(
  ...
  "content",
  "kelly-financial-services-intel-app",
);
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-financial-services-intel");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-financial-services-intel-${base.key}`, base.key);
...
test("a trusted skill-root decision-execution script exists, dry-run gated by --apply", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-financial-services-intel");
