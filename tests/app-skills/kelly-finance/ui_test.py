...
APP_ROOT = REPO_ROOT / "skills" / "kelly-finance" / "content" / "kelly-finance-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-finance"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The fixed demo dataset (demoSnapshot() in app/app/js/finance-model.js,
    ...
    # 4 checks total, recomputed metrics needs_review=2 (interest-base,
    # scenario-case), approved=1 (working-capital), done=1 (balance-sheet),
    # blocked=0. 5 forecast periods (2026-2030), 5 workbook tabs.
    page.goto(f"{base_url}/?demo=1&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#pageTitle").inner_text() == "Kelly Finance"
    assert page.locator("#pageSubtitle").inner_text() == "Five-year fundraising forecast"
    assert page.locator("#attentionMain").inner_text() == "2 Need a note or decision"
    assert page.locator("#mobileMeta").inner_text() == "4 model checks"
    assert page.locator(".metric").count() == 4
    assert page.locator(".check-row").count() == 4
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=1&lang=en#/checks")
    page.wait_for_load_state("networkidle")
    assert page.locator(".check-row").count() == 4

    page.goto(f"{base_url}/?demo=1&lang=en#/checks/check-interest-base")
    page.wait_for_load_state("networkidle")
    assert page.locator(".detail-head h2").inner_text() == "Interest expense references prior debt"
    assert page.locator(".actions button").count() == 4

    page.goto(f"{base_url}/?demo=1&lang=en#/workbook")
    page.wait_for_load_state("networkidle")
    assert page.locator(".tabs span").count() == 5

    page.goto(f"{base_url}/?demo=1&lang=en#/settings")
    page.wait_for_load_state("networkidle")
    assert page.locator("#pageTitle").inner_text() == "Settings"

    page.goto(f"{base_url}/?demo=1&lang=zh#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator(".panel-head h2").first.inner_text() == "示例科技"
...
FIXTURE_MODEL = {
    "model-id": "current",
    "snapshot-id": "fixture-snapshot-001",
    "generated-at": "2026-01-01T00:00:00.000Z",
    "source": "local",
    "company": "Fixture Co",
    "currency": "USD",
    "display-unit": "units",
    "model-purpose": "Fixture board pack",
    "periods": json.dumps(
        [{"label": "2026", "revenue": 1000000, "gross_profit": 700000, "ebitda": 250000, "net_income": 125000,
          "ending_cash": 300000, "total_assets": 800000, "free_cash_flow": 50000}]
    ),
    "revenue-cagr": "0",
    "ending-cash": "300000",
    ...
    "balance-check": "0",
    "warnings": json.dumps([]),
    "workbook-path": "/tmp/fixture.xlsx",
    "workbook-tabs": json.dumps(["Assumptions", "Income Statement", "Balance Sheet", "Cash Flow", "Checks"]),
}
FIXTURE_CHECK = {
    "check-id": "fixture-check-001",
    "title": "Fixture balance sheet check",
    "summary": "Fixture summary",
    "severity": "warning",
    "status": "needs_review",
    "check-type": "statement_tie",
    "evidence": json.dumps(["Fixture evidence line"]),
    "proposed-action": "Review formula",
    "draft": "",
    "decision-action": "",
    "decision-comment": "",
    "decided-at": "",
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-finance-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-finance-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                model_base = find_resource(nodes, "model")
                checks_base = find_resource(nodes, "checks")
                settings_base = find_resource(nodes, "settings")
                assert model_base and model_base.get("baseId"), nodes
                assert checks_base and checks_base.get("baseId"), nodes
                assert settings_base and settings_base.get("baseId"), nodes

                model_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{model_base['baseId']}/change-requests",
                    {
                        "fields": FIXTURE_MODEL,
                        "message": "Seed Kelly Finance integration fixture model",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                check_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{checks_base['baseId']}/change-requests",
                    {
                        "fields": FIXTURE_CHECK,
                        "message": "Seed Kelly Finance integration fixture check",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(
                    f"{busabase_url}/api/v1/change-requests/merge",
                    {"changeRequestIds": [model_cr["id"], check_cr["id"]]},
                )
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/checks/fixture-check-001")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert page.locator("#pageSubtitle").inner_text() == "Fixture board pack"
                    assert page.locator(".detail-head h2").inner_text() == "Fixture balance sheet check"
                    page.locator("#decisionComment").fill("Trusted: matches manual spot-check of the fixture.")
                    page.locator("[data-action='approve']").click()
                    page.wait_for_timeout(500)
                    assert page.locator(".check-row.approved").count() >= 1
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={checks_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("check-id") == "fixture-check-001"
                )
            ...
            assert sorted(keys) == sorted(["app-root", "model", "checks", "settings"]), nodes
