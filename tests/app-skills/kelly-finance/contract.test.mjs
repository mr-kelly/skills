...
const appRoot = join(skillRoot, "content", "kelly-finance-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-finance");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-finance-${base.key}`, base.key);
