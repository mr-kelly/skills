...
const appRoot = join(skillRoot, "content", "kelly-invoice-sheet-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-invoice-sheet");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-invoice-sheet-${base.key}`, base.key);
