...
const appRoot = join(repoRoot, "skills", "kelly-invoice-sheet", "content", "kelly-invoice-sheet-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-invoice-sheet-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-invoice-sheet" });
