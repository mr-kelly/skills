...
APP_ROOT = REPO_ROOT / "skills" / "kelly-devops" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-devops"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (9 services, 8 configured expiries + cert rows derived
    # from services, 3 spend providers, 6 action cards) is ported verbatim
    # from the retired app/server/demo.ts — counts verified against
    # app/app/js/providers/demo-provider.js with a throwaway Playwright probe
    # before writing these assertions: count-review=2, count-expiring=2,
    # count-down=1, 9 service rows, 17 expiry rows (4 expiring <=30d), 3
    # spend cards, 6 action cards (2 needing a decision).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-review").inner_text() == "2"
    assert page.locator("#count-expiring").inner_text() == "2"
    assert page.locator("#count-down").inner_text() == "1"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=services&lang=en#/services")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 9
    assert "9" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=expiries&lang=en#/expiries")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 17

    page.goto(f"{base_url}/?demo=spend&lang=en#/spend")
    page.wait_for_load_state("networkidle")
    assert page.locator(".spend-card").count() == 3
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-devops-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-devops-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                actions_base = find_resource(nodes, "actions")
                services_base = find_resource(nodes, "services")
                assert actions_base and actions_base.get("baseId"), nodes
                assert services_base and services_base.get("baseId"), nodes

                action_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{actions_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "action-id": "fixture-restart",
                            "ref": 1,
                            "type": "restart_service",
                            "title": "Restart the fixture service",
                            "status": "needs_review",
                            "reason": "Integration fixture reason",
                            "evidence": json.dumps(["fixture evidence"]),
                            "plan": json.dumps(["fixture plan step"]),
                            "target": json.dumps({"kind": "service", "id": "fixture-svc"}),
                            "created-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly DevOps integration fixture action",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [action_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/actions/fixture-restart")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                ...
                records = read_json(f"{busabase_url}/api/v1/records?baseId={actions_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if r.get("headCommit", {}).get("fields", {}).get("action-id") == "fixture-restart"
                )
            ...
            assert sorted(keys) == sorted(
                ["app-root", "services", "expiries", "spend_providers", "spend_products", "actions", "events", "settings"]
            ), nodes
