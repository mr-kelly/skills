...
const appRoot = join(skillRoot, "content", "kelly-devops-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-devops");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-devops-${base.key}`, base.key);
