...
APP_ROOT = REPO_ROOT / "skills" / "kelly-legal-precedent-desk" / "content" / "kelly-legal-precedent-desk-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-legal-precedent-desk"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # 3 checks [1 pass + 1 warn on the first item, 1 pass on the second]) is
    ...
    page.wait_for_load_state("networkidle")
    assert "商业租赁欠租解除类案包" in page.locator(".detail h2").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-legal-precedent-desk-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-legal-precedent-desk-home-") as app_home:
                ...
                item_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{items_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": "Pack #1",
                            "title": "Fixture precedent research pack",
                            "category": "Fixture",
                            "status": "needs_review",
                            "summary": "Fixture summary for integration test",
                            "query": "Fixture legal question",
                            "jurisdiction": "Shenzhen",
                            "match-count": "2",
                            "created-at": "2026-01-01T00:00:00.000Z",
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Legal Precedent Desk integration fixture item",
