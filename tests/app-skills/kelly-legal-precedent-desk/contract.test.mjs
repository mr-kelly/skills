...
const skillRoot = join(repoRoot, "skills", "kelly-legal-precedent-desk");
const appRoot = join(skillRoot, "content", "kelly-legal-precedent-desk-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-legal-precedent-desk");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-legal-precedent-desk-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LEGAL_PRECEDENT_DESK_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/precedent-model.js"].map(
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/create_research_batch.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_research_pack.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all three trusted scripts: create_research_batch, execute_decisions, export_research_pack", async () => {
  const importSource = await readFile(join(skillRoot, "scripts", "create_research_batch.mjs"), "utf8");
  ...
  const exportSource = await readFile(join(skillRoot, "scripts", "export_research_pack.mjs"), "utf8");
