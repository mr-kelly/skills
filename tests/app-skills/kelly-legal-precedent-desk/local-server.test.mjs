...
const appRoot = join(repoRoot, "skills", "kelly-legal-precedent-desk", "content", "kelly-legal-precedent-desk-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-legal-precedent-desk-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-legal-precedent-desk" });
