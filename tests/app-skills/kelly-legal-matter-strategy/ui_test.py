...
APP_ROOT = REPO_ROOT / "skills" / "kelly-legal-matter-strategy" / "content" / "kelly-legal-matter-strategy-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-legal-matter-strategy"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (2 items [1 needs_review, 1 approved], 2 entities,
    # 3 checks [1 warn on the first item, 1 pass, 1 warn]) is ported verbatim
    # (same ids, same copy) from the retired app/server/demo.ts — counts
    # verified against app/app/js/providers/demo-provider.js with a
    # throwaway Node probe before writing these assertions.
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-needs").inner_text() == "1"
    assert page.locator("#count-ready").inner_text() == "1"
    assert page.locator("#count-blocked").inner_text() == "0"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=items&lang=en#/items")
    page.wait_for_load_state("networkidle")
    assert page.locator(".list .row").count() == 2
    assert "2" in page.locator(".list-head strong").inner_text()
    assert page.locator("#page-title").inner_text() == "Workbench"

    page.goto(f"{base_url}/?demo=review&lang=en#/review")
    page.wait_for_load_state("networkidle")
    assert page.locator(".list .row").count() == 1
    assert page.locator("#page-title").inner_text() == "Needs review"

    page.goto(f"{base_url}/?demo=checks&lang=en#/checks")
    page.wait_for_load_state("networkidle")
    assert page.locator(".check").count() == 3

    page.goto(f"{base_url}/?demo=entities&lang=en#/entities")
    page.wait_for_load_state("networkidle")
    assert page.locator(".entity").count() == 2

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert "SaaS 服务费欠款诉前策略" in page.locator(".detail h2").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-legal-matter-strategy-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-legal-matter-strategy-home-") as app_home:
                ...
                item_cr = post_json(
                    ...
                    {
                        "fields": {
                            "item-id": "fixture-item",
                            "ref": "Strategy #1",
                            "title": "Fixture matter strategy pack",
                            "category": "Fixture",
                            "status": "needs_review",
                            "summary": "Fixture summary for integration test",
                            "matter-stage": "诉前",
                            "evidence-gap-count": "1",
                            "deadline": "2026-12-01",
                            "created-at": "2026-01-01T00:00:00.000Z",
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Legal Matter Strategy integration fixture item",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [item_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    # At a mobile viewport the detail pane (and its decision
                    # buttons) is hidden until a row is opened — CSS toggles
                    # it on via body.route-detail once the hash carries an id.
                    page.locator(".list .row").first.click()
                    page.wait_for_selector("[data-action='approve']", state="visible")
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("item-id") == "fixture-item"
                )
            ...
            assert sorted(keys) == sorted(["app-root", "items", "entities", "checks", "settings"]), nodes
