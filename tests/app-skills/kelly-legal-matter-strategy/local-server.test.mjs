...
const appRoot = join(repoRoot, "skills", "kelly-legal-matter-strategy", "content", "kelly-legal-matter-strategy-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-legal-matter-strategy-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-legal-matter-strategy" });
