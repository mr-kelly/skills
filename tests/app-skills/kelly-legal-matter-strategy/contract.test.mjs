...
const skillRoot = join(repoRoot, "skills", "kelly-legal-matter-strategy");
const appRoot = join(skillRoot, "content", "kelly-legal-matter-strategy-app");
...
const requiredFiles = [
  ...
  "app/app.js",
  "app/js/config.js",
  "app/js/matter-strategy-model.js",
  "app/js/review-views.js",
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-legal-matter-strategy");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-legal-matter-strategy-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 4, "expected items/entities/checks/settings");
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LEGAL_MATTER_STRATEGY_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/matter-strategy-model.js"].map(
...
test("has a live decision workflow: the review write path goes through records.changeRequest", async () => {
  ...
  assert.match(source, /decideItem/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/create_strategy_batch.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_strategy_pack.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all three trusted scripts: create_strategy_batch, execute_decisions, export_strategy_pack", async () => {
  const importSource = await readFile(join(skillRoot, "scripts", "create_strategy_batch.mjs"), "utf8");
  assert.match(importSource, /createBusabaseClient/);
  assert.match(importSource, /BUSABASE_BASE_URL/);
  assert.match(importSource, /--apply/);
  assert.match(importSource, /validateItem/);
  ...
  assert.match(executeSource, /itemExecution/);

  const exportSource = await readFile(join(skillRoot, "scripts", "export_strategy_pack.mjs"), "utf8");
  assert.match(exportSource, /createBusabaseClient/);
  assert.match(exportSource, /BUSABASE_BASE_URL/);
  assert.match(exportSource, /buildMarkdown/);
  // Export only counts a genuine human "approve" decision as exportable —
  // never bare status, which an import payload could spoof.
  assert.match(exportSource, /decision_action === "approve"/);
