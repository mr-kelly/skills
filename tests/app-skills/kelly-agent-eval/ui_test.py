    ...
    with tempfile.TemporaryDirectory(prefix="kelly-agent-eval-busabase-") as data_dir:
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-agent-eval-home-") as app_home:
                ...
                fixture = next(
                    r for r in record_items if r.get("headCommit", {}).get("fields", {}).get("case-id") == "fixture-case"
                )
                assert fixture["headCommit"]["fields"]["decision-action"] == "mark_blocking", fixture
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            ...
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
                record.get("headCommit", {}).get("fields", {}).get("decision-action") == "mark_blocking"
