...
APP_ROOT = REPO_ROOT / "skills" / "kelly-agent-eval" / "content" / "kelly-agent-eval-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-agent-eval"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The fixed 18-case mock suite is ported verbatim from the retired
    # lib/eval-data.ts. Counts verified against app/app/js/eval-model.js's
    # buildRawCases()/computeMetrics() with a throwaway `node -e` probe
    # (stubbing window.location/navigator) before writing these assertions:
    # 11 regressions, 0 already decided (blocking=0), 11 pending review, no
    # release decision recorded yet.
    page.goto(f"{base_url}/?demo=1&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#regression-figure").inner_text() == "11"
    assert page.locator("#pending-review-line").inner_text() == "11"
    assert page.locator("#count-blocking").inner_text() == "0"
    assert page.locator("#count-release").inner_text() == "—"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=1&lang=en#/regressions")
    page.wait_for_load_state("networkidle")
    assert page.locator(".case-row").count() == 11
    assert "11" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=1&lang=en#/cases")
    page.wait_for_load_state("networkidle")
    assert page.locator(".case-row").count() == 18
    assert "18" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=1&lang=en#/cases/support-ticket-triage")
    page.wait_for_load_state("networkidle")
    assert "Support ticket triage priority" in page.locator("#page-title").inner_text()
    assert page.locator(".rubric-bars").count() == 1
    assert page.locator("#markBlockingBtn").count() == 1
    assert page.locator("#markAcceptableBtn").count() == 1
...
FIXTURE_CASE_FIELDS = {
    ...
    "title": "Fixture regression case",
    "category": "Support",
    "prompt": "Fixture prompt used by the OSS integration test.",
    "baseline-transcript": "Fixture baseline transcript, a good answer.",
    "baseline-helpfulness": 90,
    "baseline-correctness": 90,
    "baseline-safety": 90,
    "baseline-tone": 90,
    "candidate-transcript": "Fixture candidate transcript, a much worse answer.",
    "candidate-helpfulness": 50,
    "candidate-correctness": 50,
    "candidate-safety": 50,
    "candidate-tone": 50,
    "decision-action": "",
    "decision-note": "",
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-agent-eval-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-agent-eval-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                cases_base = find_resource(nodes, "cases")
                settings_base = find_resource(nodes, "settings")
                assert cases_base and cases_base.get("baseId"), nodes
                assert settings_base and settings_base.get("baseId"), nodes

                case_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{cases_base['baseId']}/change-requests",
                    {
                        "fields": FIXTURE_CASE_FIELDS,
                        "message": "Seed Kelly Agent Eval integration fixture case",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                settings_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{settings_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "record-id": "run",
                            "kind": "run",
                            "payload": json.dumps({"run_id": "fixture-run", "generated_at": "2026-01-01T00:00:00.000Z"}),
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly Agent Eval integration fixture run metadata",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(
                    f"{busabase_url}/api/v1/change-requests/merge",
                    {"changeRequestIds": [case_cr["id"], settings_cr["id"]]},
                )
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/cases/fixture-case")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert "Fixture regression case" in page.locator("#page-title").inner_text()
                    page.locator("#decisionNote").fill("Blocking: fixture regression is not acceptable.")
                    page.locator("#markBlockingBtn").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={cases_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r for r in record_items if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("case-id") == "fixture-case"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["decision-action"] == "mark_blocking", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(["app-root", "cases", "settings"]), nodes
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            ...
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
                (record.get("headCommit", {}).get("payload") or record.get("headCommit", {}).get("fields", {})).get("decision-action") == "mark_blocking"
