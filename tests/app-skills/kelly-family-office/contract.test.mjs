...
const appRoot = join(skillRoot, "content", "kelly-family-office-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-family-office");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-family-office-${base.key}`, base.key);
