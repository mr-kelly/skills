...
APP_ROOT = REPO_ROOT / "skills" / "kelly-retail-intel" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-retail-intel"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.wait_for_load_state("networkidle")
    assert "Weather and local events suggest a different product push today" in page.locator("#page-title").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-retail-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-retail-intel-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{signals_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": 1,
                            "title": "Fixture store promotion event",
                        ...
                        "message": "Seed Kelly Retail Intel integration fixture signal",
