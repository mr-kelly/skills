...
APP_ROOT = REPO_ROOT / "skills" / "kelly-lead-funnel" / "content" / "kelly-lead-funnel-app"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.goto(f"{base_url}/?demo=1#/board")
    page.wait_for_load_state("networkidle")
    # The demo pipeline is the 21-lead mock dataset ported verbatim from the
    # retired lib/mock-leads.ts: 5 kanban columns, 21 lead cards total.
    assert page.locator(".board-column").count() == 5
    assert page.locator(".lead-card").count() == 21
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-lead-funnel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-lead-funnel-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    assert page.locator(".lead-card").count() == 0
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                leads_base = find_resource(nodes, "leads")
                settings_base = find_resource(nodes, "settings")
                assert leads_base and settings_base, nodes

                lead_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{leads_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "lead-id": "lead-fixture",
                            "brand-name": "Fixture Lead Co",
                            "category": "services",
                            "city": "Fixture City",
                            "store-count": 12,
                            "est-monthly-revenue": 200000,
                            "lead-source": "referral",
                            "data-verifiable": "true",
                            "stage": "new",
                            "notes": "[]",
                            "stage-history": json.dumps([{"from": None, "to": "new", "at": "2026-07-01T00:00:00.000Z"}]),
                            "created-at": "2026-07-01T00:00:00.000Z",
                            "updated-at": "2026-07-01T00:00:00.000Z",
                        },
                        "message": "Seed fixture lead",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [lead_cr["id"]]})

                # A fresh app process must discover the existing resources and
                # records, and a direct kanban stage move (no separate
                # approval step) must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".lead-card", has_text="Fixture Lead Co")
                    assert card.is_visible()
                    card.click()
                    page.wait_for_timeout(200)
                    assert "/leads/lead-fixture" in page.url
                    page.locator("[data-move='data_verified']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
            ...
            assert sorted(keys) == sorted(["app-root", "leads", "settings"]), nodes
        ...
        with managed_process(["node", "server.js"], APP_ROOT, {"HOME": app_home, "PORT": str(port)}, f"{base_url}/health"):
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    print("PASS OSS - lazy provisioning, direct kanban stage-move write, and persistence against temporary Busabase")
                except Exception:
