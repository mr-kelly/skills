...
APP_ROOT = REPO_ROOT / "skills" / "kelly-restaurant-intel" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-restaurant-intel"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.wait_for_load_state("networkidle")
    assert "A local event nearby changes expected customer flow" in page.locator("#page-title").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-restaurant-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-restaurant-intel-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{signals_base['baseId']}/change-requests",
                    {
                        ...
                        "message": "Seed Kelly Restaurant Intel integration fixture signal",
