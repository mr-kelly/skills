...
const appRoot = join(repoRoot, "skills", "kelly-restaurant-intel", "content", "kelly-restaurant-intel-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-restaurant-intel");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-restaurant-intel-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(
    source,
    /KELLY_RESTAURANT_INTEL_UI_PORT|KELLY_RESTAURANT_INTEL_DATA_PROVIDER|local-file-provider/,
  );
...
test("a trusted skill-root decision-execution script exists, dry-run gated by --apply", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-restaurant-intel");
