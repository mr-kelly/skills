...
const appRoot = join(repoRoot, "skills", "kelly-social", "content", "kelly-social-app");
...
const requiredFiles = [
  ...
  "app/app.js",
  "app/js/config.js",
  "app/js/social-model.js",
  "app/js/publishing-views.js",
];
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-social");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-social-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  ...
  assert.ok(
    ...
    "kelly-social should declare accounts/posts/sync_log/calendar/drafts/shorts/engagement/settings",
  );
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_SOCIAL_DATA_PROVIDER|local-file-provider/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp never performs a real publish/send — it only records approval intent", async () => {
  const source = await readFile(join(browserRoot, "js", "providers", "busabase-provider.js"), "utf8");
  assert.doesNotMatch(source, /fetch\(.*(twitter|facebook|instagram|graph\.facebook|api\.twitter|x\.com)/i);
  assert.match(source, /out of band/i);
});

test("retires the pre-Busabase local-file provider layer", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-social");
  for (const path of [
    "lib",
    "config.example.json",
    "app/setup-gate.js",
    ...
    "scripts/ingest_snapshot.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships a trusted ingest script that writes with its own Busabase credentials", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-social");
  const source = await readFile(join(skillRoot, "scripts", "ingest_snapshot.mjs"), "utf8");
  assert.match(source, /createBusabaseClient/);
