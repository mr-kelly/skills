...
const appRoot = join(skillRoot, "content", "kelly-sales-outreach-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-sales-outreach");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-sales-outreach-${base.key}`, base.key);
