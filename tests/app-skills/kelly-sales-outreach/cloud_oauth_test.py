...
APP_ROOT = REPO_ROOT / "skills" / "kelly-sales-outreach" / "content" / "kelly-sales-outreach-app"
REQUIRED_ENV = (
...
def run_cloud_oauth(config: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-sales-outreach-cloud-home-") as app_home:
        env = {
            "HOME": app_home,
            "PORT": str(port),
            "BUSABASE_BASE_URL": "",
            ...
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    if status.get("requiresSpace"):
                        ...
                        page.get_by_role("heading", name=re.compile("初始化|待审核")).wait_for(timeout=20_000)
                        status = read_browser_json(page, "/auth/status")
                    ...
                    if config["allow_mutation"] and initialize.count():
                        initialize.click()
                        page.get_by_role("heading", name="先告诉我你卖什么", exact=True).wait_for(
                            timeout=30_000
                        )
                        page.locator('[data-onboarding="sellerName"]').fill("Cloud OAuth Test")
                        page.locator('[data-onboarding="offerName"]').fill("Test Sales Product")
                        page.locator('[data-onboarding="offerSummary"]').fill(
                            "Dedicated product description for the Sales Outreach onboarding gate."
                        )
