...
const appRoot = join(repoRoot, "skills", "kelly-sales-outreach", "content", "kelly-sales-outreach-app");
let baseUrl;
let runtime;
let home;
...
test.before(async () => {
  ...
  home = await mkdtemp(join(tmpdir(), "kelly-sales-outreach-home-"));
...
test("serves health and canonical browser assets", async () => {
  ...
  assert.deepEqual(await healthResponse.json(), { ok: true, app: "kelly-sales-outreach", mode: "outreach-desk" });
...
test("rejects cross-origin OAuth starts", async () => {
  ...
  assert.match(location.searchParams.get("oauth_error"), /origin did not match|来源不匹配/i);
});
...
test("requires an explicit valid Space before proxying a multi-Space account", async () => {
  ...
  try {
    ...
    assert.equal(noneStatus.reason, "SPACE_SELECTION_REQUIRED");
    ...
    assert.match((await invalid.json()).error, /not accessible|不属于当前账号/i);
