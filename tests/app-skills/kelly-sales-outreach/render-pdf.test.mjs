// A generated PDF failed to render on a real machine because the one Chrome the
...
import {
  ...
  playwrightChromiums,
  renderPdf,
} from "../../../skills/kelly-sales-outreach/scripts/render_pdf.mjs";
...
<body><h1>AI 客服质检</h1><p>全量分析会话并给出改进项</p></body></html>`;

async function fixture() {
  const dir = await mkdtemp(join(tmpdir(), "sales-outreach-pdf-"));
  const htmlPath = join(dir, "one-pager.html");
  await writeFile(htmlPath, HTML, "utf8");
  return { dir, htmlPath, pdfPath: join(dir, "one-pager.pdf"), userDataDir: join(dir, "chrome") };
}
