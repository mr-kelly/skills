...
APP_ROOT = REPO_ROOT / "skills" / "kelly-agent-builder" / "content" / "kelly-agent-builder-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-agent-builder"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.goto(f"{base_url}/?demo=1#/catalog")
    page.wait_for_load_state("networkidle")
    assert page.locator("#page-title").inner_text() == "Agent Catalog"
    ...
    assert page.locator("#agentForm").is_visible()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-agent-builder-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-agent-builder-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    try:
                        page.get_by_text("No agent configs yet").wait_for(timeout=10_000)
                    except Exception:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{agents_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "agent-id": "agent-fixture",
                            "name": "Integration Fixture Agent",
                            "status": "live",
                            "owning-team": "QA",
                            "monthly-quota": 100,
                            "calls-this-month": 0,
                        ...
                        "message": "Seed Kelly Agent Builder integration fixture",
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert page.get_by_text("Integration Fixture Agent").is_visible()
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
            ...
            assert sorted(keys) == sorted(["app-root", "agents", "settings"]), nodes
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            ...
            agents_base = find_resource(nodes, "agents")
            records = read_json(f"{busabase_url}/api/v1/records?baseId={agents_base['baseId']}")
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
                (record.get("headCommit", {}).get("payload") or record.get("headCommit", {}).get("fields", {})).get("name") == "Integration Fixture Agent"
