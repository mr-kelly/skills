...
APP_ROOT = REPO_ROOT / "skills" / "kelly-inquiry" / "content" / "kelly-inquiry-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-inquiry"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset ("Lumina Lighting Co.") has 12 inquiries across
    # new/replied/quoted/negotiating/won/lost.
    page.goto(f"{base_url}/?demo=inquiries#/inquiries")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap table tbody tr").count() == 12

    # 5 quote worksheets.
    page.goto(f"{base_url}/?demo=quotes#/quotes")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap table tbody tr").count() == 5

    # 7 products in the knowledge base.
    page.goto(f"{base_url}/?demo=products#/products")
    page.wait_for_load_state("networkidle")
    assert page.locator(".product-grid .product-card").count() == 7

    # 6 approval items (refs 1-6): 2 needs_review, 1 changes_requested,
    # 1 approved, 1 done, 1 blocked by the min-price guard.
    page.goto(f"{base_url}/?demo=approvals#/approvals")
    page.wait_for_load_state("networkidle")
    assert page.locator(".approval-list .approval-card").count() == 6
    assert page.locator(".approval-card .status-chip.blocked").count() == 1

    # The featured hot WhatsApp inquiry has two pending drafts (a reply and a
    # quote) already queued, rendered as queued-bubbles in the transcript.
    page.goto(f"{base_url}/?demo=detail&lang=en#/inquiries/wa-mueller-led-panels")
    page.wait_for_load_state("networkidle")
    assert page.locator(".bubble-row.out .queued-bubble").count() == 2
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-inquiry-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-inquiry-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                inquiries_base = find_resource(nodes, "inquiries")
                approvals_base = find_resource(nodes, "approvals")
                assert inquiries_base and inquiries_base.get("baseId"), nodes
                assert approvals_base and approvals_base.get("baseId"), nodes

                inquiry_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{inquiries_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "inquiry-id": "inq-fixture",
                            "account-id": "",
                            "channel": "whatsapp",
                            "customer-name": "Fixture Customer",
                            "customer-company": "Fixture Co",
                            "stage": "new",
                            "value-estimate": "1000",
                            "currency": "USD",
                            "owner": "Kelly",
                        ...
                        "message": "Seed Kelly Inquiry integration fixture inquiry",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [inquiry_cr["id"]]})

                approval_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{approvals_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "item-id": "appr-fixture",
                            "kind": "reply",
                            "inquiry-id": "inq-fixture",
                            "account-id": "",
                            "channel": "whatsapp",
                            "customer": "Fixture Customer",
                            "text": "Thanks for the inquiry, here is our best price.",
                            "reason": "Integration fixture",
                            "suggested-by": "agent",
                            "status": "needs_review",
                            "created-at": "2026-07-06T08:05:00.000Z",
                        },
                        "message": "Seed Kelly Inquiry integration fixture approval",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                ...
                # records, and a human verdict on the seeded approval must
                # write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".approval-card", has_text="Fixture Customer")
                    assert card.is_visible()
                    card.locator("[data-action='decide'][data-decision='approve']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("item-id") == "appr-fixture"
                )
            ...
            assert sorted(keys) == sorted(
                ["app-root", "accounts", "inquiries", "messages", "products", "quotes", "approvals", "sync-log", "settings"]
            ), nodes
