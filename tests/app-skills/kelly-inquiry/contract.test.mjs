...
const appRoot = join(skillRoot, "content", "kelly-inquiry-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-inquiry");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-inquiry-${base.key}`, base.key);
