...
const appRoot = join(skillRoot, "content", "kelly-jobhunt-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-jobhunt");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-jobhunt-${base.key}`, base.key);
