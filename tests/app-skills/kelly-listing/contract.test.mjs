...
const skillRoot = join(repoRoot, "skills", "kelly-listing");
const appRoot = join(skillRoot, "content", "kelly-listing-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-listing");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-listing-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 6, "expected products/drafts/checks/claims/claim_rules/settings");
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_LISTING_DATA_PROVIDER|local-file-provider|config\.local\.json/);
...
test("the AirApp itself never reads a local file or performs the real follow-up", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/listing-model.js"].map(
...
test("has a live decision workflow: the review write path goes through records.changeRequest", async () => {
  ...
  assert.match(source, /decideDraft/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/ingest_drafts.ts",
    "scripts/run_checks.ts",
    "scripts/execute_decisions.ts",
    "scripts/export_listings.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships all four trusted scripts: ingest_drafts, run_checks, execute_decisions, export_listings", async () => {
  const ingestSource = await readFile(join(skillRoot, "scripts", "ingest_drafts.mjs"), "utf8");
  assert.match(ingestSource, /createBusabaseClient/);
  assert.match(ingestSource, /BUSABASE_BASE_URL/);
  assert.match(ingestSource, /--apply/);
  assert.match(ingestSource, /validateDraft/);
  ...
  assert.match(checksSource, /evaluateDraft/);
  ...
  assert.match(executeSource, /draftExecution/);

  const exportSource = await readFile(join(skillRoot, "scripts", "export_listings.mjs"), "utf8");
  assert.match(exportSource, /createBusabaseClient/);
  assert.match(exportSource, /BUSABASE_BASE_URL/);
  assert.match(exportSource, /listingMarkdown/);
