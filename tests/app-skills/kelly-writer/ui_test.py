...
APP_ROOT = REPO_ROOT / "skills" / "kelly-writer" / "content" / "kelly-writer-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-writer"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset has 4 channel drafts (verified against the ported
    # demo-provider.js dataset: dist-blog=needs_review, dist-linkedin=to_approve,
    # dist-newsletter=approved, dist-x=needs_review).
    page.goto(f"{base_url}/?demo=1#/drafts")
    page.wait_for_load_state("networkidle")
    assert page.locator(".queue-card").count() == 4
    assert page.locator("[data-action='approve']").count() == 4
    assert page.locator(".queue-filter[data-filter='needs_review'] small").inner_text() == "2"
    assert page.locator(".queue-filter[data-filter='approved'] small").inner_text() == "1"
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-writer-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-writer-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{drafts_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "channel": "linkedin",
                            "title": "Integration Fixture Draft",
                            "body": "Draft for the integration fixture.",
                            "status": "needs_review",
                        },
                        "message": "Seed Kelly Writer integration fixture",
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".queue-card", has_text="Integration Fixture Draft")
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("draft-id") == "draft-fixture"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["status"] == "approved", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(["app-root", "drafts", "settings"]), nodes
