...
const skillRoot = join(repoRoot, "skills", "kelly-tickets");
const appRoot = join(skillRoot, "content", "kelly-tickets-app");
...
const requiredFiles = [
  ...
  "app/app.js",
  "app/js/config.js",
  "app/js/tickets-model.js",
  "app/js/ticket-views.js",
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-tickets");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-tickets-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 6, "expected crews/intake/tickets/proposals/sync_log/settings");
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_TICKETS_DATA_PROVIDER|local-file-provider|config\.local\.json/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp itself never reads a local file", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/tickets-model.js"].map(
...
test("has live decision workflows: proposal/intake/ticket writes go through records.changeRequest", async () => {
  ...
  assert.match(source, /decideIntake/);
  assert.match(source, /reviseTicket/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/ingest_intake.ts",
    "scripts/apply_triage.ts",
...
test("ships all three trusted scripts: ingest_intake, apply_triage, execute_decisions", async () => {
  const ingestSource = await readFile(join(skillRoot, "scripts", "ingest_intake.mjs"), "utf8");
  assert.match(ingestSource, /createBusabaseClient/);
  assert.match(ingestSource, /BUSABASE_BASE_URL/);
  assert.match(ingestSource, /--apply/);
  assert.match(ingestSource, /maskContact/);

  const triageSource = await readFile(join(skillRoot, "scripts", "apply_triage.mjs"), "utf8");
  assert.match(triageSource, /createBusabaseClient/);
  assert.match(triageSource, /BUSABASE_BASE_URL/);
  assert.match(triageSource, /--apply/);
  assert.match(triageSource, /slaHoursFor/);
  ...
  // The planner never performs the real crew notification itself.
  assert.doesNotMatch(executeSource, /sendMessage|sendEmail|nodemailer/i);
