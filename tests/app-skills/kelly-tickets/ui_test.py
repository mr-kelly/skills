...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (14 intake items, 10 tickets, 6 dispatch proposals, 4
    # crews) is ported verbatim from the retired app/server/demo.ts — counts
    # verified against app/app/js/providers/demo-provider.js with a
    # throwaway `node -e` probe (stubbing window.location/navigator) before
    # writing these assertions: count-review=3 (needs_review proposals),
    # count-unclassified=3 (intake triage_state="new"), count-sla=4
    # (open/assigned/in_progress/waiting tickets with sla_state at_risk or
    # breached).
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#count-review").inner_text() == "3"
    assert page.locator("#count-unclassified").inner_text() == "3"
    assert page.locator("#count-sla").inner_text() == "4"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=intake&lang=en#/intake")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 14
    assert "14" in page.locator("#page-subtitle").inner_text()

    page.goto(f"{base_url}/?demo=dispatch&lang=en#/dispatch")
    page.wait_for_load_state("networkidle")
    assert page.locator(".proposal-card").count() == 6

    page.goto(f"{base_url}/?demo=board&lang=en#/board")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == 10

    page.goto(f"{base_url}/?demo=detail&lang=en")
    page.wait_for_load_state("networkidle")
    assert "T-1001" in page.locator("#page-title").inner_text()
    assert page.locator(".timeline-item").count() == 6
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-tickets-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-tickets-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                crews_base = find_resource(nodes, "crews")
                tickets_base = find_resource(nodes, "tickets")
                proposals_base = find_resource(nodes, "proposals")
                assert crews_base and crews_base.get("baseId"), nodes
                assert tickets_base and tickets_base.get("baseId"), nodes
                assert proposals_base and proposals_base.get("baseId"), nodes

                crew_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{crews_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "crew-id": "fixture-crew",
                            "name": "Fixture Crew",
                            "skills": json.dumps(["plumbing"]),
                            "members": "Lead: Fixture",
                            "contact-env": "KELLY_TICKETS_CREW_FIXTURE_CONTACT",
                            "active": "true",
                        },
                        "message": "Seed Kelly Tickets integration fixture crew",
                ...
                ticket_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{tickets_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "resolved-at": "",
                            "sla-due-at": "2026-01-01T04:00:00.000Z",
                            "intake-ids": json.dumps([]),
                            "resolution-note": "",
                            "history": json.dumps(
                                [{"event": "intake", "actor": "kelly-tickets", "at": "2026-01-01T00:00:00.000Z", "note": ""}]
                            ),
                        },
                        "message": "Seed Kelly Tickets integration fixture ticket",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                proposal_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{proposals_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "proposal-id": "fixture-dispatch",
                            "ref": 1,
                            "ticket-id": "fixture-ticket",
                            "title": "Dispatch fixture crew",
                            "summary": "Fixture leak",
                            "proposed-crew-id": "fixture-crew",
                            "proposed-assignee": "",
                            "priority": "P1",
                            "sla-due-at": "2026-01-01T04:00:00.000Z",
                            "sla-hours": 4,
                            "reason": "Integration fixture reason",
                            "note-to-crew": "Fixture note",
                            "status": "needs_review",
                            "decision-action": "",
                            "decision-note": "",
                            "decision-draft": "",
                        ...
                        "message": "Seed Kelly Tickets integration fixture dispatch proposal",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(
                    f"{busabase_url}/api/v1/change-requests/merge",
                    {"changeRequestIds": [crew_cr["id"], ticket_cr["id"], proposal_cr["id"]]},
                )
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    page.locator("[data-decision='approve'][data-id='fixture-dispatch']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()

                records = read_json(f"{busabase_url}/api/v1/records?baseId={proposals_base['baseId']}")
                record_items = records if isinstance(records, list) else records.get("records", [])
                fixture = next(
                    r
                    for r in record_items
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("proposal-id") == "fixture-dispatch"
                )
