...
APP_ROOT = REPO_ROOT / "skills" / "kelly-launch" / "content" / "kelly-launch-app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-launch"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    assert page.locator(".readiness-card").is_visible()
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=1#/assets")
    page.wait_for_load_state("networkidle")
    assert page.locator(".queue-card").count() == 16
    assert page.locator("[data-action='approve']").count() == 16

    page.goto(f"{base_url}/?demo=1#/launchday")
    page.wait_for_load_state("networkidle")
    assert page.locator(".runbook-step").count() == 8
    ...
    for width, height in ((390, 844), (360, 740)):
        ...
        errors = attach_error_capture(page)
        page.goto(f"{base_url}/?demo=1#/checklist")
        page.wait_for_load_state("networkidle")
        assert_no_horizontal_overflow(page)
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-launch-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-launch-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{items_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": 1,
                            "phase": "assemble",
                            "title": "Integration Fixture Item",
                            "readiness": "SHIP",
                            "proposed-action": "publish_asset",
                            "status": "needs_review",
                            "draft": "Draft for the integration fixture.",
                        },
                        "message": "Seed Kelly Launch integration fixture",
                ...
                # and a human verdict on the seeded item must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page = context.new_page()
                    errors = attach_error_capture(page)
                    page.goto(f"{app_url}/#/assets")
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".queue-card", has_text="Integration Fixture Item")
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("item-id") == "item-fixture"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["status"] == "approved", fixture

            nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
            keys = resource_keys(nodes)
            assert sorted(keys) == sorted(["app-root", "items", "channels", "runbook", "settings"]), nodes
