...
const appRoot = join(skillRoot, "content", "kelly-feedback-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-feedback");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-feedback-${base.key}`, base.key);
