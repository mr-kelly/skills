...
APP_ROOT = REPO_ROOT / "skills" / "kelly-feedback" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-feedback"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset has 27 feedback items across 7 sources, 2 new/untriaged.
    page.goto(f"{base_url}/?demo=inbox#/inbox")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap table tbody tr").count() == 27

    # 8 clustered feature requests.
    page.goto(f"{base_url}/?demo=requests#/requests")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap table tbody tr").count() == 8

    # 5 roadmap proposals (1 needs_review), 2 items in each of now/next/later.
    page.goto(f"{base_url}/?demo=roadmap#/roadmap")
    page.wait_for_load_state("networkidle")
    assert page.locator(".proposal-card").count() == 5
    assert page.locator(".roadmap-column").count() == 3
    assert page.locator(".roadmap-card").count() == 6

    # The featured CSV-export request: 3 representative quotes, weighted score 17.
    page.goto(f"{base_url}/?demo=detail&lang=en#/requests/req-csv-export")
    page.wait_for_load_state("networkidle")
    assert page.locator(".quote").count() == 3
    assert "17" in page.locator("#page-subtitle").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-feedback-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-feedback-home-") as app_home:
                ...
                record_cr = post_json(
                    ...
                    {
                        "fields": {
                            "proposal-id": "prop-fixture",
                            "type": "promote_request",
                            "title": "Integration Fixture Proposal",
                            "status": "needs_review",
                            "target-lane": "next",
                            "reason": "Fixture reason for the OSS integration test.",
                            "created-at": "2026-07-02T08:00:00.000Z",
                        },
                        "message": "Seed Kelly Feedback integration fixture",
                ...
                # records, and a human verdict on the seeded proposal must
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".proposal-card", has_text="Integration Fixture Proposal")
                    assert card.is_visible()
                    card.locator("button[data-verdict='approve']").click()
                    page.wait_for_timeout(500)
                    assert_no_horizontal_overflow(page)
                    assert not errors, errors
                    context.close()
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("proposal-id") == "prop-fixture"
                )
