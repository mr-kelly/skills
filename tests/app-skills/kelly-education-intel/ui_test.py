...
APP_ROOT = REPO_ROOT / "skills" / "kelly-education-intel" / "app"
RESULTS_ROOT = REPO_ROOT / "test-results" / "kelly-education-intel"
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # throwaway `node -e` probe against buildSnapshot() before writing these
    # assertions: 3+3+3 = 9 total needs_review, 0 approved, 0 blocked.
    ...
    page.wait_for_load_state("networkidle")
    assert "Exam or admissions dates" in page.locator("#page-title").inner_text()
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-education-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-education-intel-home-") as app_home:
                ...
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{signals_base['baseId']}/change-requests",
                    {
                        "fields": {
                            ...
                            "ref": 1,
                            "title": "Fixture exam or admissions date",
                        ...
                        "message": "Seed Kelly Education Intel integration fixture signal",
