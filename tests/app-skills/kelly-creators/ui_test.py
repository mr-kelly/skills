...
APP_ROOT = REPO_ROOT / "skills" / "kelly-creators" / "content" / "kelly-creators-app"
...
# item) — verified with a throwaway `node -e` probe against
# demoProvider.getState(), not guessed.
ENGAGEMENT_COUNT = 14
TOTAL_ROW_COUNT = 15  # engagements + the one quality-gate item
...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    assert page.locator(".overview-grid").is_visible()
    assert page.locator(".due-row").count() == 5  # top-5 candidates by fit score
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=1#/creators")
    page.wait_for_load_state("networkidle")
    assert page.locator(".creator-card").count() == ENGAGEMENT_COUNT

    page.goto(f"{base_url}/?demo=1#/outreach")
    page.wait_for_load_state("networkidle")
    assert page.locator(".queue-card").count() == TOTAL_ROW_COUNT
    assert page.locator("[data-action='approve']").count() == TOTAL_ROW_COUNT

    page.goto(f"{base_url}/?demo=1#/roi")
    page.wait_for_load_state("networkidle")
    assert page.locator(".table-wrap tbody tr").count() == ENGAGEMENT_COUNT
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-creators-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-creators-home-") as app_home:
                ...
                nodes = read_json(f"{busabase_url}/api/v1/nodes?depth=2")
                creators_base = find_resource(nodes, "creators")
                assert creators_base and creators_base.get("baseId"), nodes
                record_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{creators_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "creator-id": "cr-fixture",
                            "ref": 1,
                            ...
                            "niche": "beauty",
                            "status": "needs_review",
                            "proposed-action": "send_outreach",
                            "reason": "Integration fixture engagement.",
                            "suggested-reply": "Draft outreach for the integration fixture.",
                        },
                        "message": "Seed Kelly Creators integration fixture",
                ...
                # and a human "approve" verdict on the seeded engagement must write
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    card = page.locator(".queue-card", has_text="Fixture Creator")
            ...
            assert sorted(keys) == sorted(["app-root", "creators", "settings"]), nodes
