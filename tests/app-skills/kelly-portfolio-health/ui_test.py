...
APP_ROOT = REPO_ROOT / "skills" / "kelly-portfolio-health" / "content" / "kelly-portfolio-health-app"
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-portfolio-health-busabase-") as data_dir:
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-portfolio-health-home-") as app_home:
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("contract-id") == "rbf-fixture"
                )
                assert (fixture["headCommit"].get("payload") or fixture["headCommit"]["fields"])["flagged"] == "true", fixture
        ...
        with managed_process(busabase_command, REPO_ROOT, {}, f"{busabase_url}/api/health", timeout=90):
            ...
            record_items = records if isinstance(records, list) else records.get("records", [])
            assert any(
                (record.get("headCommit", {}).get("payload") or record.get("headCommit", {}).get("fields", {})).get("flagged") == "true" for record in record_items
            )
