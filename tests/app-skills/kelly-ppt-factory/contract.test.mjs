...
const skillRoot = join(repoRoot, "skills", "kelly-ppt-factory");
const appRoot = join(skillRoot, "content", "kelly-ppt-factory-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-ppt-factory");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-ppt-factory-${base.key}`, base.key);
...
test("every declared Base stays within the records.list limit=100 server cap", async () => {
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
  assert.ok(appConfig.bases.length >= 7, "expected projects/decks/slideCards/styleSystems/qaChecks/exports/settings");
  for (const base of appConfig.bases) {
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_PPT_FACTORY_CONFIG|local-file-provider|config\.local\.json/);
  assert.match(source, /createBusabaseClient/);
});

test("the AirApp itself never reads a local file or generates a PPTX", async () => {
  const sources = await Promise.all(
    ["app.js", "js/providers/busabase-provider.js", "js/providers/demo-provider.js", "js/ppt-model.js"].map((path) =>
      readFile(join(browserRoot, path), "utf8"),
    ),
  );
  const source = sources.join("\n");
  ...
  assert.doesNotMatch(source, /pptxgenjs/);
});
...
test("has a live decision workflow: the review write path goes through records.changeRequest", async () => {
  ...
  assert.match(source, /async decideItem\(/);
  const { appConfig } = await import(join(browserRoot, "js", "config.js"));
...
test("retires the pre-Busabase local-file provider layer", async () => {
  for (const path of [
    ...
    "scripts/generate_pptx.ts",
  ]) {
    await assert.rejects(readFile(join(skillRoot, path)));
  }
});

test("ships both trusted scripts: generate_pptx and execute_decisions, each with their own credentials", async () => {
  const generateSource = await readFile(join(skillRoot, "scripts", "generate_pptx.mjs"), "utf8");
  assert.match(generateSource, /createBusabaseClient/);
  assert.match(generateSource, /BUSABASE_BASE_URL/);
  assert.match(generateSource, /pptxgenjs/);
  // Generation is gated on a genuine human "approve" decision, not bare status.
  assert.match(generateSource, /decision_action !== "approve"/);
  ...
  assert.doesNotMatch(executeSource, /status:\s*"generated"/);
  assert.match(executeSource, /itemExecution/);

  const pkg = await readJson(join(skillRoot, "package.json"));
  assert.equal(pkg.dependencies["busabase-sdk"], "0.17.2");
  assert.match(pkg.dependencies.pptxgenjs, /^\^?3\./);
});
