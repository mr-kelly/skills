...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    # The demo dataset (3 projects, 3 decks, 5 slide cards, 3 QA checks,
    # 2 exports, 3 review-queue rows [1 needs_review, 1 changes_requested on
    # slides, 1 approved on a deck]) is ported verbatim (same ids, same
    # copy) from the retired app/server/demo.ts — counts confirmed by
    # reading app/app/js/providers/demo-provider.js's fixtures directly
    # before writing these assertions.
    page.goto(f"{base_url}/?demo=overview&lang=en#/overview")
    page.wait_for_load_state("networkidle")
    assert page.locator("#attentionCount").inner_text() == "2"
    assert page.locator("#approvedCount").inner_text() == "1"
    assert page.locator("#readyCount").inner_text() == "2"
    assert_no_horizontal_overflow(page)

    page.goto(f"{base_url}/?demo=projects&lang=en#/projects")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 3

    page.goto(f"{base_url}/?demo=decks&lang=en#/decks")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 3

    page.goto(f"{base_url}/?demo=slides&lang=en#/slides")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 5

    page.goto(f"{base_url}/?demo=review&lang=en#/review")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 3

    page.goto(f"{base_url}/?demo=exports&lang=en#/exports")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 2

    page.goto(f"{base_url}/?demo=overview&lang=en#/style")
    page.wait_for_load_state("networkidle")
    assert page.locator("#listPanel .rows .row").count() == 1
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-ppt-factory-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-ppt-factory-home-") as app_home:
                ...
                slide_cr = post_json(
                    f"{busabase_url}/api/v1/bases/{slide_cards_base['baseId']}/change-requests",
                    {
                        "fields": {
                            "slide-id": "fixture-slide",
                            "ref": "1",
                            "deck-id": "fixture-deck",
                            "project-id": "fixture-project",
                            "status": "needs_review",
                            "slide-type": "concept",
                            "layout": "headline left",
                            "title": "Fixture slide card",
                            "objective": "Fixture objective for integration test",
                            "review-summary": "Needs a sharper headline before generation.",
                            "created-at": "2026-01-01T00:00:00.000Z",
                            "updated-at": "2026-01-01T00:00:00.000Z",
                        },
                        "message": "Seed Kelly PPT Factory integration fixture slide card",
                        "submittedBy": "kelly-skills-test",
                    },
                )
                post_json(f"{busabase_url}/api/v1/change-requests/merge", {"changeRequestIds": [slide_cr["id"]]})
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    # it on via body.mobile-detail-open once the hash carries an id.
                    page.locator("#listPanel .rows .row").first.click()
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("slide-id") == "fixture-slide"
                )
