...
const appRoot = join(repoRoot, "skills", "kelly-brand", "content", "kelly-brand-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-brand");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-brand-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /KELLY_BRAND_DATA_PROVIDER|local-file-provider/);
...
test("retires the pre-Busabase local-file provider layer", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-brand");
  for (const path of [
    "lib",
    "config.example.json",
    "app/setup-gate.js",
...
test("has a trusted execute_decisions.mjs script with a --apply dry-run-by-default contract", async () => {
  const skillRoot = join(repoRoot, "skills", "kelly-brand");
