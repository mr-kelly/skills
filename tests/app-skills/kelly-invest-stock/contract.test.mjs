...
const appRoot = join(repoRoot, "skills", "kelly-invest-stock", "content", "kelly-invest-stock-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-invest-stock");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-invest-stock-${base.key}`, base.key);
