...
def test_demo_ui(browser, base_url: str) -> None:
    ...
    page.goto(f"{base_url}/?demo=1#/signals")
    page.wait_for_load_state("networkidle")
    assert page.locator("h1").inner_text() == "signals"
    assert page.locator(".item-row").count() == 3
    assert_no_horizontal_overflow(page)

    page.locator(".item-row").first.click()
    page.wait_for_timeout(150)
    assert "/signals/" in page.url
    assert page.locator(".detail").is_visible()
    assert page.locator("[data-action='approve']").is_visible()

    page.goto(f"{base_url}/?demo=1#/sources")
    page.wait_for_load_state("networkidle")
...
def test_busabase_provisioning(browser) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="kelly-beauty-intel-busabase-") as data_dir:
        busabase_command = [
            "npx",
            "-y",
        ...
        with managed_process(
            ...
            with tempfile.TemporaryDirectory(prefix="kelly-beauty-intel-home-") as app_home:
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health") as (
                    ...
                    try:
                        page.get_by_text("0 items").wait_for(timeout=10_000)
                    except Exception:
                ...
                record_cr = post_json(
                    ...
                    {
                        "fields": {
                            "signal-id": "signal-fixture",
                            "ref": 1,
                            "title": "Integration Fixture Signal",
                            "summary": "Seeded for the OSS integration test.",
                            "status": "needs_review",
                            "risk": "[]",
                        },
                        "message": "Seed Kelly Beauty Intel integration fixture",
                ...
                # and a human verdict on that record must write straight to Busabase.
                app_port = free_port()
                app_url = f"http://127.0.0.1:{app_port}"
                ...
                with managed_process(["node", "server.js"], APP_ROOT, app_env, f"{app_url}/health"):
                    ...
                    page.wait_for_load_state("networkidle")
                    assert page.locator("[data-provision]").count() == 0
                    assert page.get_by_text("Integration Fixture Signal").first.is_visible()
                    assert_no_horizontal_overflow(page)

                    page.locator(".item-row", has_text="Integration Fixture Signal").click()
                    page.wait_for_timeout(150)
                ...
                fixture = next(
                    ...
                    if (r.get("headCommit", {}).get("payload") or r.get("headCommit", {}).get("fields", {})).get("signal-id") == "signal-fixture"
                )
            ...
            assert sorted(keys) == sorted(
                ["app-root", "signals", "actions", "drafts", "sources", "settings"]
            ), nodes
