...
const appRoot = join(repoRoot, "skills", "kelly-beauty-intel", "content", "kelly-beauty-intel-app");
...
test("keeps the package manifest and runtime declarations aligned", async () => {
  const templateRoot = join(repoRoot, "skills", "kelly-beauty-intel");
  ...
  for (const base of appConfig.bases) {
    assert.equal(base.slug, `kelly-beauty-intel-${base.key}`, base.key);
...
test("does not persist secrets or a second data provider in browser storage", async () => {
  ...
  assert.doesNotMatch(source, /local-file-provider|app\/\.data/);
